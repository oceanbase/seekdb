...
int main(int argc, char **argv)
{
  ...
  CHECK(read_extension_package(root, "flat", "", source, error) == OB_SUCCESS);
  CHECK(source.native_module_ == "org.flat" && source.version_ == "2" && source.scripts_.size() == 2);
  CHECK(source.scripts_[0].sql_ == "SELECT '$libdir/flat';");
  CHECK(source.scripts_[1].sql_ == "SELECT '$libdir/flat_v2';");
  CHECK(validate_extension_package_source(source, error) == OB_SUCCESS);
  CHECK(read_extension_update(root, "flat", "1", "2", source, error) == OB_SUCCESS);
  CHECK(source.scripts_.size() == 1 && source.scripts_[0].sql_ == "SELECT '$libdir/flat_v2';");
  CHECK(read_extension_package(root, "ambiguous", "", source, error) == OB_INVALID_ARGUMENT);
  CHECK(source.name_.empty() && !error.empty());
  ...
  CHECK(read_extension_package(argv[2], "native_math", "", source, error) == OB_SUCCESS);
  CHECK(source.requires_superuser_);
  CHECK(source.version_ == "1.0" && source.native_module_ == "org.seekdb.sql_extension" && !source.native_install_);
  CHECK(source.scripts_.size() == 1 && source.scripts_[0].sql_.find("MODULE_PATHNAME") == std::string::npos);
  CHECK(source.scripts_[0].sql_.find("AS 'org.seekdb.sql_extension'") != std::string::npos);
  CHECK(source.scripts_[0].sql_.find(" RETURN ") == std::string::npos);
  CHECK(read_extension_update(argv[2], "native_math", "1.0", "1.1", source, error) == OB_SUCCESS);
  CHECK(source.scripts_.size() == 1 && source.scripts_[0].sql_.find("native_successor") != std::string::npos);
  CHECK(source.scripts_[0].sql_.find("CREATE FUNCTION native_increment") == std::string::npos);
  CHECK(source.scripts_[0].sql_.find("MODULE_PATHNAME") == std::string::npos);
  CHECK(read_extension_update(argv[2], "native_math", "1.1", "1.2", source, error) == OB_SUCCESS);
  CHECK(source.scripts_.size() == 1 && source.version_ == "1.2");
  CHECK(source.scripts_[0].sql_.find("DROP FUNCTION native_increment") != std::string::npos);
  CHECK(source.scripts_[0].sql_.find("DEFAULT 99") != std::string::npos);
  CHECK(source.scripts_[0].sql_.find("MODULE_PATHNAME") == std::string::npos);
  CHECK(read_extension_package(argv[2], "native_math", "1.2", source, error) == OB_SUCCESS);
  CHECK(source.scripts_.size() == 3 && source.version_ == "1.2");
  CHECK(read_extension_package(root, "policy", "1", source, error) == OB_SUCCESS);
  CHECK(!source.requires_superuser_ && validate_extension_package_source(source, error) == OB_SUCCESS);
  CHECK(read_extension_package(root, "policy", "3", source, error) == OB_SUCCESS);
  CHECK(source.requires_superuser_);
  CHECK(read_extension_update(root, "policy", "1", "3", source, error) == OB_SUCCESS);
  CHECK(source.requires_superuser_);
  CHECK(read_extension_update(root, "policy", "2", "3", source, error) == OB_SUCCESS);
  CHECK(!source.requires_superuser_);
