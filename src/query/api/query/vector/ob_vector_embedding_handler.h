...
#ifndef OCEANBASE_QUERY_VECTOR_OB_VECTOR_EMBEDDING_HANDLER_H_
#define OCEANBASE_QUERY_VECTOR_OB_VECTOR_EMBEDDING_HANDLER_H_
...
class ObIEmbeddingCallback
{
public:
  virtual ~ObIEmbeddingCallback() = default;
...
class ObEmbeddingTask
{
  ...
  int init(const ObString &model_url,
          ...
          ObIEmbeddingCallback *cb_handle = nullptr);
  template <typename ThreadPoolType>
  int do_work(ThreadPoolType *thread_pool);
  int64_t get_task_id() const { return task_id_; }
  // thread-safe accessors for virtual-table queries
  ...
  // public method for externally marking the task as failed
...
private:
  ...
  ObIEmbeddingCallback *cb_handle_;
