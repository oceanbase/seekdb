...
#include "query/engine/expr/ob_expr_in.h"
...
namespace query
{
class ObIPushdownAggregateRuntime;
}
namespace share
{
namespace aggregate
...
struct ObDatumRow;
// Query-owned description of a projected datum.  It references the query
// expression model, so it must not be owned by Storage's row representation.
...
class ObExternalFilterExecutionContext;
...
enum PushdownExecutorType //FARM COMPAT WHITELIST
{
  ...
  RESERVED_FILTER_EXECUTOR_7,
  RESERVED_FILTER_EXECUTOR_8,
  RESERVED_FILTER_EXECUTOR_9,
  RESERVED_FILTER_EXECUTOR_10,
  SEMISTRUCT_FILTER_EXECUTOR,
  EXTERNAL_FILTER_EXECUTOR,
  MAX_EXECUTOR_TYPE
};
...
public:
  ...
  {
    return AND_FILTER == type_ || OR_FILTER == type_;
  }
...
public:
  ...
  {
    return type_ == WHITE_FILTER_EXECUTOR || type_ == DYNAMIC_FILTER_EXECUTOR || type_ == SEMISTRUCT_FILTER_EXECUTOR;
  }
  ...
  virtual OB_INLINE bool is_external_filter_node() const { return type_ == EXTERNAL_FILTER_EXECUTOR; }
  ...
  virtual int execute_external_filter(ObExternalFilterExecutionContext &context)
  {
    UNUSED(context);
...
public:
  ObWhiteFilterParam()
      : datum_(nullptr), hash_func_(nullptr), cmp_func_(nullptr), access_ctx_(nullptr)
  {}
  ...
  inline bool operator==(const ObWhiteFilterParam &other) const
  {
    ...
    if (datum_->is_null() && other.datum_->is_null()) {
      ...
      int cmp_ret = 0;
      (*cmp_func_)(*datum_, *other.datum_, cmp_ret, access_ctx_);
      equal_ret = cmp_ret == 0;
    }
    return equal_ret;
  }
...
public:
  ObWhiteFilterParamHashSet()
      : set_(), hash_func_(nullptr), cmp_func_(nullptr), access_ctx_(nullptr)
  {}
  ~ObWhiteFilterParamHashSet()
  {
    destroy();
  ...
  {
    ObWhiteFilterParam param(&datum, &hash_func_, &cmp_func_, access_ctx_);
  ...
  inline void set_hash_and_cmp_func(const ObExprHashFuncType hash_func,
                                    const ObDatumCmpFuncType cmp_func,
                                    const ObDatumAccessContext *access_ctx)
  {
    hash_func_ = hash_func;
...
{
public:
  ObWhiteFilterSmallHashSet() : set_(), hash_func_(nullptr), access_ctx_(nullptr) {}
...
public:
  ObWhiteFilterExecutor(common::ObIAllocator &alloc,
      ...
      : ObPhysicalFilterExecutor(alloc, op, PushdownExecutorType::WHITE_FILTER_EXECUTOR),
        cmp_func_(nullptr), datum_access_ctx_(nullptr), null_param_contained_(false),
        datum_params_(alloc), filter_(filter)
      {}
  ...
  // Evaluate one materialized datum with this white-filter predicate.  The
  // predicate owns comparison semantics; readers only materialize datums.
  int filter_datum(const common::ObDatum &datum, bool &filtered) const;
...
protected:
  ...
  int init_param_set(const int64_t count,
                     const ObExpr *cur_arg,
                     const ObDatumAccessContext *access_ctx);
...
public:
  ...
  int init_small_set(const int64_t count,
                     const ObExprHashFuncType hash_func,
                     const ObDatumAccessContext *access_ctx);
...
private:
  DISALLOW_COPY_AND_ASSIGN(ObPushdownOperator);
};
...
struct PushdownFilterInfo
{
  ...
  {
    bool ret = is_inited_;
    if (is_pd_filter_ && !orig_filter_is_null_ && !filter_->is_sample_node() && !filter_->is_external_filter_node()) {
