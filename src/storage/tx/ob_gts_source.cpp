...
void ObGtsSource::reset()
{
  ...
  server_.reset();
  get_gts_queue_.reset();
  wait_gts_queue_.reset();
...
int ObGtsSource::init(const ObAddr &server)
{
  ...
  } else if (OB_FAIL(get_gts_queue_.init(GET_GTS))) {
    TRANS_LOG(WARN, "gts queue init error", KR(ret));
  } else if (OB_FAIL(wait_gts_queue_.init(WAIT_GTS_ELAPSING))) {
  ...
  if (OB_FAIL(ret)) {
    get_gts_queue_.destroy();
    wait_gts_queue_.destroy();
  }
...
int ObGtsSource::get_gts(ObTsCbTask *task, int64_t &gts)
{
  ...
  if (OB_SUCCESS == (ret = gts_local_cache_.get_gts(tmp_gts))) {
    ...
    if (OB_SUCCESS != (tmp_ret = get_gts_queue_.push(task))) {
...
{
  return get_gts_queue_.get_task_count() + wait_gts_queue_.get_task_count();
}

int ObGtsSource::gts_callback_interrupted(const int errcode, const share::ObLSID ls_id)
{
  int ret = OB_SUCCESS;
  get_gts_queue_.gts_callback_interrupted(errcode, ls_id);
  wait_gts_queue_.gts_callback_interrupted(errcode, ls_id);
  const int64_t task_count = get_task_count();
...
int ObGtsSource::wait_gts_elapse(const int64_t ts, ObTsCbTask *task, bool &need_wait)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_SUCCESS == ret && tmp_need_wait) {
      if (OB_FAIL(wait_gts_queue_.push(task))) {
...
int ObGtsSource::refresh_gts(const bool need_refresh)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_SUCCESS == (tmp_ret = gts_local_cache_.get_srr_and_gts_safe(drain_srr, drain_gts, drain_recv_ts))) {
      (void)get_gts_queue_.foreach_task(drain_srr, drain_gts, drain_recv_ts);
      (void)wait_gts_queue_.foreach_task(drain_srr, drain_gts, drain_recv_ts);
    }
  }
  statistics_();
...
int ObGtsSource::handle_gts_result(const int64_t queue_index)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    UNUSED(queue_index);
    if (OB_FAIL(get_gts_queue_.foreach_task(srr, gts, receive_gts_ts))) {
      if (OB_EAGAIN == ret) {
        ret = OB_SUCCESS;
      } else {
    ...
    if (OB_SUCC(ret) && OB_FAIL(wait_gts_queue_.foreach_task(srr, gts, receive_gts_ts))) {
      if (OB_EAGAIN == ret) {
        ret = OB_SUCCESS;
      } else {
        TRANS_LOG(WARN, "iterate wait task failed", KR(ret), K(queue_index));
      }
    }
    if (OB_SUCC(ret) && gts_local_cache_.no_rpc_on_road()) {
      int tmp_ret = OB_SUCCESS;
