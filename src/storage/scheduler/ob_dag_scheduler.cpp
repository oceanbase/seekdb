...
_RLOCAL(ObDagWorker *, ObDagWorker::self_);
_RLOCAL(bool, ObDagWorker::is_reserve_mode_);
_RLOCAL(compaction::ObCompactionMemoryContext *, ObDagWorker::mem_ctx_);
...
int ObDagWorker::start()
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObDagWorker not init", K(ret));
  } else if (OB_FAIL(lib::ThreadPool::start())) {
    LOG_WARN("failed to start ObDagWorker", K(ret));
  }
  return ret;
}

void ObDagWorker::stop()
{
  lib::ThreadPool::stop();
  notify(DWS_STOP);
}

void ObDagWorker::wait()
{
  lib::ThreadPool::wait();
}
...
bool ObDagWorker::get_force_cancel_flag()
{
...
void ObDagWorker::set_task(ObITask *task)
{
  task_ = task;
  hold_by_compaction_dag_ = false;
...
void ObDagWorker::run1()
{
  ...
  while (!has_set_stop()) {
    ...
    if (DWS_RUNNABLE == status_ && NULL != task_) {
      ...
      task_ = NULL;
      reset_compaction_thread_locals();
      if (OB_FAIL(::oceanbase::share::server_service<::oceanbase::share::ObDagScheduler>()->deal_with_finish_task(cur_task, *this, ret/*task error_code*/))) {
...
int ObDagWorker::yield()
{
  ...
  if (!((++counter) & CHECK_INTERVAL)) {
    ...
    if (last_check_time_ + check_period_ <= curr_time) {
      ...
      } else if (DWS_RUNNING == status_ && ::oceanbase::share::server_service<::oceanbase::share::ObDagScheduler>()->try_switch(*this)) {
        status_ = DWS_WAITING;
...
int ObDagPrioScheduler::init(
    ...
    ObIAllocator &reserved_allocator,
    ObDagScheduler &scheduler)
{
  int ret = OB_SUCCESS;
  if (0 >= dag_limit) {
    ret = OB_INVALID_ARGUMENT;
    COMMON_LOG(WARN, "init ObDagPrioScheduler with invalid arguments", K(ret), K(dag_limit));
  } else if (OB_FAIL(dag_map_.create(ObDagScheduler::DAG_MAP_BUCKET_NUM, "DagMap", "DagNode"))) {
    COMMON_LOG(WARN, "failed to create dap map", K(ret), K(dag_limit));
  } else {
    allocator_ = &allocator;
    reserved_allocator_ = &reserved_allocator;
    ...
    ObIDag &dag)
{
  ...
  if (OB_UNLIKELY(list_index >= DAG_LIST_MAX)) {
    ...
    bool add_ret = false;
    ObDagListIndex add_list_index = list_index;
    if (!emergency) {
    ...
    ObIDag *&dag)
{
  ...
  if (OB_ISNULL(dag) || OB_UNLIKELY(!dag->is_valid())) {
    ...
    COMMON_LOG(WARN, "ObDagScheduler is full", K(ret), "dag_limit", scheduler_->get_dag_limit((ObDagPrio::ObDagPrioEnum)dag->get_priority()), KPC(dag));
    ...
    const bool try_move_child)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    compaction::ObCompactionSuggestionMgr *suggestion_mgr = ::oceanbase::share::server_service<::oceanbase::compaction::ObCompactionSuggestionMgr>();
    ...
    if (need_add) {
      if (OB_TMP_FAIL(::oceanbase::share::server_service<::oceanbase::share::ObDagWarningHistoryManager>()->add_dag_warning_info(dag))) {
        ...
        if (OB_SUCCESS != dag->get_dag_ret()) {
          if (OB_TMP_FAIL(::oceanbase::share::server_service<::oceanbase::compaction::ObDiagnoseTabletMgr>()->add_diagnose_tablet(
        ...
        } else if (OB_TMP_FAIL(::oceanbase::share::server_service<::oceanbase::compaction::ObDiagnoseTabletMgr>()->delete_diagnose_tablet(
              merge_dag.tablet_id_, ObIDag::get_diagnose_tablet_type(dag->get_type())))) {
...
void ObDagPrioScheduler::pause_worker_(ObDagWorker &worker)
{
  if (OB_UNLIKELY(OB_ISNULL(scheduler_))) {
...
int ObDagPrioScheduler::loop_ready_dag_list(bool &is_found)
{
  ...
  if (!is_inited()) {
    ...
    ObMutexGuard guard(prio_lock_);
    if (running_task_cnts_ < limits_) {
      // if extra_erase_dag_net not null, the is_found must be false.
      is_found = (OB_SUCCESS == schedule_one_());

      while (running_task_cnts_ < limits_ && is_found) {
        if (OB_SUCCESS != schedule_one_()) {
          break;
        }
      }
    }
...
void ObDagPrioScheduler::dump_dag_status()
{
  ...
  COMMON_LOG(INFO, "dump_dag_status", "priority", OB_DAG_PRIOS[priority_].dag_prio_str_,
          K(limits), K(running_task), K(ready_dag_count), K(waiting_dag_count));
}
  ...
  if (is_compaction_dag_prio()) {
    ...
    for (int64_t idx = 0; idx < DAG_LIST_MAX; ++idx) {
      DLIST_FOREACH_NORET(dag, dag_list_[idx]) {
        ...
        } else if (OB_TMP_FAIL(::oceanbase::share::server_service<::oceanbase::compaction::ObDiagnoseTabletMgr>()->add_diagnose_tablet(merge_dag->tablet_id_,
    ...
    if (dag->is_dag_failed()) { // dag can retry & this task is the last running task
      if (OB_ALLOCATE_MEMORY_FAILED == error_code && is_mini_compaction_dag(dag->get_type())) {
        ...
        if (OB_ISNULL(mem_pool)) {
          LOG_WARN_RET(OB_ERR_UNEXPECTED, "get unexpected null runtime memory pool");
        } else {
...
bool ObDagPrioScheduler::try_switch(ObDagWorker &worker)
{
...
void ObDagNetScheduler::destroy()
{
  ...
  for (; iter != dag_net_map_.end(); ++iter) { // ignore failure
    ...
    if (OB_NOT_NULL(cur_dag_net)) {
      allocator = cur_dag_net->uses_reserved_allocator() ? reserved_allocator_ : allocator_;
    ...
    ObDagScheduler &scheduler)
{
  ...
  } else if (OB_FAIL(dag_net_map_.create(ObDagScheduler::DAG_MAP_BUCKET_NUM, "DagNetMap", "DagNetNode"))) {
    COMMON_LOG(WARN, "failed to create running dap net map", K(ret), K(dag_limit));
  } else if (OB_FAIL(dag_net_id_map_.create(ObDagScheduler::DAG_MAP_BUCKET_NUM, "DagNetIdMap", "DagNetIdNode"))) {
...
void ObDagNetScheduler::dump_dag_status()
{
  ...
  int64_t dag_net_count[ObDagNetType::DAG_NET_TYPE_MAX + 1];
  {
    ObMutexGuard guard(dag_net_map_lock_);
...
/***************************************ObDagScheduler impl********************************************/
int ObDagScheduler::server_module_init(ObDagScheduler* &scheduler)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(scheduler->init())) {
    COMMON_LOG(WARN, "failed to init ObDagScheduler", K(ret));
  } else {
    FLOG_INFO("success to init ObDagScheduler", K(ret), KP(scheduler));
  }
  return ret;
}
...
int ObDagScheduler::init_allocator(
    ...
    const int64_t dag_limit /*= DEFAULT_MAX_DAG_NUM*/)
{
  ...
  if (IS_INIT) {
  ...
  } else if (OB_FAIL(init_allocator("ReservedSched", reserved_mem_context_))) {
    COMMON_LOG(WARN, "failed to init reserved scheduler allocator", K(ret));
    ...
    COMMON_LOG(WARN, "failed to init independent dag allocator", K(ret));
      ...
      dag_limit, get_allocator(false/*use_reserved_allocator*/), get_allocator(true/*use_reserved_allocator*/), *this))) {
  ...
  // Initialize every priority scheduler. DAG classes may opt into the reserved
  // allocator independently of their scheduling priority.
  for (int64_t i = 0; OB_SUCC(ret) && i < ObDagPrio::DAG_PRIO_MAX; ++i) {
    if (OB_FAIL(prio_sche_[i].init(
        dag_limit, i, get_allocator(false/*use_reserved_allocator*/), get_allocator(true/*use_reserved_allocator*/), *this))) {
...
void ObDagScheduler::reset()
{
  ...
  if (NULL != reserved_mem_context_) {
    DESTROY_CONTEXT(reserved_mem_context_);
    reserved_mem_context_ = nullptr;
  }
...
void ObDagScheduler::inner_free_dag(ObIDag &dag)
{
  ObIAllocator &allocator = get_allocator(dag.uses_reserved_allocator());
  inner_free_dag(allocator, dag);
}

void ObDagScheduler::inner_free_dag(ObIAllocator &allocator, ObIDag &dag)
{
...
int ObDagScheduler::add_dag_net(ObIDagNet *dag_net)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int64_t ObDagScheduler::get_dag_limit(const ObDagPrio::ObDagPrioEnum dag_prio)
{
...
void ObDagScheduler::get_suggestion_reason(
    const int64_t priority,
    int64_t &reason)
{
...
void ObDagScheduler::inner_get_suggestion_reason(const ObDagType::ObDagTypeEnum type, int64_t &reason)
{
  reason = compaction::ObCompactionSuggestionMgr::ObCompactionSuggestionReason::MAX_REASON;
...
void ObDagScheduler::diagnose_for_suggestion()
{
  int tmp_ret = OB_SUCCESS;
  if (REACH_THREAD_TIME_INTERVAL(DUMP_DAG_STATUS_INTERVAL)) {
    ...
    for (int64_t i = 0; i < ObIDag::MergeDagPrioCnt; ++i) {
      ...
      } else if (FALSE_IT(value = prio_sche_[ObIDag::MergeDagPrio[i]].get_limit())) {
...
void ObDagScheduler::dump_dag_status(const bool force_dump/*false*/)
{
  int tmp_ret = OB_SUCCESS;
...
int ObDagScheduler::gene_basic_info(
    ObDagSchedulerInfo *info_list,
    common::ObIArray<void *> &scheduler_infos,
    int64_t &idx)
{
...
common::ObIAllocator &ObDagScheduler::get_allocator(const bool use_reserved_allocator)
{
  common::ObIAllocator *allocator = &mem_context_->get_malloc_allocator();
  if (use_reserved_allocator) {
    allocator = &reserved_mem_context_->get_malloc_allocator();
  }
  return *allocator;
}

int ObDagScheduler::get_all_dag_scheduler_info(
...
int ObDagScheduler::get_all_dag_info(
    common::ObIAllocator &allocator,
    common::ObIArray<void *> &dag_infos)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::get_all_compaction_dag_info(
    ObIAllocator &allocator,
    ObIArray<compaction::ObTabletCompactionProgress *> &progress_array)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::get_minor_exe_dag_info(
...
int ObDagScheduler::check_compaction_dag_exist_with_cancel(bool &exist)
{
  int ret = OB_SUCCESS;
  exist = false;
...
int ObDagScheduler::get_min_end_scn_from_major_dag(SCN &min_end_scn)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::get_compaction_dag_count(int64_t dag_count)
{
  int ret = OB_SUCCESS;
  dag_count = 0;
  for (int64_t i = 0; OB_SUCC(ret) && i < ObIDag::MergeDagPrioCnt; ++i) {
...
int ObDagScheduler::diagnose_minor_exe_dag(
    const compaction::ObMergeDagHash *merge_dag_info,
    compaction::ObDiagnoseTabletCompProgress &progress)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::get_max_major_finish_time(const int64_t version, int64_t &estimated_finish_time)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDagScheduler::diagnose_dag(
    const ObIDag *dag,
    compaction::ObDiagnoseTabletCompProgress &progress)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::diagnose_dag_net(
    ObIDagNet *dag_net,
    common::ObIArray<compaction::ObDiagnoseTabletCompProgress> &progress_list,
    ObDagId &dag_net_id,
    int64_t &start_time)
...
int ObDagScheduler::diagnose_all_compaction_dags()
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
...
int ObDagScheduler::deal_with_finish_task(ObITask *&task, ObDagWorker &worker, int error_code)
{
  int ret = OB_SUCCESS;
  ObIDag *dag = nullptr;
  if (OB_ISNULL(task)) {
...
void ObDagScheduler::finish_dag_net(ObIDagNet *dag_net)
{
  int tmp_ret = OB_SUCCESS;
...
bool ObDagScheduler::dag_count_overflow(const ObDagType::ObDagTypeEnum type)
{
  return get_dag_count(type) >= get_dag_limit(OB_DAG_TYPES[type].init_dag_prio_);
}

int64_t ObDagScheduler::allowed_schedule_dag_count(const ObDagType::ObDagTypeEnum type)
{
  int64_t count = get_dag_limit(OB_DAG_TYPES[type].init_dag_prio_) - get_dag_count(type);
  return count < 0 ? 0 : count;
}

int64_t ObDagScheduler::get_dag_count(const ObDagType::ObDagTypeEnum type)
{
  int64_t count = -1;
...
int64_t ObDagScheduler::get_dag_net_count(const ObDagNetType::ObDagNetTypeEnum type)
{
...
void ObDagScheduler::loop_dag_net()
{
  int tmp_ret = OB_SUCCESS;
...
int ObDagScheduler::loop_ready_dag_lists()
{
  int ret = OB_SUCCESS;
  bool is_found = false;
...
int ObDagScheduler::dispatch_task(ObITask &task, ObDagWorker *&ret_worker, const int64_t priority)
{
  int ret = OB_SUCCESS;
  ret_worker = NULL;
...
{
  int ret = OB_SUCCESS;
  // TODO add an upper worker count limit
...
int ObDagScheduler::set_compaction_dag_limit(const int64_t new_val)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::set_thread_score(const int64_t priority, const int64_t score)
{
  int ret = OB_SUCCESS;
  int64_t old_val = 0;
  int64_t new_val = 0;
...
int64_t ObDagScheduler::get_running_task_cnt(const ObDagPrio::ObDagPrioEnum priority)
{
  int64_t count = -1;
...
int ObDagScheduler::get_limit(const int64_t prio, int64_t &limit)
{
  int ret = OB_SUCCESS;
  limit = 0;
  if (IS_NOT_INIT) {
...
int ObDagScheduler::cancel_dag(const ObIDag *dag, const bool force_cancel)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::check_dag_net_exist(
    const ObDagId &dag_id, bool &exist)
{
  int ret = OB_SUCCESS;
...
int ObDagScheduler::cancel_dag_net(const ObDagId &dag_id)
{
  int ret = OB_SUCCESS;
...
// for unittest
int ObDagScheduler::get_first_dag_net(ObIDagNet *&dag_net)
{
  int ret = OB_SUCCESS;
  dag_net = nullptr;
  if (IS_NOT_INIT) {
