...
struct ObSparseRetrievalMergeParam
{
  ...
  TO_STRING_KV(KPC_(dim_weights), KPC(limit_param_), KP_(eval_ctx),
      ...
      K_(topk_limit), K_(max_batch_size), K_(match_only), K_(drain_after_limit));
  ...
  bool match_only_;
  // Keep consuming the bounded input after enough output rows are buffered.
  // This leaves vectorized child scans at a reusable batch boundary.
  bool drain_after_limit_;
};
