    ...
    int64_t &cmp_ret)
{
  ...
  } else if (OB_FAIL(cmp_fast(l, r, cmp_ret))) {
...
int ObSRDaaTIterImpl::cache_result(int64_t &count, const ObDatum &id_datum, const double relevance)
{
  ...
  } else if (OB_FAIL(buffered_domain_id_data_[count].from_datum_fast(id_datum))) {
    LOG_WARN("failed to cache domain id", K(ret), K(id_datum));
  } else {
