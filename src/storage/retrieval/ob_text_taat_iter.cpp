...
int ObTextTaaTIter::pre_process()
{
  ...
  if (!iter_param_->need_project_relevance()) {
    // Score-free TAAT plans have no BM25 document-count estimator.  They still
    // need at least one hash partition; leaving this at zero makes the base
    // iterator report end-of-stream before reading any posting list.
    partition_cnt_ = 1;
  } else {
