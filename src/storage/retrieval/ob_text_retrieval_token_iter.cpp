...
{
  // A score-free retrieval plan has no inverted-index aggregate parameter.
  // Its iterator can still be reused, but must not dereference that optional
  // parameter while deciding whether the cached document count is valid.
...
int ObTextRetrievalTokenIter::update_scan_param(const ObString &token, common::ObArenaAllocator &allocator)
{
  ...
  if (OB_ISNULL(range_template_param)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected null range template param", K(ret), KP(range_template_param),
             K(need_inv_idx_agg()));
    ...
    // Relevance plans use the aggregate range as the token range template.
    // Score-free plans deliberately omit that aggregate scan, so TAAT must
    // derive the next token range from the inverted-index scan itself.
...
int ObTextRetrievalDaaTTokenIter::save_docids()
{
  ...
  if (OB_ISNULL(inv_scan_domain_id_col_)) {
    ...
    const ObDatumVector &doc_id_datum = inv_scan_domain_id_col_->locate_expr_datumvector(*eval_ctx_);
    for (int64_t i = 0; OB_SUCC(ret) && i < count_; ++i) {
      // In the score-free path no relevance expression evaluates this skip
      // bitmap.  count_ still describes every row returned by the inverted
      // scan, so DaaT must materialize a docid for every one of those slots.
      if (OB_FAIL(doc_id_[i].from_datum(*doc_id_datum.at(i)))) {
...
int ObTextRetrievalDaaTTokenIter::get_curr_score(double &score) const
{
  ...
  if (OB_UNLIKELY(cur_idx_ >= count_)) {
    ...
    // Presence is sufficient for score-free natural-language matching.  The
    // merge collector still expects every dimension iterator to initialize
    // its score output even when no score is projected.
    score = 1.0;
  }
  return ret;
}
