...
struct ObSRMergeCmp
{
  ...
  OB_INLINE int cmp_fast(const ObSRMergeItem &l, const ObSRMergeItem &r, int64_t &cmp_ret)
  {
    ...
    if (OB_UNLIKELY(left.is_null() || right.is_null())) {
      ...
      cmp_ret = left.get_int() < right.get_int() ? -1 : left.get_int() > right.get_int() ? 1 : 0;
    } else if (UINT64_CMP == fast_cmp_type_) {
      cmp_ret = left.get_uint64() < right.get_uint64()
          ? -1 : left.get_uint64() > right.get_uint64() ? 1 : 0;
    } else if (BINARY_STRING_CMP == fast_cmp_type_) {
      const int64_t min_len = MIN(left.len_, right.len_);
      const int byte_cmp = min_len > 0 ? MEMCMP(left.ptr_, right.ptr_, min_len) : 0;
      cmp_ret = byte_cmp > 0 ? 1 : (byte_cmp < 0 ? -1
          : (left.len_ > right.len_ ? 1 : (left.len_ < right.len_ ? -1 : 0)));
    } else {
      int tmp_ret = 0;
      if (OB_FAIL(cmp_func_(left, right, tmp_ret))) {
      } else {
