...
int ObCheckpointDiagnoseMgr::init()
{
  ...
  if (IS_INIT) {
    ...
    for (int i = 0; OB_SUCC(ret) && i < MAX_TRACE_INFO_ARR_SIZE; i++) {
      ...
      if (OB_FAIL(trace_info_arr_[i].memtable_diagnose_info_map_.create(bucket_count, "CkptDgnMem", "CkptDgnMemNode"))) {
        LOG_WARN("failed to create map", KR(ret));
      } else if (OB_FAIL(trace_info_arr_[i].checkpoint_unit_diagnose_info_map_.create(bucket_count, "CkptDgnMemCU", "CkptDgnCUNode"))) {
        LOG_WARN("failed to create map", KR(ret));
      }
    }
    if (OB_SUCC(ret)) {
    ...
    const int logstream_clock)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (checkpoint::INVALID_TRACE_ID == tmp_trace_id) {
      share::g_mp->checkpoint_diagnose_mgr()->acquire_trace_id(ls_id, tmp_trace_id);
    }
    if (checkpoint::INVALID_TRACE_ID != tmp_trace_id) {
