...
int ObFragmentWriterV2<T>::flush_buffer()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_FAIL(SERVER_TMP_FILE_MANAGER.aio_write(io_info, file_io_handle_))) {
...
int ObFragmentReaderV2<T>::prefetch()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      } else if (OB_FAIL(SERVER_TMP_FILE_MANAGER.aio_read(io_info, file_io_handles_[handle_index]))) {
