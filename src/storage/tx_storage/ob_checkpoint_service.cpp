...
void ObCheckPointService::ObCheckpointTask::runTimerTask()
{
  ...
  if (OB_ISNULL(ls_svr)) {
    ...
    if (OB_FAIL(ls_svr->get_ls(share::SYS_LS, ls_handle, ObLSGetMod::APPLY_MOD))) {
      if (OB_LS_NOT_EXIST == ret) {
        ret = OB_SUCCESS;
      } else {
        STORAGE_LOG(WARN, "get sys ls failed", K(ret));
      }
    } else if (OB_ISNULL(ls = ls_handle.get_ls())) {
      ret = OB_ERR_UNEXPECTED;
      STORAGE_LOG(WARN, "log stream not exist", K(ret), K(share::SYS_LS));
    } else if (FALSE_IT(++ls_cnt)) {
  ...
  if (OB_ISNULL(ls_svr)) {
    ...
    auto flush_ls = [&](ObLS &ls) -> int {
      ...
      if (OB_ISNULL(checkpoint_executor) || OB_ISNULL(data_checkpoint)) {
        ...
        STORAGE_LOG(WARN, "flush ls to recycle clog failed", KR(tmp_ret), K(ls));
      } else {
        ++succ_ls_cnt;
      }
      return ret;
    };
    ret = ls_svr->foreach_ls(flush_ls);
