...
bool ObTenantFreezer::exist_ls_freezing()
{
  ...
  if (IS_NOT_INIT) {
    ...
    auto check_ls_freezing = [&](ObLS &ls) -> int {
      if (ls.get_freezer()->is_ls_freeze_running()) {
        exist_ls_freezing = true;
      }
      return OB_SUCCESS;
    };
    if (OB_FAIL(ls_srv->foreach_ls(check_ls_freezing))) {
...
bool ObTenantFreezer::exist_ls_throttle_is_skipping()
{
  ...
  if ((cur_ts - last_update_ts > UPDATE_INTERVAL) &&
    ...
    if (IS_NOT_INIT) {
      ...
      auto check_ls_throttle = [&](ObLS &ls) -> int {
        if (ls.get_freezer()->throttle_is_skipping()) {
          exist_ls_throttle_is_skipping = true;
        }
        return OB_SUCCESS;
      };
      if (OB_FAIL(ls_srv->foreach_ls(check_ls_throttle))) {
        LOG_WARN("[TenantFreezer] iter ls failed", K(ret));
      }
    }
...
int ObTenantFreezer::tenant_freeze_data_()
{
  ...
  if (OB_FAIL(ls_srv->foreach_ls(freeze_ls))) {
    LOG_WARN("[TenantFreezer] fail to foreach log stream", KR(ret));
  } else {
    if (ls_cnt > 0) {
...
int ObTenantFreezer::tenant_freeze(const ObFreezeSourceFlag source)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(ls_svr->foreach_ls(freeze_ls))) {
      LOG_WARN("foreach log stream failed", K(ret));
    }
  }
                                                  ...
                                                  bool for_statistic_print)
{
  ...
  if (IS_NOT_INIT) {
    ...
    auto collect_tx_data_info = [&](ObLS &ls) -> int {
      ++ls_cnt;
      int tmp_ret = OB_SUCCESS;
    ...
    if (OB_FAIL(ls_srv->foreach_ls(collect_tx_data_info))) {
      LOG_WARN("[TenantFreezer] fail to foreach log stream", KR(ret));
    } else if (0 == ls_cnt) {
