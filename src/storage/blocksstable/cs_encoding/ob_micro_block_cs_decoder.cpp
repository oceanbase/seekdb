...
ObICSEncodeBlockReader::ObICSEncodeBlockReader()
  : request_cnt_(0), cached_decoder_(NULL), decoders_(nullptr),
    ...
    buf_allocator_(ObMemAttr(MTL_ID(), "IENB_CSREADER"), OB_MALLOC_NORMAL_BLOCK_SIZE),
...
ObMicroBlockCSDecoder::ObMicroBlockCSDecoder()
  : request_cnt_(0), cached_decoder_(nullptr), decoders_(nullptr),
    ...
    transform_allocator_(ObMemAttr(MTL_ID(), "MICB_TRANSFORM"), OB_MALLOC_NORMAL_BLOCK_SIZE),
    buf_allocator_(ObMemAttr(MTL_ID(), "MICB_CSDECODER"), OB_MALLOC_NORMAL_BLOCK_SIZE),
    allocated_decoders_buf_(nullptr),
    allocated_decoders_buf_size_(0),
    ...
    const storage::ObAggCellBase &agg_cell)
{
  ...
  if (OB_UNLIKELY(column_decoder->decoder_->is_new_column())) {
    bret = (PD_MIN == agg_type || PD_MAX == agg_type || PD_STR_PREFIX_MIN == agg_type || PD_STR_PREFIX_MAX == agg_type);
