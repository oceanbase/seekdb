...
#include "ob_tablet_local_status.h"
...
ObTabletLocalStatus::ObTabletLocalStatus()
...
bool ObTabletLocalStatus::is_valid() const
{
  return ObTabletRestoreStatus::is_valid(restore_status_)
      && ObTabletDataStatus::is_valid(data_status_);
}

int ObTabletLocalStatus::serialize(char *buf, const int64_t len, int64_t &pos) const
{
  int ret = OB_SUCCESS;
  int64_t new_pos = pos;
...
int ObTabletLocalStatus::deserialize(const char *buf, const int64_t len, int64_t &pos)
{
  int ret = OB_SUCCESS;
  int64_t new_pos = pos;
...
int64_t ObTabletLocalStatus::get_serialize_size() const
{
  return serialization::encoded_length_i64(local_status_);
}

int ObTabletLocalStatus::set_restore_status(const ObTabletRestoreStatus::STATUS &restore_status)
{
  int ret = OB_SUCCESS;
...
int ObTabletLocalStatus::get_restore_status(ObTabletRestoreStatus::STATUS &restore_status) const
{
  int ret = OB_SUCCESS;
  if (!is_valid()) {
    ret = OB_NOT_INIT;
...
int ObTabletLocalStatus::set_data_status(const ObTabletDataStatus::STATUS &data_status)
{
  int ret = OB_SUCCESS;
...
int ObTabletLocalStatus::get_data_status(ObTabletDataStatus::STATUS &data_status) const
{
  int ret = OB_SUCCESS;
  if (!is_valid()) {
    ret = OB_NOT_INIT;
...
int ObTabletLocalStatus::set_expected_status(const ObTabletExpectedStatus::STATUS &expected_status)
{
  int ret = OB_SUCCESS;
...
int ObTabletLocalStatus::get_expected_status(ObTabletExpectedStatus::STATUS &expected_status) const
{
  int ret = OB_SUCCESS;
...
int ObTabletLocalStatus::init_status()
{
  int ret = OB_SUCCESS;
...
void ObTabletLocalStatus::reset()
{
