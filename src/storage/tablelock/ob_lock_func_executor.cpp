                               ...
                               const int64_t timeout_us)
{
  ...
  if (OB_SUCC(ret)) {
    SMART_VAR(ObLockContext, stack_ctx2) {
      ...
      OZ (lock_obj_(sess, tx_param, session_id, session_create_ts, lock_id, timeout_us));
                                 ...
                                 const int64_t timeout_us)
{
  ...
  } else if (OB_FAIL(arg.owner_id_.convert_from_session_id(session_id, session_create_ts))) {
