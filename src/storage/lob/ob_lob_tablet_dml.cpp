    ...
    ObLobAccessParam &lob_param)
{
  ...
  lob_param.namespace_access_mode_ = run_ctx.dml_param_.namespace_access_mode_;
    ...
    blocksstable::ObStorageDatum &new_datum)
{
  ...
  if (OB_FAIL(new_lob.get_lob_data_byte_len(lob_param.update_len_))) {
    ...
    if (OB_FAIL(insert_lob_col(run_ctx, new_row, col_idx, new_datum, nullptr, disk_locator_data/*empty data*/))) {
    }
