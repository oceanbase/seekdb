...
int ObComplementDataParam::prepare_task_ranges()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (user_parallelism_ <= 1) {
      ...
      if (OB_FAIL(split_task_ranges(task_id_, orig_ls_id_, orig_tablet_id_, orig_schema_tablet_size_, user_parallelism_))) {
    ...
    const int64_t hint_parallelism)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_UNLIKELY(task_id <= 0 || !ls_id.is_valid() || !tablet_id.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (OB_FAIL(ranges.push_back(range))) {
      ...
      if (OB_SUCC(ret)) {
        ...
        FLOG_INFO("succeed to get range and concurrent cnt", K(ret), K(task_id), K(tablet_id),
    ...
    const share::schema::ObTableSchema &hidden_table_schema)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    direct_load_type_ = ObDDLUtil::use_idempotent_mode() ?
