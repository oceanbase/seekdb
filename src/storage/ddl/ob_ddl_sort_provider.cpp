...
int ObDDLSortProvider::build_row_meta_from_schema(const ObDDLTableSchema &ddl_schema, ObIAllocator &rowmeta_allocator, sql::RowMeta &sk_row_meta)
{
  ...
  ObSEArray<ObExpr, 8> expr_storage;
  ...
  } else if (OB_FAIL(expr_storage.reserve(rowkey_cnt))) {
    LOG_WARN("reserve expr storage failed", K(ret), K(rowkey_cnt));
  } else if (OB_FAIL(exprs.reserve(rowkey_cnt))) {
    LOG_WARN("reserve expr ptrs failed", K(ret), K(rowkey_cnt));
  }
  for (int64_t i = 0; OB_SUCC(ret) && i < rowkey_cnt; ++i) {
    const ObColumnSchemaItem &column_item = column_items.at(i);
    ObExpr expr;
    expr.is_fixed_length_data_ = is_fixed_length(column_item.col_type_.get_type());
    expr.len_ = expr.is_fixed_length_data_ ? get_type_fixed_length(column_item.col_type_.get_type()) : 0;
    if (OB_FAIL(expr_storage.push_back(expr))) {
      LOG_WARN("push back expr failed", K(ret), K(i));
    } else if (OB_FAIL(exprs.push_back(&expr_storage.at(expr_storage.count() - 1)))) {
      LOG_WARN("push back expr ptr failed", K(ret), K(i));
    }
  }
  if (OB_SUCC(ret)) {
    const int32_t extra_size = StoreRow::get_extra_size(true /*is_sort_key*/);
    if (OB_FAIL(sk_row_meta.init(exprs, extra_size,
...
int ObDDLSortProvider::build_full_row_meta_from_schema(const ObDDLTableSchema &ddl_schema, const bool enable_encode_sortkey, ObIAllocator &rowmeta_allocator, sql::RowMeta &full_row_meta)
{
  ...
  ObSEArray<ObExpr, 16> expr_storage;
  ObSEArray<ObExpr *, 16> exprs;
  if (OB_UNLIKELY(rowkey_cnt <= 0 || column_items.count() < rowkey_cnt)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid rowkey schema", K(ret), K(rowkey_cnt), K(column_items.count()));
  } else if (OB_FAIL(expr_storage.reserve(info_cnt))) {
    LOG_WARN("reserve expr storage failed", K(ret), K(column_items.count()), K(info_cnt));
  } else if (OB_FAIL(exprs.reserve(info_cnt))) {
    LOG_WARN("reserve expr ptrs failed", K(ret), K(column_items.count()), K(info_cnt));
  } else if (enable_encode_sortkey) {
    ObExpr expr;
    expr.is_fixed_length_data_ = false;
    expr.len_ = 0;
    if (OB_FAIL(expr_storage.push_back(expr))) {
      LOG_WARN("push back encode sortkey expr failed", K(ret));
    } else if (OB_FAIL(exprs.push_back(&expr_storage.at(expr_storage.count() - 1)))) {
      LOG_WARN("push back encode sortkey expr ptr failed", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    const int32_t extra_size = StoreRow::get_extra_size(true /*is_sort_key*/);
    if (OB_FAIL(full_row_meta.init(exprs, extra_size,
                                  false /*reorder_fixed_expr*/,
                                  &rowmeta_allocator))) {
