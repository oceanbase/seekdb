...
ObDDLSortProvider::ObDDLSortProvider()
  : is_inited_(false),
    ...
    op_monitor_info_(),
    sk_row_meta_(),
    rowmeta_allocator_(ObMemAttr("DDLSortRowMeta")),
...
int ObDDLSortProvider::init(ObDDLIndependentDag *ddl_dag)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
  ...
  } else if (OB_FAIL(sort_impl_map_.create(64, ObMemAttr("DDLSortMap")))) {
    LOG_WARN("create sort impl map failed", K(ret));
  } else if (OB_FAIL(slice_sort_impl_map_.create(64, ObMemAttr("DDLSliceSortMap")))) {
    ...
    ddl_dag_ = ddl_dag;
    tempstore_read_alignment_size_ = 2L << 20;
    is_inited_ = true;
  }
  if (OB_FAIL(ret)) {
    destroy();
...
int ObDDLSortProvider::init_sort_impl(const bool need_slice_decider, ObDDLSortProvider::SortHandle *&handle)
{
  ...
  if (OB_FAIL(ret)) {
                                  ...
                                  max_batch_size,
                                  ObMemAttr("DDLSortImpl", ObCtxIds::WORK_AREA),
                                  compressor_type,
                                  false /*enable_trunc*/,
...
int ObDDLSortProvider::get_final_merge_ways(int64_t &final_merge_ways)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    final_merge_ways = MAX_MERGE_MEMORY_PER_THREAD / max(tempstore_read_alignment_size_, 2L << 20);
...
int ObDDLSortProvider::create_sort_handle(const bool need_slice_decider, SortHandle *&handle)
{
  ...
  param.set_mem_attr(ObMemAttr("DDLSortRow", ObCtxIds::WORK_AREA)).set_properties(lib::ALLOC_THREAD_SAFE);
