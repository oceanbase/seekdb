...
int ObDDLIndependentDag::init_ddl_table_schema()
{
  ...
  if (OB_SUCC(ret) && ddl_task_param_.is_partition_local_ && is_fts_doc_word_aux(ddl_table_schema_.table_item_.index_type_)) {
    ...
    static constexpr char DOC_WORD_SUFFIX[] = "_fts_doc_word";
    const int64_t DOC_WORD_SUFFIX_LEN = STRLEN(DOC_WORD_SUFFIX);
    if (OB_FAIL(schema_guard.get_table_schema(table_id, table_schema))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && OB_INVALID_ID == fts_table_id && i < index_infos.count(); i++) {
        if (is_fts_index_aux(index_infos.at(i).index_type_)) {
          if (OB_FAIL(schema_guard.get_table_schema(index_infos.at(i).table_id_, word_doc_table_schema))) {
            LOG_WARN("get table schema failed", K(ret), K(index_infos.at(i).table_id_));
          } else if (OB_ISNULL(word_doc_table_schema)) {
            ret = OB_TABLE_NOT_EXIST;
            LOG_WARN("table not exist", K(ret), K(index_infos.at(i).table_id_));
...
int ObDDLIndependentDag::init_sort_ls_tablet_ids()
{
  ...
  if (OB_UNLIKELY(OB_INVALID_TENANT_ID == tenant_id)) {
    ...
    if (OB_FAIL(ObMultiVersionSchemaService::get_instance().get_tenant_schema_guard(schema_guard))) {
    ...
    } else if (OB_FAIL(schema_guard.get_table_schema(ddl_table_schema_.table_id_, doc_word_table_schema))) {
      LOG_WARN("get table schema failed", K(ret), K(ddl_table_schema_.table_id_));
    } else if (OB_FAIL(schema_guard.get_table_schema(sort_ddl_table_schema->table_id_, word_doc_table_schema))) {
      LOG_WARN("get table schema failed", K(ret), K(sort_ddl_table_schema->table_id_));
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < ls_tablet_ids_.count(); ++i) {
        ...
        if (OB_FAIL(doc_word_table_schema->get_part_idx_by_tablet(ls_tablet_ids_.at(i).second, part_idx, subpart_idx))) {
          LOG_WARN("failed to get part idx", K(ret), K(i), "tablet_id", ls_tablet_ids_.at(i).second);
        } else if (OB_FAIL(part_idxs.push_back(part_idx))) {
          LOG_WARN("failed to push back part idx", K(ret), K(part_idx));
        } else if (PARTITION_LEVEL_TWO == doc_word_table_schema->get_part_level()
                   && OB_FAIL(subpart_idxs.push_back(subpart_idx))) {
          LOG_WARN("failed to push back subpart idx", K(ret), K(subpart_idx));
        }
      }
    }
    if (OB_FAIL(ret)) {
...
{
  return generate_start_tasks(start_tasks, nullptr);
}
...
int ObDDLIndependentDag::generate_write_macro_block_tasks(ObIArray<ObITask *> &need_schedule_tasks, ObITask *next_task)
{
  return generate_fixed_tasks(need_schedule_tasks, next_task);
}
