...
ObColumnClusteredDag::ObColumnClusteredDag()
  : px_thread_count_(0), px_finished_count_(0), sample_scan_finished_count_(0),
    mutex_(common::ObLatchIds::COLUMN_STORE_DDL_RESCAN_LOCK),
    is_range_count_ready_(false), total_slice_count_(0), use_static_plan_(true), sample_cond_()
{
  ...
  if (OB_UNLIKELY(nullptr == init_param || !init_param->is_valid())) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (is_partition_local && is_fts_doc_word_aux(ddl_table_schema_.table_item_.index_type_)) {
        ...
        if (OB_FAIL(generate_write_macro_block_tasks(need_schedule_tasks))) {
                                                           ...
                                                           common::ObIArray<common::ObNewRange> &ranges)
{
  int ret = OB_SUCCESS;
  ranges.reset();
  const uint64_t table_id = ddl_table_schema_.table_id_;
