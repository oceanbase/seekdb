...
ObDDLSortChunk::ObDDLSortChunk()
  : sort_op_chunk_(nullptr), file_size_(0), allocator_(nullptr)
{
}
...
ObDDLSlice::ObDDLSlice()
  : is_inited_(false), has_end_chunk_(false), tablet_id_(), slice_idx_(-1),
    chunk_queue_(), remain_cg_blocks_(), ddl_sort_chunks_(),
    sorted_mutex_(common::ObLatchIds::DDL_SORT_PROVIDER_LOCK)
{
...
ObDDLSlice::~ObDDLSlice()
{
  ...
  chunk_queue_.destroy();
  {
    ...
    for (int64_t i = 0; i < ddl_sort_chunks_.count(); ++i) {
      ddl_sort_chunks_.at(i).free_sort_op_chunk();
    }
    ddl_sort_chunks_.reset();
  }
}

int ObDDLSlice::push_sorted_chunk(ChunkType *&sort_op_chunk, const int64_t file_size,
                                  ObIAllocator *chunk_allocator)
{
  int ret = OB_SUCCESS;
...
static bool compare_ddl_sort_chunk_by_file_size_desc(const ObDDLSortChunk &a,
                                                     const ObDDLSortChunk &b)
{
  return a.get_file_size() > b.get_file_size();
}

int ObDDLSlice::pop_sorted_chunks(const int64_t final_merge_ways,
                                  const int64_t current_merge_ways,
                                  common::ObIArray<ObDDLSortChunk> &ddl_sort_chunks)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(final_merge_ways <= 2 || current_merge_ways <= 2)) {
    ...
    if (ddl_sort_chunks_.count() <= final_merge_ways) {
      ...
      const int64_t pop_cnt = min(current_merge_ways,
                                  ddl_sort_chunks_.count() - final_merge_ways + 1);
      ob_sort(ddl_sort_chunks_.begin(), ddl_sort_chunks_.end(),
              compare_ddl_sort_chunk_by_file_size_desc);
      for (int64_t i = 0; OB_SUCC(ret) && i < pop_cnt; ++i) {
        const int64_t chunk_idx = ddl_sort_chunks_.count() - 1 - i;
        if (OB_FAIL(ddl_sort_chunks.push_back(ddl_sort_chunks_.at(chunk_idx)))) {
...
ObDDLTabletContext::ObDDLTabletContext()
  : is_inited_(false), arena_(ObMemAttr("ddl_tblt_ctx")),
    ...
    fts_forward_part_ranges_(), fts_inverted_part_ranges_(),
    fts_forward_final_range_(), fts_inverted_final_range_(), fts_expect_range_cnt_(0)
{
...
int ObDDLTabletContext::append_sample_range(const bool is_inverted,
                                            const sql::ObPxTabletRange &range)
{
  int ret = OB_SUCCESS;
...
int ObDDLTabletContext::set_final_sample_range(const bool is_inverted,
                                               const sql::ObPxTabletRange &range)
{
  int ret = OB_SUCCESS;
  lib::ObMutexGuard guard(mutex_);
  sql::ObPxTabletRange &target =
      is_inverted ? fts_inverted_final_range_ : fts_forward_final_range_;
  int64_t pos = 0;
  target.reset();
  if (OB_FAIL(target.deep_copy_from<true>(range, arena_, nullptr, 0, pos))) {
...
int ObDDLTabletContext::set_expect_range_count(const int64_t expect_cnt)
{
  ...
  if (expect_cnt <= 0) {
    ...
    lib::ObMutexGuard guard(mutex_);
    fts_expect_range_cnt_ = expect_cnt;
  }
  return ret;
}
