...
{
  return ATOMIC_LOAD(&is_independent_freezed_);
}
...
{
  UNUSEDx(table_key, ls, freezer, memtable_mgr, schema_version, freeze_clock);
...
{
  UNUSEDx(param, context, key_range);
  row_iter = nullptr;
  return OB_NOT_SUPPORTED;
}
...
int ObDDLKV::get(
    ...
    ObDatumRow &row)
{
  UNUSEDx(param, context, rowkey, row);
  return OB_NOT_SUPPORTED;
}
