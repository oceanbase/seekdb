...
int64_t ObDirectLoadMgr::generate_context_id()
{
  return ATOMIC_AAF(&context_id_generator_, 1);
}

ObDirectLoadMgr::ObDirectLoadMgr()
...
int64_t ObDirectLoadMgr::generate_slice_id()
{
  return ATOMIC_AAF(&slice_id_generator_, 1);
}

int ObDirectLoadMgr::server_module_init(ObDirectLoadMgr *&direct_load_mgr)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(direct_load_mgr)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("unexpected error", K(ret));
  } else if (OB_FAIL(direct_load_mgr->init())) {
    LOG_WARN("init failed", K(ret));
  }
  return ret;
}

int ObDirectLoadMgr::init()
{
  int ret = OB_SUCCESS;
  ...
  lib::ObMemAttr attr("DirectLoadMgr");
  ...
  } else if (OB_FAIL(bucket_lock_.init(bucket_num, ObLatchIds::DIRECT_LOAD_MGR_LOCK,
      ObLabel("TenDLBucket")))) {
...
int ObDirectLoadMgr::get_agent_exec_context(
...
// 3. LS online creates it for an existing direct-load tablet.
int ObDirectLoadMgr::create_tablet_direct_load(
    const int64_t context_id,
...
int ObDirectLoadMgr::replay_create_tablet_direct_load(
...
int ObDirectLoadMgr::alloc_tablet_direct_load_mgr(
...
int ObDirectLoadMgr::try_create_tablet_direct_load_mgr_nolock(
    ...
    ObTabletDirectLoadMgrHandle &direct_load_mgr_handle)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    } else if (OB_UNLIKELY(!is_full_direct_load_task)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("incremental direct load mgr should be newly created", K(ret), K(mgr_key), K(major_sstable_exist));
    } else if (OB_ISNULL(direct_load_mgr = direct_load_mgr_handle.get_obj())) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected err", K(ret), K(mgr_key));
    }
    if (OB_SUCC(ret) && (!major_sstable_exist || !is_full_direct_load_task)) {
...
int ObDirectLoadMgr::alloc_execution_context_id(
    int64_t &context_id)
{
  int ret = OB_SUCCESS;
...
int ObDirectLoadMgr::alloc_slice_id(int64_t &slice_id)
{
  slice_id = generate_slice_id();
  return OB_SUCCESS;
}

int ObDirectLoadMgr::open_tablet_direct_load(
    const ObDirectLoadType &type,
    const ObTabletID &tablet_id,
    const int64_t context_id)
{
...
int ObDirectLoadMgr::close_tablet_direct_load(
    const int64_t context_id,
    const ObDirectLoadType &type,
    const ObTabletID &tablet_id,
    ...
    const int64_t execution_id)
{
  ...
  } else if (OB_FAIL(close_tablet_direct_load_for_sn(
      context_id, type, tablet_id, need_commit, task_id, table_id, execution_id))) {
...
int ObDirectLoadMgr::close_tablet_direct_load_for_sn(
    const int64_t context_id,
    const ObDirectLoadType &type,
    const ObTabletID &tablet_id,
    const bool need_commit,
    ...
    const int64_t execution_id)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    // For full direct load, ObTabletDirectLoadMgr is removed from the server registry when,
...
int ObDirectLoadMgr::get_tablet_cache_interval(
    const int64_t context_id,
    const ObTabletID &tablet_id,
...
int ObDirectLoadMgr::check_and_process_finished_tablet(
    const ObTabletID &tablet_id,
    const int64_t task_id,
    const int64_t table_id,
    const int64_t execution_id)
...
int ObDirectLoadMgr::get_tablet_mgr_and_check_major(
    const ObTabletID &tablet_id,
...
int ObDirectLoadMgr::get_tablet_mgr(
...
int ObDirectLoadMgr::get_tablet_mgr_no_lock(
    const ObTabletDirectLoadMgrKey &mgr_key,
    ObTabletDirectLoadMgrHandle &direct_load_mgr_handle)
{
  int ret = OB_SUCCESS;
...
int ObDirectLoadMgr::get_tablet_exec_context_with_rlock(
...
int ObDirectLoadMgr::GetGcCandidateOp::operator() (common::hash::HashMapPair<ObTabletDirectLoadMgrKey, ObBaseTabletDirectLoadMgr *> &kv)
{
  int ret = OB_SUCCESS;
...
int ObDirectLoadMgr::gc_tablet_direct_load()
{
  int ret = OB_SUCCESS;
  if (!tablet_mgr_map_.empty() && ObDDLUtil::reach_time_interval(10 * 1000 * 1000, last_gc_time_)) {
    ...
    for (int64_t i = 0; i < candidate_mgrs.count(); i++) { // overwrite ret
      ...
      if (OB_ISNULL(ls_svr)) {
        ret = OB_ERR_SYS;
        LOG_WARN("invalid runtime ObLSService", K(ret));
      } else if (OB_FAIL(ls_svr->get_ls(ls))) {
        LOG_WARN("get local log stream failed", K(ret));
...
int ObDirectLoadMgr::remove_tablet_direct_load(const ObTabletDirectLoadMgrKey &mgr_key)
{
  ObBucketHashWLockGuard guard(bucket_lock_, mgr_key.hash());
  return remove_tablet_direct_load_nolock(mgr_key);
}

int ObDirectLoadMgr::remove_tablet_direct_load_nolock(const ObTabletDirectLoadMgrKey &mgr_key)
{
  int ret = OB_SUCCESS;
#ifdef ERRSIM
  if (OB_SUCC(ret)) {
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_FAIL(tablet_mgr_map_.get_refactored(mgr_key, tablet_direct_load_mgr))) {
      ...
      LOG_INFO("remove tablet direct load mgr from server registry", K(ret), K(mgr_key), K(common::lbt()), K(tablet_direct_load_mgr->get_ref()));
    ...
    const ObTabletDirectLoadInsertParam &build_param)
{
  ...
  if (OB_UNLIKELY(!build_param.is_valid())) {
    ...
    if (OB_FAIL(lob_param.assign(build_param))) {
      ...
      LOG_WARN("get runtime schema failed", K(ret), K(lob_param));
...
int ObTabletDirectLoadMgr::prepare_index_builder_if_need(const ObTableSchema &table_schema)
{
  ...
  if (sqc_build_ctx_.index_builder_ != nullptr) {
    LOG_INFO("index builder is already prepared");
          ...
          data_format_version_, get_micro_index_clustered(), 0/*concurrent_cnt*/,
...
// This interface is used by normal execution and start-log replay.
