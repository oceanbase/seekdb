...
ObDDLContinuousVector::ObDDLContinuousVector(ObContinuousBase *continuous_vector)
...
ObDDLContinuousVector::~ObDDLContinuousVector()
{
  if (nullptr != buf_) {
    ob_free_align(buf_);
    buf_ = nullptr;
...
void ObDDLContinuousVector::reuse(const int64_t batch_size)
{
...
int ObDDLContinuousVector::expand(const int64_t need_size)
{
  int ret = OB_SUCCESS;
...
template <>
inline int ObDDLContinuousVector::_append_batch(const int64_t batch_idx,
...
template <>
inline int ObDDLContinuousVector::_append_batch<false>(const int64_t batch_idx,
...
template <>
inline int ObDDLContinuousVector::_append_batch<true>(const int64_t batch_idx,
                                                             const ObDatum *datums,
                                                             const int64_t offset,
                                                             const int64_t size)
{
...
template <>
inline int ObDDLContinuousVector::_append_selective(const int64_t batch_idx,
                                                           ObContinuousBase *src_vec,
...
template <>
inline int ObDDLContinuousVector::_append_selective<false>(const int64_t batch_idx,
                                                                  const ObDatum *datums,
                                                                  const uint16_t *selector,
                                                                  const int64_t size)
{
...
template <>
inline int ObDDLContinuousVector::_append_selective<true>(const int64_t batch_idx,
                                                                 const ObDatum *datums,
                                                                 const uint16_t *selector,
                                                                 const int64_t size)
{
...
template <>
inline int ObDDLContinuousVector::_shallow_copy(ObContinuousBase *src_vec,
...
template <>
inline int ObDDLContinuousVector::_shallow_copy(ObDiscreteBase *src_vec,
                                                       const int64_t batch_size)
{
...
template <>
inline int ObDDLContinuousVector::_shallow_copy<false>(const ObDatum *datums,
                                                              const int64_t batch_size)
{
...
template <>
inline int ObDDLContinuousVector::_shallow_copy<true>(const ObDatum *datums,
                                                             const int64_t batch_size)
{
  return _append_batch<true>(0 /*batch_idx*/, datums, 0 /*offset*/, batch_size);
}

int ObDDLContinuousVector::append_datum(const int64_t batch_idx, const ObDatum &datum)
{
  int ret = OB_SUCCESS;
...
int ObDDLContinuousVector::append_batch(const int64_t batch_idx,
                                               const ObDDLVector &src, const int64_t offset,
                                               const int64_t size)
{
  return _append_batch(batch_idx,
                       static_cast<const ObDDLContinuousVector &>(src).continuous_vector_,
                       offset, size);
}

int ObDDLContinuousVector::append_batch(const int64_t batch_idx, ObIVector *src,
                                               const int64_t offset, const int64_t size)
{
  int ret = OB_SUCCESS;
...
int ObDDLContinuousVector::append_selective(const int64_t batch_idx,
                                                   const ObDDLVector &src,
                                                   const uint16_t *selector, const int64_t size)
{
  return _append_selective(
    batch_idx, static_cast<const ObDDLContinuousVector &>(src).continuous_vector_, selector,
    size);
}

int ObDDLContinuousVector::append_selective(const int64_t batch_idx, ObIVector *src,
                                                   const uint16_t *selector, const int64_t size)
{
  int ret = OB_SUCCESS;
  const VectorFormat format = src->get_format();
...
int ObDDLContinuousVector::shallow_copy(ObIVector *src, const int64_t batch_size)
{
  int ret = OB_SUCCESS;
  const VectorFormat format = src->get_format();
  switch (format) {
...
int ObDDLContinuousVector::shallow_copy(const ObDatumVector &datum_vec,
                                               const int64_t batch_size)
{
  if (datum_vec.is_batch()) {
