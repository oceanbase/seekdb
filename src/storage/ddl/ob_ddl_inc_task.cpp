...
int ObDDLIncStartTask::generate_next_task(ObITask *&next_task)
{
  ...
  if (next_tablet_idx >= dag->get_tablet_ids().count()) {
    ret = OB_ITER_END;
  } else {
...
int ObDDLIncStartTask::process()
{
  int ret = OB_SUCCESS;
  ObDDLIndependentDag *dag = static_cast<ObDDLIndependentDag *>(dag_);
  const ObTabletID &tablet_id = dag->get_tablet_ids().at(tablet_idx_);
  ...
  } else if (OB_FAIL(redo_writer.init(tablet_id))) {
    LOG_WARN("fail to init inc redo writer", KR(ret), K(tablet_id),
...
int ObDDLIncCommitTask::generate_next_task(ObITask *&next_task)
{
  ...
  if (tablet_idx_ == -1) {
    ret = OB_ITER_END;
  } else if (next_tablet_idx >= dag->get_tablet_ids().count()) {
    ret = OB_ITER_END;
  } else {
...
int ObDDLIncCommitTask::process()
{
  ...
  ObDDLIndependentDag *dag = static_cast<ObDDLIndependentDag *>(dag_);
  if (tablet_idx_ != -1) {
    tablet_id_ = dag->get_tablet_ids().at(tablet_idx_);
  }
  ObDDLTabletContext *tablet_ctx = nullptr;
  ObDDLIncRedoLogWriter redo_writer;
