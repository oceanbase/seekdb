...
int ObDAGFtsMacroBlockWriteOp::init(const ObTabletID &tablet_id, const int64_t slice_idx)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    if (OB_ISNULL(ddl_dag)) {
      ...
      if (FAILEDx(ObDDLStorageUtil::init_macro_block_seq(slice_idx, start_seq))) {
                                       ...
                                       ObChunk &output_chunk)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_ISNULL(ddl_dag)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ddl dag is null", K(ret), K(tablet_id_), K(slice_idx_));
    } else if (OB_FAIL(ddl_dag->get_tablet_context(tablet_id_, tablet_context))) {
      LOG_WARN("get tablet context failed", K(ret), K(tablet_id_), K(slice_idx_));
    } else if (OB_ISNULL(tablet_context)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("tablet context is null", K(ret), K(tablet_id_), K(slice_idx_));
    } else if (OB_FAIL(tablet_context->get_or_create_slice(slice_idx_, ddl_slice, is_new_slice))) {
      LOG_WARN("get ddl slice failed", K(ret), K(tablet_id_), K(slice_idx_));
    } else if (OB_FAIL(macro_block_writer_.close())) {
      LOG_WARN("failed to close writer", K(ret), K(tablet_id_), K(slice_idx_));
    } else if (OB_FAIL(ddl_slice->set_block_flushed(0 /* cg_idx */))) {
      LOG_WARN("set block flushed failed", K(ret), K(tablet_id_), K(slice_idx_));
    } else {
