...
ObDDLBatchRows::ObDDLBatchRows()
  : allocator_("DDL_BatchRows"),
    vectors_(),
    max_batch_size_(0),
    row_flag_(),
    size_(0),
...
ObDDLBatchRows::~ObDDLBatchRows() { reset(); }

void ObDDLBatchRows::reset()
{
  is_inited_ = false;
  for (int64_t i = 0; i < vectors_.count(); ++i) {
    ObDDLVector *vector = vectors_.at(i);
    if (nullptr != vector) {
      vector->~ObDDLVector();
    }
  }
...
int ObDDLBatchRows::init(const ObIArray<ObColDesc> &col_descs,
...
int ObDDLBatchRows::init(const ObIArray<ObColumnSchemaItem> &column_schemas,
...
int ObDDLBatchRows::init_vectors(const ObIArray<ObColDesc> &col_descs,
...
int ObDDLBatchRows::init_vectors(const common::ObIArray<ObColumnSchemaItem> &column_schemas, int64_t max_batch_size)
{
  int ret = OB_SUCCESS;
...
int ObDDLBatchRows::append_row(const ObDatumRow &datum_row)
{
  return append_row(datum_row.storage_datums_, datum_row.count_);
}

int ObDDLBatchRows::append_row(const ObStorageDatum *datums, const int64_t column_count)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDDLBatchRows::append_row(const ObIArray<ObDatum *> &datums)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDDLBatchRows::append_batch(const ObDDLBatchRows &vectors, const int64_t offset,
                                        const int64_t size)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
...
int ObDDLBatchRows::append_batch(const IVectorPtrs &vectors, const int64_t offset,
                                        const int64_t size)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
...
int ObDDLBatchRows::append_selective(const ObDDLBatchRows &src,
...
int ObDDLBatchRows::append_selective(const IVectorPtrs &vectors, share::ObBatchSelector &selector)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObDDLBatchRows::append_selective(const ObIArray<ObDatumVector> &datum_vectors,
...
int64_t ObDDLBatchRows::memory_usage() const
{
  int64_t size = 0;
...
int64_t ObDDLBatchRows::bytes_usage() const
{
  int64_t size = 0;
  for (int64_t i = 0; i < vectors_.count(); ++i) {
    ObDDLVector *vector = vectors_.at(i);
