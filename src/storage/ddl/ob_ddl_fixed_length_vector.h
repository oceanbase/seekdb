...
{
template <typename T>
class ObDDLFixedLengthBase : public ObDDLVector
{
public:
  ObDDLFixedLengthBase(ObFixedLengthBase *fixed_vector)
    : fixed_vector_(fixed_vector),
      vec_data_(reinterpret_cast<T *>(fixed_vector->get_data())),
      data_(vec_data_)
  {
  }
  ~ObDDLFixedLengthBase() override = default;
                   ...
                   const int64_t size) override
  {
    return _append_batch(
      batch_idx, static_cast<const ObDDLFixedLengthBase &>(src).fixed_vector_, offset, size);
  }
                       ...
                       const uint16_t *selector, const int64_t size) override
  {
    return _append_selective(batch_idx,
                             static_cast<const ObDDLFixedLengthBase &>(src).fixed_vector_,
                             selector, size);
  }
...
template <typename T>
class ObDDLFixedLengthVector final : public ObDDLFixedLengthBase<T>
{
  using SuperClass = ObDDLFixedLengthBase<T>;

public:
  ObDDLFixedLengthVector(ObFixedLengthBase *fixed_vector) : SuperClass(fixed_vector) {}
  ~ObDDLFixedLengthVector() override = default;
};
