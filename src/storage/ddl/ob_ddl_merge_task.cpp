...
int ObDDLTableMergeDag::check_allow_major_merge()
{
  ...
  } else if (OB_FAIL(ObDDLDirectLoadUtil::get_tablet_handle(ddl_param_.tablet_id_, tablet_handle))) {
...
{
  int ret = OB_SUCCESS;
  LOG_INFO("ddl merge task start process", K(*this), "ddl_event_info", ObDDLEventInfo(GCTX.self_addr()));
  ObLSService *ls_service = ::oceanbase::share::server_service<::oceanbase::storage::ObLSService>();
  ObLS *ls = nullptr;
  ObTabletHandle tablet_handle;
  if (OB_UNLIKELY(!is_inited_)) {
...
int prepare_full_direct_load_ddl_param(const ObDDLTableMergeDagParam &merge_param, ObTabletDDLParam &ddl_param)
{
  ...
  } else if (OB_FAIL(prepare_ddl_param_for_idem_sn(merge_param, ddl_param))) {
...
int ObDDLTableMergeTask::merge_full_direct_load_ddl_kvs_for_sn(ObLS *ls, ObTablet &tablet)
{
  ...
  if (OB_FAIL(tablet.get_ddl_kv_mgr(ddl_kv_mgr_handle))) {
    ...
    if (OB_FAIL(ret)) {
      ...
      const bool is_data_complete = merge_param_.is_commit_;
      if (!is_data_complete) {
    ...
    if (OB_SUCC(ret) && merge_param_.is_commit_ && is_major_exist) {
      ...
      LOG_INFO("commit ddl sstable finished", K(ret), K(ddl_param), K(merge_param_), "ddl_event_info", ObDDLEventInfo(GCTX.self_addr()));
    }
  }
  return ret;
}
