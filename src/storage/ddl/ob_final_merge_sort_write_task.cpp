...
int ObDDLFinalMergeSortOp::try_create_sort_impl()
{
  ...
  const ObDDLTableSchema *sort_schema = nullptr;
  param.set_mem_attr(ObMemAttr("DDLMergeSort", ObCtxIds::WORK_AREA))
  ...
  if (OB_FAIL(ret)) {
    ...
    const int64_t tempstore_read_alignment_size = 2L << 20;
    ...
    if (OB_FAIL(sort_impl_->init(compare_,
                                 ...
                                 &addon_row_meta_,
                                 max_batch_size,
                                 ObMemAttr("DDLMergeSort", ObCtxIds::WORK_AREA),
                                 compressor_type,
                                 false /*enable_trunc*/,
...
ObDDLFinalMergeSortWriteTask::ObDDLFinalMergeSortWriteTask()
  ...
  allocator_(ObMemAttr("DDLMergeSort"))
{
}
...
int create_sort_chunk_array(ObChunk *&chunk)
{
  ...
  if (nullptr != chunk) {
    ...
    ObChunk *new_chunk = OB_NEW(ObChunk, ObMemAttr("DDLChunk"));
    ObArray<ObDDLSortChunk> *sql_sort_chunk_array = OB_NEW(ObArray<ObDDLSortChunk>, ObMemAttr("DDLSortChunkArr"));
