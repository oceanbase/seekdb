...
ObDDLDiscreteVector::ObDDLDiscreteVector(ObDiscreteBase *discrete_vector)
  : discrete_vector_(discrete_vector),
    vec_lens_(discrete_vector->get_lens()),
    vec_ptrs_(discrete_vector->get_ptrs()),
    lens_(vec_lens_),
    ptrs_(vec_ptrs_),
    allocator_("DDL_Discrete")
{

}

void ObDDLDiscreteVector::reuse(const int64_t batch_size)
{
...
template <>
int ObDDLDiscreteVector::_append_batch(const int64_t batch_idx, ObContinuousBase *src_vec,
...
template <>
int ObDDLDiscreteVector::_append_batch(const int64_t batch_idx, ObDiscreteBase *src_vec,
                                              const int64_t offset, const int64_t size)
{
  int ret = OB_SUCCESS;
...
template <>
int ObDDLDiscreteVector::_append_batch<false>(const int64_t batch_idx, const ObDatum *datums,
                                                     const int64_t offset, const int64_t size)
{
  int ret = OB_SUCCESS;
  // Calculate total_size
...
template <>
int ObDDLDiscreteVector::_append_batch<true>(const int64_t batch_idx, const ObDatum *datums,
                                                    const int64_t offset, const int64_t size)
{
  UNUSED(offset);
  int ret = OB_SUCCESS;
...
template <>
inline int ObDDLDiscreteVector::_append_selective(const int64_t batch_idx,
...
template <>
inline int ObDDLDiscreteVector::_append_selective<false>(const int64_t batch_idx,
...
template <>
inline int ObDDLDiscreteVector::_append_selective<true>(const int64_t batch_idx,
                                                               const ObDatum *datums,
                                                               const uint16_t *selector,
                                                               const int64_t size)
{
...
template <>
inline int ObDDLDiscreteVector::_shallow_copy(ObContinuousBase *vec,
...
template <>
inline int ObDDLDiscreteVector::_shallow_copy(ObDiscreteBase *vec, const int64_t batch_size)
{
...
template <>
inline int ObDDLDiscreteVector::_shallow_copy<false>(const ObDatum *datums,
                                                            const int64_t batch_size)
{
  for (int64_t i = 0; i < batch_size; ++i) {
...
template <>
inline int ObDDLDiscreteVector::_shallow_copy<true>(const ObDatum *datums,
                                                           const int64_t batch_size)
{
  const ObDatum &datum = datums[0];
  for (int64_t i = 0; i < batch_size; ++i) {
...
int ObDDLDiscreteVector::append_datum(const int64_t batch_idx, const ObDatum &datum)
{
  int ret = OB_SUCCESS;
  char *buf = nullptr;
  if (OB_ISNULL(buf = alloc_buf(datum.len_))) {
...
int ObDDLDiscreteVector::append_batch(const int64_t batch_idx, const ObDDLVector &src,
                                             const int64_t offset, const int64_t size)
{
  return _append_batch(
    batch_idx, static_cast<const ObDDLDiscreteVector &>(src).discrete_vector_, offset, size);
}

int ObDDLDiscreteVector::append_batch(const int64_t batch_idx, ObIVector *src,
                                             const int64_t offset, const int64_t size)
{
  int ret = OB_SUCCESS;
...
int ObDDLDiscreteVector::append_batch(const int64_t batch_idx,
...
int ObDDLDiscreteVector::append_selective(const int64_t batch_idx,
                                                 const ObDDLVector &src,
                                                 const uint16_t *selector, const int64_t size)
{
  return _append_selective(batch_idx,
                           static_cast<const ObDDLDiscreteVector &>(src).discrete_vector_,
                           selector, size);
}

int ObDDLDiscreteVector::append_selective(const int64_t batch_idx, ObIVector *src,
                                                 const uint16_t *selector, const int64_t size)
{
  int ret = OB_SUCCESS;
  const VectorFormat format = src->get_format();
...
int ObDDLDiscreteVector::shallow_copy(ObIVector *src, const int64_t batch_size)
{
  int ret = OB_SUCCESS;
  const VectorFormat format = src->get_format();
  switch (format) {
...
int ObDDLDiscreteVector::shallow_copy(const ObDatumVector &datum_vec,
                                             const int64_t batch_size)
{
  if (datum_vec.is_batch()) {
