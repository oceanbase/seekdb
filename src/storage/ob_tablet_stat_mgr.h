...
#ifndef OCEANBASE_STORAGE_TABLET_STAT_MGR_H_
#define OCEANBASE_STORAGE_TABLET_STAT_MGR_H_
...
public:
  ...
  TO_STRING_KV(K_(tablet_stat), K_(total_tablet_stat), K_(is_low_resource), K_(boost_factor));
public:
...
public:
  ...
  bool is_low_resource_;
};


struct ObRuntimeSysStat
{
public:
  ObRuntimeSysStat();
  ~ObRuntimeSysStat() = default;
  void reset();
...
{
public:
  static int server_module_init(ObTabletStatMgr* &tablet_stat_mgr);
  ObTabletStatMgr();
  virtual ~ObTabletStatMgr();
  int init();
...
private:
  ...
  {
  public:
    TabletStatUpdater(ObTabletStatMgr &mgr) : last_update_time_(0), mgr_(mgr) {}
    virtual ~TabletStatUpdater() {}
    virtual void runTimerTask();
  public:
    int64_t last_update_time_; // for mittest
  private:
    ObTabletStatMgr &mgr_;
  };
...
#endif /* OCEANBASE_STORAGE_TABLET_STAT_MGR_H_ */
