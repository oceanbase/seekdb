...
#ifndef OCEABASE_STORAGE_TABLET_LOCAL_STATUS_
#define OCEABASE_STORAGE_TABLET_LOCAL_STATUS_
...
public:
  enum STATUS : uint8_t
  {
    ...
    UNDEFINED = 4,              // invalid status sentinel
    REMOTE = 5,
    RESTORE_STATUS_MAX
  };
public:
...
class ObTabletLocalStatus final
{
public:
  ObTabletLocalStatus();
  ~ObTabletLocalStatus() = default;
  bool is_valid() const;
  int serialize(char *buf, const int64_t len, int64_t &pos) const;
  int deserialize(const char *buf, const int64_t len, int64_t &pos);
  int64_t get_serialize_size() const;
  void reset();
  int64_t get_local_status() const { return local_status_; }
  void set_local_status(const int64_t local_status) { local_status_ = local_status; }
