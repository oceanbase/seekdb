...
public:
  ...
  int deserialize(const char*,
                  const int64_t,
                  int64_t&,
                  ObIAllocator &allocator = share::g_mp->mds_service()->get_buffer_ctx_allocator());// To be determined based on the actual ctx type at compile time through subclass reflection
