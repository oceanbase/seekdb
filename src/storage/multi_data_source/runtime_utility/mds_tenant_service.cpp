...
void ObTenantMdsService::run_recyle_timer_task()
{
...
void ObTenantMdsService::run_dump_status_timer_task()
{
  ObCurTraceId::init(GCONF.self_addr_);
...
int ObTenantMdsService::for_each_ls_in_tenant(const ObFunction<int(ObLS &)> &op)
{
  ...
  } else if (MDS_FAIL(share::g_mp->ls_service()->get_ls(share::SYS_LS, handle, ObLSGetMod::MDS_TABLE_MOD))) {
    if (OB_LS_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
    } else {
      MDS_LOG_NONE(WARN, "fail to get sys ls");
    }
...
int ObTenantMdsService::try_recycle_mds_table_(ObTablet &tablet,
...
int ObTenantMdsService::try_gc_mds_table_(ObTablet &tablet)
{
