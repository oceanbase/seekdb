...
public:
  ...
  ~ObMdsService() {
    ...
    MDS_LOG_RET(INFO, OB_SUCCESS, "ObMdsAllocator destructed");
  }
  static int server_module_init(ObMdsService *&mds_service);
  static int server_module_start(ObMdsService *&mds_service);
  static void server_module_stop(ObMdsService *&mds_service);
  static void server_module_wait(ObMdsService *&mds_service);
  void destroy() { this->~ObMdsService(); }
  static int for_each_ls(const ObFunction<int(ObLS &)> &op);
  ...
  share::ObBufferCtxAllocator &get_buffer_ctx_allocator() { return buffer_ctx_allocator_; }
...
private:
  ...
  public:
    ...
    void runTimerTask() override { ObMdsService::run_dump_status_timer_task(); }
  };

private:
  bool is_inited_;
  share::ObBufferCtxAllocator buffer_ctx_allocator_;
