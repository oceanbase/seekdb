...
int ObTabletSplitMdsUserData::calc_split_dst(ObLS &ls, const ObDatumRowkey &rowkey, const int64_t abs_timeout_us, ObTabletID &dst_tablet_id) const
{
  ...
  if (RANGE_PART_SPLIT_SRC == status_) {
    ...
    if (OB_FAIL(ret)) {
      ...
      LOG_WARN("rowkey not in any split dst tablet", K(ret), K(rowkey), K(*this));
...
int ObTabletSplitMdsUserData::get_split_src_tablet(ObLS &ls, ObTabletHandle &src_tablet_handle) const
{
  ...
  if (OB_FAIL(get_split_src_tablet_id(src_tablet_id))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid tablet", K(ret), K(src_tablet_id));
  } else if (OB_UNLIKELY(src_tablet_handle.get_obj()->is_empty_shell())) {
    ret = OB_TABLET_NOT_EXIST;
    LOG_WARN("split src tablet becomes empty shell", K(ret), K(src_tablet_id));
  }
  return ret;
}
    ...
    ObTabletSplitTscInfo &split_info)
{
  ...
  if (RANGE_PART_SPLIT_DST == status_) {
    ...
    if (OB_FAIL(get_split_src_tablet(ls, src_tablet_handle))) {
      LOG_WARN("failed to get src tablet handle", K(ret));
    } else if (OB_FAIL(ObTabletSplitMdsHelper::get_valid_timeout(abs_timeout_us, timeout_us))) {
      LOG_WARN("failed to get valid timeout", K(ret), K(abs_timeout_us));
    } else if (OB_FAIL(src_tablet_handle.get_obj()->ObITabletMdsInterface::get_split_data(src_data, timeout_us))) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < src_data.ref_tablet_ids_.count(); i++) {
        if (tablet_id == src_data.ref_tablet_ids_.at(i)) {
          if (i == 0) {
            ...
            if (OB_FAIL(ls.get_tablet(prev_dst_tablet_id, prev_dst_tablet_handle, ObTabletCommon::DEFAULT_GET_TABLET_DURATION_US,
                    ObMDSGetTabletMode::READ_WITHOUT_CHECK))) {
              LOG_WARN("failed to get tablet", K(ret));
            } else if (OB_ISNULL(prev_dst_tablet_handle.get_obj())) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("invalid tablet", K(ret));
            } else if (OB_UNLIKELY(prev_dst_tablet_handle.get_obj()->is_empty_shell())) {
              ret = OB_TABLET_NOT_EXIST;
              LOG_WARN("split dst tablet becomes empty shell", K(ret), K(prev_dst_tablet_id));
            } else if (OB_FAIL(ObTabletSplitMdsHelper::get_valid_timeout(abs_timeout_us, timeout_us))) {
              LOG_WARN("failed to get valid timeout", K(ret), K(abs_timeout_us));
  ...
  if (OB_FAIL(ret)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid tsc split info", K(ret), K(tablet_id), K(split_info));
  }
  return ret;
}
