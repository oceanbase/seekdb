                             ...
                             ObTenantMetaMemMgr *t3m)
{
  ...
  t3m_ = t3m;
  is_inited_ = true;
  LOG_INFO("tx ctx memtable mgr init successfully", K(tablet_id), K(this));
...
int ObTxCtxMemtableMgr::create_memtable(const CreateMemtableArg &arg)
{
  ...
  if (get_memtable_count_() > 0) {
    ...
    if (NULL == tx_ctx_memtable) {
    ...
    } else if (OB_FAIL(tx_ctx_memtable->init(table_key))) {
      ...
      LOG_WARN("tx ctx memtable register_common_checkpoint failed", K(ret));
    } else {
...
int ObTxCtxMemtableMgr::unregister_from_common_checkpoint_(const ObTxCtxMemtable *memtable)
{
  ...
  if (OB_ISNULL(ls_tx_svr = freezer_->get_ls_tx_svr())) {
    ...
    LOG_WARN("ls_tx_svr is null", K(ret));
                                                             ...
                                                             memtable))) {
    LOG_WARN("tx ctx unregister_common_checkpoint failed", K(ret), K(memtable));
  } else {
    LOG_INFO("unregister from common checkpoint successfully", K(memtable));
  }
  return ret;
}
  ...
  if (get_memtable_count_() > 0 && force) {
    ...
    if (nullptr != tables_[idx] && memtable == tables_[idx]) {
      ...
      if (OB_TMP_FAIL(unregister_from_common_checkpoint_(memtable))) {
        LOG_WARN("unregister from common checkpoint failed", K(tmp_ret), K(memtable));
      }
      release_head_memtable();
      FLOG_INFO("succeed to release tx ctx memtable", K(ret));
    }
  } else if (!force) {
    // just for flush
