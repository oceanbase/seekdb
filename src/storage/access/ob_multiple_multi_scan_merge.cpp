...
int ObMultipleMultiScanMerge::calc_scan_range()
{
  ...
  if (OB_FAIL(inner_calc_scan_range(ranges_, cow_ranges_, curr_scan_index_, curr_rowkey_))) {
      ...
      for (int64_t i = l; i < r && OB_SUCC(ret); ++i) {
        ...
        if (curr_scan_index == i) {
          ...
          if (OB_FAIL(range_key.compare(curr_rowkey, read_info->get_datum_utils(), cmp_ret))) {
            ...
            range.change_boundary(curr_rowkey, is_reverse_scan);
...
int ObMultipleMultiScanMerge::is_range_valid() const
{
  ...
  if (OB_ISNULL(ranges_)) {
    ret = OB_ERR_UNEXPECTED;
    STORAGE_LOG(WARN, "ranges is null", K(ret), KP(ranges_));
...
int ObMultipleMultiScanMerge::construct_iters()
{
  ...
  if (OB_ISNULL(ranges_)) {
    ...
    STORAGE_LOG(TRACE, "construct iters begin", K(tables_.count()), K(iters_.count()),
                KPC_(ranges), K_(access_ctx_->trans_version_range), K_(tables), KPC_(access_param));
    ObITable *table = NULL;
