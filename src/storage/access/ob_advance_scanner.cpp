...
namespace storage
{
ObAdvanceScanner::ObAdvanceScanner(const ObStorageDatumUtils &datum_utils)
...
ObAdvanceScanner::~ObAdvanceScanner()
{
...
int ObAdvanceScanner::switch_info(
    const bool is_reverse_scan,
    const ObDatumRange &scan_range,
    const ObITableReadInfo &read_info,
    common::ObIAllocator &stmt_allocator)
...
int ObAdvanceScanner::advance_scan(const ObDatumRange &scan_range)
{
  int ret = OB_SUCCESS;
  int cmp_ret = 0;
  if (IS_NOT_INIT) {
...
int ObAdvanceScanner::skip(ObMicroIndexInfo &index_info, ObAdvanceScanState &prev_state, ObAdvanceScanState &state)
{
  int ret = OB_SUCCESS;
  ...
  if (left_border_reached_) {
    ...
    if (OB_UNLIKELY(!complete_range_.is_valid())) {
      ...
      LOG_TRACE("[ADVANCE SCAN] can skip", K(prev_state), K(state), K(endkey), K_(complete_range), KPC(this));
    }
  ...
  if (OB_SUCC(ret) && !state_determined) {
    state.set_state(0, ObAdvanceScanNodeState::PREFIX_UNCERTAIN);
  }
  prev_state = state;
  return ret;
}

int ObAdvanceScanner::skip(
    ...
    const bool first)
{
  ...
  ObAdvanceScanState &state = index_info.advance_state_;
  LOG_DEBUG("[ADVANCE SCAN] skip in micro", K(first), K(state), K(micro_scanner), KPC(this));
  if (state.is_skipped()) {
  ...
  if (OB_SUCC(ret) && !left_border_reached_ && !state.is_skipped()) {
    ...
    LOG_DEBUG("[ADVANCE SCAN] micro scanner skip to range", KR(ret), K(has_data), K(range_covered), K(state), KPC(this));
  }
  return ret;
}

int ObAdvanceScanFactory::build_advance_scanner(
    ...
    ObAdvanceScanner *&advance_scanner)
{
  ...
  if (OB_FAIL(ret) && nullptr != advance_scanner) {
    ...
    stmt_allocator.free(advance_scanner);
    advance_scanner = nullptr;
  }
  return ret;
}

void ObAdvanceScanFactory::destroy_advance_scanner(ObAdvanceScanner *&advance_scanner)
{
  if (nullptr != advance_scanner) {
    ObIAllocator *stmt_allocator = advance_scanner->get_stmt_alloc();
    advance_scanner->~ObAdvanceScanner();
