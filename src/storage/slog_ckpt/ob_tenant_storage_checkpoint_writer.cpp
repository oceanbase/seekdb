    ...
    ObTenantCheckpointSlogHandler *ckpt_slog_handler)
{
  ...
  } else if ((OB_UNLIKELY(ObTenantStorageMetaType::CKPT == meta_type && nullptr == ckpt_slog_handler))) {
    ret = OB_INVALID_ARGUMENT;
...
int ObTenantStorageCheckpointWriter::record_ls_meta(MacroBlockId &ls_entry_block)
{
  ...
  if (OB_FAIL(ls_item_writer_.init(false /*whether need addr*/, mem_attr))) {
    ...
    auto record_ls = [&](ObLS &ls) -> int {
      if (OB_FAIL(do_record_ls_meta(ls, unused_scn))) {
        LOG_WARN("fail to do record storage meta", K(ret), K(ls));
      }
      return ret;
    };
    if (OB_FAIL(share::g_mp->ls_service()->foreach_ls(record_ls, ObLSGetMod::STORAGE_MOD))) {
      LOG_WARN("fail to foreach log stream", K(ret));
    }
