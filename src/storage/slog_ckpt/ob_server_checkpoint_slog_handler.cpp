...
int ObServerCheckpointSlogHandler::start_replay()
{
  ...
  if (IS_NOT_INIT) {
    ...
    tenant_meta_for_replay_ = omt::ObTenantMeta();
    tenant_meta_valid_for_replay_ = false;
...
void ObServerCheckpointSlogHandler::get_replay_result(
    omt::ObTenantMeta &tenant_meta, bool &is_valid) const
{
  is_valid = tenant_meta_valid_for_replay_;
  if (tenant_meta_valid_for_replay_) {
    tenant_meta = tenant_meta_for_replay_;
  }
}

// Single sys slot get, preserve map's OB_HASH_NOT_EXIST
int ObServerCheckpointSlogHandler::get_replay_tenant_meta_(omt::ObTenantMeta &meta) const
{
  int ret = OB_SUCCESS;
  if (!tenant_meta_valid_for_replay_) {
    ret = OB_HASH_NOT_EXIST;
  } else {
    meta = tenant_meta_for_replay_;
  }
  return ret;
}

// Single sys slot set (cover), valid becomes true
int ObServerCheckpointSlogHandler::set_replay_tenant_meta_(const omt::ObTenantMeta &meta)
{
  tenant_meta_for_replay_ = meta;
  tenant_meta_valid_for_replay_ = true;
  return OB_SUCCESS;
}
...
int ObServerCheckpointSlogHandler::replay_delete_tenant_prepare(const char *buf, const int64_t buf_len)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_HASH_NOT_EXIST == ret) {
      ...
      LOG_WARN("failed to get tenant meta", K(ret));
    }
