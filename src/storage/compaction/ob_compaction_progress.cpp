  ...
  if (OB_FAIL(share::g_mp->ls_service()->get_ls(ls))) {
    ...
    while (OB_SUCC(ret)) {
      if (OB_FAIL(tablet_iter.get_next_tablet(tablet_handle))) {
        ...
        ++cnt;
        size += tablet_handle.get_obj()->get_last_major_total_macro_block_count() * DEFAULT_MACRO_BLOCK_SIZE;
      }
    } // end loop tablet
  }
...
int ObCompactionProgressIterator::open()
{
  ...
  } else if (OB_FAIL(share::g_mp->compaction_progress_mgr()->get_list(progress_array_))) {
...
int ObTabletCompactionProgressIterator::open()
{
  ...
  if (is_opened_) {
    ...
    ret = OB_NOT_INIT;
    STORAGE_LOG(WARN, "server modules are not ready", K(ret));
  } else if (OB_FAIL(share::g_mp->dag_scheduler()->get_all_compaction_dag_info(allocator_, progress_array_))) {
    LOG_WARN("failed to get compaction info", K(ret));
  } else {
    cur_idx_ = 0;
    is_opened_ = true;
