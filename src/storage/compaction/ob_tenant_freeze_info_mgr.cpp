...
int ObTenantFreezeInfoMgr::try_update_reserved_snapshot()
{
  ...
  if (OB_FAIL(ret) || reserved_snapshot <= 0) {
    ...
    int tmp_ret = OB_SUCCESS;
    auto sync_reserved_snapshot = [&](ObLS &ls) -> int {
      if (OB_TMP_FAIL(ls.try_sync_reserved_snapshot(reserved_snapshot, true/*update_flag*/))) {
        LOG_WARN("failed to update min reserved snapshot", K(tmp_ret), K(ls), K(reserved_snapshot));
      }
      return OB_SUCCESS;
    };
    if (OB_FAIL(share::g_mp->ls_service()->foreach_ls(sync_reserved_snapshot, ObLSGetMod::COMPACT_MODE))) {
