...
/*
 * ObMediumChecker implement
 * */
int ObMediumChecker::server_module_init(ObMediumChecker *&tablet_medium_checker)
{
  return tablet_medium_checker->init();
}

ObMediumChecker::ObMediumChecker()
  : is_inited_(false),
    tablet_check_set_(),
    lock_()
{}
...
int ObMediumChecker::add_tablet(const ObTabletID &tablet_id, const int64_t medium_scn)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObMediumChecker::check_medium_finish_schedule()
{
  int ret = OB_SUCCESS;
...
int ObMediumChecker::check_medium_finish(
...
int ObMediumChecker::reput_check_info(ObIArray<ObTabletCheckInfo> &tablet_check_infos)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
