...
ObPartitionRowMergeIter::ObPartitionRowMergeIter(common::ObIAllocator &allocator)
...
int ObPartitionMacroMergeIter::inner_init(const ObMergeParameter &merge_param)
{
  ...
  const storage::ObITableReadInfo *index_read_info = merge_param.static_param_.rowkey_read_info_;
  if (OB_FAIL(ret)) {
    ...
    const blocksstable::ObMicroBlockData *&micro_block_data) const
{
  ...
  if (OB_UNLIKELY(is_small_sstable_iter())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("small sstable macro block cannot be reused as a whole", K(ret));
...
int ObPartitionMicroMergeIter::open_curr_range(const bool for_rewrite, const bool for_compare)
{
  ...
  if (OB_UNLIKELY(micro_block_opened_)) {
    ...
    if (is_small_sstable_iter()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("small sstable without reusable micro block must be rewritten",
               K(ret), KPC(this));
    } else if (macro_block_opened_) {
      ret = OB_INNER_STAT_ERROR;
...
int ObPartitionMicroMergeIter::get_curr_range(ObDatumRange &range) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (is_small_sstable_iter() && !need_reuse_micro_block_) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected small sstable iterator state", K(ret), KPC(this));
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("small sstable macro range should already be open", K(ret), KPC(this));
  } else {
    range = curr_block_desc_.range_;
  }
  if (OB_SUCC(ret)) {
...
int ObPartitionMinorMacroMergeIter::check_macro_block_recycle(const ObMacroBlockDesc &macro_desc, bool &can_recycle)
{
  ...
  // TODO: enable recycle after the minor-merge reuse path handles every table state.
  can_recycle = false;
  return ret;
}
