...
{
ObTabletSchedulerTaskMgr::ObTabletSchedulerTaskMgr()
  : merge_loop_timer_(),
    medium_loop_timer_(),
    sstable_gc_timer_(),
...
void ObTabletSchedulerTaskMgr::destroy()
{
...
void ObTabletSchedulerTaskMgr::stop()
{
...
void ObTabletSchedulerTaskMgr::wait()
{
...
void ObTabletSchedulerTaskMgr::MergeLoopTask::runTimerTask()
{
  int ret = OB_SUCCESS;
...
void ObTabletSchedulerTaskMgr::MediumLoopTask::runTimerTask()
{
  int ret = OB_SUCCESS;
...
void ObTabletSchedulerTaskMgr::SSTableGCTask::runTimerTask()
{
  int ret = OB_SUCCESS;
  if (ObBasicMergeScheduler::could_start_loop_task()) {
...
void ObTabletSchedulerTaskMgr::InfoPoolResizeTask::runTimerTask()
{
  int ret = OB_SUCCESS;
  int64_t cost_ts = ObTimeUtility::fast_current_time();
  ObCurTraceId::init(GCONF.self_addr_);
...
void ObTabletSchedulerTaskMgr::TabletUpdaterRefreshTask::runTimerTask()
{
  int ret = OB_SUCCESS;
  int64_t cost_ts = ObTimeUtility::fast_current_time();
  ObCurTraceId::init(GCONF.self_addr_);
...
void ObTabletSchedulerTaskMgr::MediumCheckTask::runTimerTask()
{
  int ret = OB_SUCCESS;
  int64_t cost_ts = ObTimeUtility::fast_current_time();
  ObCurTraceId::init(GCONF.self_addr_);
...
int ObTabletSchedulerTaskMgr::start()
{
  int ret = OB_SUCCESS;
...
int ObTabletSchedulerTaskMgr::restart_scheduler_timer_task(
...
int ObTabletSchedulerTaskMgr::set_active_medium_loop(
  const bool active,
