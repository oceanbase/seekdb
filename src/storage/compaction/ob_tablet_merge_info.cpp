    ...
    ObTableHandleV2 &merge_table_handle)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    SMART_VARS_2((ObSSTableMergeRes, res), (ObTabletCreateSSTableParam, param)) {
      ...
      } else if (OB_FAIL(build_create_sstable_param(ctx, res, param))) {
        LOG_WARN("fail to build create sstable param", K(ret));
      } else if (OB_FAIL(ObTabletCreateDeleteHelper::create_sstable(
          param, ctx.mem_ctx_.get_allocator(), merge_table_handle))) {
        LOG_ERROR("fail to create sstable", K(ret), K(param));
        CTX_SET_DIAGNOSE_LOCATION(ctx);
      }

      if (OB_SUCC(ret)) {
        LOG_TRACE("succeed to merge sstable", K(param), KPC(this));
      }
    }
  }
  return ret;
