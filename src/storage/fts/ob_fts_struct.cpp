/*
 * Copyright (c) 2026 OceanBase.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#define USING_LOG_PREFIX STORAGE_FTS

#include "storage/fts/ob_fts_struct.h"

#include "share/datum/ob_datum_funcs.h"
#include "storage/ob_storage_util.h"

namespace oceanbase
{
namespace storage
{

int ObFTToken::init(const char *ptr,
                    const int64_t length,
                    const ObObjMeta &meta,
                    const common::ObDatumHashFuncType hash_func,
                    const ObDatumCmpFuncType cmp_func)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(nullptr == ptr || length <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid token buffer", K(ret), KP(ptr), K(length), K(meta));
  } else {
    token_.set_string(ptr, length);
    meta_ = meta;
    hash_func_ = hash_func;
    cmp_func_ = cmp_func;
    is_calc_hash_val_ = false;
    hash_val_ = 0;
  }
  return ret;
}

int ObFTToken::hash(uint64_t &hash_val) const
{
  int ret = OB_SUCCESS;
  if (OB_LIKELY(is_calc_hash_val_)) {
    hash_val = hash_val_;
  } else {
    common::ObDatumHashFuncType hash_func = hash_func_;
    if (OB_ISNULL(hash_func)) {
      sql::ObExprBasicFuncs *funcs =
          ObDatumFuncs::get_basic_func(meta_.get_type(), meta_.get_collation_type());
      if (OB_ISNULL(funcs) || OB_ISNULL(funcs->default_hash_)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("failed to get token hash function", K(ret), K(meta_), KP(funcs));
      } else {
        hash_func = funcs->default_hash_;
      }
    }
    // 首次计算后把结果留在 token 内，同一对象随后经过停止词表和聚合表时直接复用。
    if (OB_SUCC(ret) && OB_FAIL(hash_func(token_, 0, hash_val_))) {
      LOG_WARN("failed to calculate token hash", K(ret), K(token_));
    } else if (OB_SUCC(ret)) {
      is_calc_hash_val_ = true;
      hash_val = hash_val_;
    }
  }
  return ret;
}

bool ObFTToken::operator==(const ObFTToken &other) const
{
  int ret = OB_SUCCESS;
  bool is_equal = false;
  if (OB_FAIL(do_compare(other, is_equal))) {
    LOG_ERROR("failed to compare fulltext token", K(ret), K(*this), K(other));
    ob_abort();
  }
  return is_equal;
}

int ObFTToken::do_compare(const ObFTToken &other, bool &is_equal) const
{
  int ret = OB_SUCCESS;
  int cmp_ret = 0;
  is_equal = false;
  if (is_calc_hash_val_ && other.is_calc_hash_val_ && hash_val_ != other.hash_val_) {
    // hash 不同可立即短路，避免进入 collation 比较函数。
  } else {
    ObDatumCmpFuncType cmp_func = nullptr == cmp_func_
                                      ? get_datum_cmp_func(meta_, other.meta_)
                                      : cmp_func_;
    if (OB_ISNULL(cmp_func)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("token compare function is null", K(ret), K(meta_), K(other.meta_));
    } else if (OB_FAIL(cmp_func(token_, other.token_, cmp_ret))) {
      LOG_WARN("failed to compare token datum", K(ret), K(token_), K(other.token_));
    } else {
      is_equal = (0 == cmp_ret);
    }
  }
  return ret;
}

ObFTTokenInfo::ObFTTokenInfo(const ObFTTokenInfo &other)
    : allocator_(other.allocator_),
      count_(other.count_),
      position_list_(other.position_list_),
      position_list_holder_(other.position_list_holder_)
{
  retain_position_list_();
}

ObFTTokenInfo &ObFTTokenInfo::operator=(const ObFTTokenInfo &other)
{
  if (this != &other) {
    release_position_list_();
    allocator_ = other.allocator_;
    count_ = other.count_;
    position_list_ = other.position_list_;
    position_list_holder_ = other.position_list_holder_;
    retain_position_list_();
  }
  return *this;
}

ObFTTokenInfo::~ObFTTokenInfo()
{
  release_position_list_();
}

void ObFTTokenInfo::retain_position_list_()
{
  if (nullptr != position_list_holder_) {
    ++position_list_holder_->ref_cnt_;
  }
}

void ObFTTokenInfo::release_position_list_()
{
  if (nullptr != position_list_holder_ && 0 == --position_list_holder_->ref_cnt_) {
    position_list_holder_->~ObFTPositionListHolder();
    allocator_->free(position_list_holder_);
  }
  allocator_ = nullptr;
  position_list_ = nullptr;
  position_list_holder_ = nullptr;
}

int ObFTTokenInfo::update_one_position(common::ObIAllocator &allocator, const int64_t position)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(position < 0)) {
    ret = OB_INVALID_ARGUMENT;
  } else if (count_ >= MAX_POSITION_LIST_COUNT) {
    // 与上游一致：超过上限只继续累计词频，不让超长文档中断建索引。
  } else {
    if (OB_ISNULL(position_list_)) {
      void *buf = allocator.alloc(sizeof(ObFTPositionListHolder));
      if (OB_ISNULL(buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("failed to allocate position list", K(ret));
      } else {
        allocator_ = &allocator;
        position_list_holder_ = new (buf) ObFTPositionListHolder();
        position_list_ = &position_list_holder_->position_list_;
      }
    }
    if (OB_FAIL(ret)) {
    } else if (count_ > 0 && OB_UNLIKELY(position_list_->at(count_ - 1) >= position)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("token positions must be strictly increasing", K(ret), K(position), KPC(position_list_));
    } else if (OB_FAIL(position_list_->push_back(position))) {
      LOG_WARN("failed to append token position", K(ret), K(position));
    }
  }
  if (OB_SUCC(ret)) {
    ++count_;
  }
  return ret;
}

int ObFTTokenInfo::update_without_pos_list()
{
  ++count_;
  return OB_SUCCESS;
}

} // namespace storage
} // namespace oceanbase
