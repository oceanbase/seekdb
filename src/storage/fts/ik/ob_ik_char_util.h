...
private:
  ...
  static int do_classify_single_decode(const char *input, const uint8_t char_len, CharType &type);
...
template <ObCharsetType CS_TYPE>
inline int ObFTCharUtil::do_classify_single_decode(const char *input, const uint8_t char_len, CharType &type)
{
  int ret = OB_SUCCESS;
  ob_wc_t unicode = 0;
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(ObCharset::first_valid_char(coll_type, input, remaining_len, char_len))) {
    STORAGE_FTS_LOG(WARN, "Failed to get first valid char", K(ret));
  } else {
    ObCharsetType cs_type = ObCharset::charset_type_by_coll(coll_type);
    switch (cs_type) {
    case CHARSET_UTF8MB4: { ret = do_classify_single_decode<CHARSET_UTF8MB4>(input, static_cast<uint8_t>(char_len), type); break; }
    case CHARSET_UTF16:   { ret = do_classify_single_decode<CHARSET_UTF16>(input, static_cast<uint8_t>(char_len), type); break; }
    case CHARSET_UTF16LE: { ret = do_classify_single_decode<CHARSET_UTF16LE>(input, static_cast<uint8_t>(char_len), type); break; }
    default: ret = OB_NOT_SUPPORTED; STORAGE_FTS_LOG(WARN, "Not supported charset", K(ret), K(cs_type)); break;
    }
  }
  return ret;
}
