...
{
  int ret = OB_SUCCESS;
  int64_t char_len_result = 0;
  if (OB_FAIL(ObCharset::first_valid_char(coll_type, input, input_len, char_len_result))) {
    STORAGE_FTS_LOG(WARN, "Failed to get first valid char", K(ret));
  } else {
    char_len = char_len_result;
    if (OB_FAIL(classify_first_char(coll_type, input, static_cast<uint8_t>(char_len_result), type))) {
