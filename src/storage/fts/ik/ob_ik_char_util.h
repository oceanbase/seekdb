...
public:
  ...
  /// @brief Combined first_valid_char + classify_first_char with single decode_unicode.
  ///        Merges ObCharset::first_valid_char() and ObFTCharUtil::classify_first_char()
  ///        into one call.  decode_unicode() is called exactly once instead of up to 6 times.
...
private:
  /// @brief Classify a single character using a pre-decoded unicode codepoint.
