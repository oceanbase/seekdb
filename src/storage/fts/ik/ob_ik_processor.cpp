...
TokenizeContext::TokenizeContext(ObCollationType coll_type,
    ...
    : coll_type_(coll_type), charset_info_(ObCharset::get_charset(coll_type)),
      ...
      next_char_len_(0), next_char_type_(ObFTCharUtil::CharType::USELESS),
      handle_size_(0), is_smart_(is_smart), token_list_(allocator),
...
{
  return reuse(fulltext, fulltext_len, is_smart_);
}
