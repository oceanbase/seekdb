...
int TokenizeContext::reuse_document(const char *fulltext, int64_t fulltext_len)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(fulltext) || fulltext_len <= 0) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid document for tokenize context reuse", K(ret), KP(fulltext), K(fulltext_len));
  } else {
    fulltext_ = fulltext;
    ...
    if (OB_FAIL(prepare_next_char())) {
      LOG_WARN("failed to prepare reused document", K(ret));
    }
  }
  return ret;
}
