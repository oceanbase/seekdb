...
// Reusable segmented storage for POD-like objects on tokenization hot paths.
// The real block size is rounded up to a power of two so locating an element
// needs only a shift and a mask.
template <typename T, int64_t block_capacity = 256>
class ObFastSegmentArray
{
  static_assert(block_capacity > 2 && block_capacity <= (1 << 30),
      "block capacity must be greater than 2 and no greater than (1 << 30)");
...
private:
    ...
    if (block_idx >= block_count_) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_UNLIKELY(nullptr == block_arr_[block_idx])) {
          ...
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("unexpected initialized segment block", K(ret), K(block_idx));
        }
      }
    }
    return ret;
  ...
  int expand_block_array_(const int64_t need_cap)
  {
    ...
    if (OB_ISNULL(next_arr)) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("failed to allocate segment block pointer array", K(ret), K(next_cap));
    } else {
