...
class ObFastSegmentArray
{
  static_assert(block_capacity > 2 && block_capacity <= (1 << 30),
      "block capacity must be greater than 2 and less than or equal to (1 << 30)");

public:
  static constexpr int64_t BLOCK_POINTER_ARRAY_CAPACITY = 4;
...
private:
    ...
    if (block_idx >= block_count_) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_UNLIKELY(nullptr == block_arr_[block_idx])) {
          ...
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("Unexpected existing block", K(ret), K(block_idx));
        }
      }
    }
    return ret;
  ...
  int expand_block_array_(const int64_t need_cap)
  {
    ...
    if (nullptr == next_block_arr) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("Failed to allocate block pointer array", K(ret), K(next_block_arr_cap));
    } else {
      MEMSET(next_block_arr, 0, bytes);
      if (nullptr != block_arr_) {
        MEMCPY(next_block_arr,
               block_arr_,
               block_count_ * static_cast<int64_t>(sizeof(T *)));
        allocator_.free(block_arr_);
        block_arr_ = nullptr;
      }
