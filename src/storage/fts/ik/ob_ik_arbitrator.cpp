...
int ObIKArbitrator::output_result(TokenizeContext &ctx)
{
  ...
  for (int64_t current = ctx.get_buffer_start_cursor();
       OB_SUCC(ret) && current < ctx.get_buffer_end_cursor();) {
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(chains_.create(IK_HANDLE_SIZE_LIMIT * 2L, ObMemAttr("IK ARBITRATE")))) {
