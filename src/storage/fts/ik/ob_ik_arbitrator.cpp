    ...
    if (OB_FAIL(ObCharset::first_valid_char(ctx.collation(),
      ...
      if (!keep_single()) {
        ...
        if (ObFTCharUtil::CharType::CHINESE == type) {
          ...
          if (OB_FAIL(ctx.results().push_back(token))) {
          ...
          if (OB_FAIL(ObFTCharUtil::is_ignore_single_cjk(ctx.collation(),
            ...
            LOG_WARN("Failed to check ignore", K(ret));
                     ...
                     && OB_FAIL(ctx.results().push_back(token))) {
      ...
      for (ObList<ObIKToken, ObIAllocator>::const_iterator iter = chain->list().tokens().begin();
        ...
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(ctx.results().push_back(token))) {
          LOG_WARN("Failed to add token to ctx result", K(ret));
        } else
          // output the token
...
{
  int ret = OB_SUCCESS;
  // Hash nodes reference chains allocated from alloc_; clear them first.
  if (chains_.created() && OB_FAIL(chains_.reuse())) {
    LOG_WARN("failed to reuse chain map", K(ret));
  } else {
    // FTS next-stage optimization (reference 5bebabc): retain one hot page,
    // but release burst pages so a large document does not inflate every
    // later batch in the same parser lifetime.
