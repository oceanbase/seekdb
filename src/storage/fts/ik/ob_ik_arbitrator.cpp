...
int ObIKArbitrator::output_result(TokenizeContext &ctx)
{
  ...
  for (int64_t current = 0; OB_SUCC(ret) && current < ctx.fulltext_len();) {
    ...
    if (OB_FAIL(ctx.classify_char_at(current, char_len, type))) {
      LOG_WARN("Failed to classify next valid char", K(ret));
...
int ObIKArbitrator::prepare(TokenizeContext &ctx)
{
  ...
  if (OB_FAIL(reuse())) {
    LOG_WARN("reuse arbitrator failed", K(ret));
  } else if (!chains_.created()
