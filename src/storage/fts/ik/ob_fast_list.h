...
// FTS next-stage optimization (Op1): provide stable list nodes from reusable
// blocks while preserving the ordering and iterator semantics used by IK.
...
private:
  ...
  int insert_before_(Node *pos, const T &value)
  {
    ...
    if (OB_FAIL(pool_.push_back(dummy))) {
      ...
      Node *anchor = nullptr == pos ? static_cast<Node *>(root_) : pos;
      node->next = anchor;
