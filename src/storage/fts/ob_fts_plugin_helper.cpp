...
int ObFTParsePluginData::refresh_custom_dict(const uint64_t table_id,
                                             const ObString &qualified_table_name,
                                             int64_t &range_count)
{
  int ret = OB_SUCCESS;
  range_count = 0;
  ObArenaAllocator allocator("RefreshFTDict");
  ObFTCacheRangeContainer cache_container(allocator);
  ObFTDictDesc dict_desc("main_dict",
                         ObFTDictType::DICT_IK_MAIN,
                         ObCharsetType::CHARSET_UTF8MB4,
                         ObCollationType::CS_TYPE_UTF8MB4_BIN);
  // Parser properties persist the qualified table name, but not the resolved table ID.
  // Keep REFRESH on the same name-based identity until both paths can use persisted IDs.
  dict_desc.table_id_ = OB_INVALID_ID;
  dict_desc.table_name_ = qualified_table_name;
  if (OB_UNLIKELY(!is_inited_) || OB_ISNULL(dict_hub_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("fulltext plugin data is not initialized", K(ret));
  } else if (OB_UNLIKELY(OB_INVALID_ID == table_id || qualified_table_name.empty())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid custom dictionary identity", K(ret), K(table_id), K(qualified_table_name));
  } else if (OB_FAIL(dict_hub_->refresh_custom_cache(dict_desc, cache_container))) {
    LOG_WARN("failed to refresh custom dictionary", K(ret), K(dict_desc));
  } else {
    range_count = cache_container.get_handles().size();
  }
  return ret;
}
    ...
    ObFTTokenMap &ft_token_map) const
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    param.ik_param_.main_dict_name_ = parser_property_.dict_table_name_;
    param.ik_param_.quan_dict_name_ = parser_property_.quantifier_table_name_;
    param.ik_param_.stopword_dict_name_ = parser_property_.stopword_table_name_;
    param.is_ddl_mode_ = is_ddl_mode_;
    param.need_casedown_ = need_tolower;
