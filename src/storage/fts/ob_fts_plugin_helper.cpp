...
{
  // A reused IK parser pins its dictionary handles. Restrict reuse to a DDL
  // build, where the dictionary snapshot remains fixed for the whole build.
  return enable_parser_reuse_ && is_ddl_mode_ && parser_name_.is_ik();
}
