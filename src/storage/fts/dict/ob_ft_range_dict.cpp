...
int ObFTRangeDict::build_cache_from_custom_table(const ObFTDictDesc &desc,
                                                 ObFTCacheRangeContainer &range_container)
{
  int ret = OB_SUCCESS;
  range_container.reset();
  if (OB_UNLIKELY(desc.table_name_.empty())) {
    ...
    const char *second_dot =
        OB_ISNULL(dot) ? nullptr : ObString(desc.table_name_.ptr() + desc.table_name_.length() - dot - 1,
                                            dot + 1).find('.');
    if (OB_ISNULL(dot)
        || dot == desc.table_name_.ptr()
        || dot == desc.table_name_.ptr() + desc.table_name_.length() - 1
        || OB_NOT_NULL(second_dot)
        || OB_NOT_NULL(desc.table_name_.find('`'))) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("custom dict table must be a safe qualified name", K(ret), K(desc.table_name_));
    } else {
      const int32_t db_len = static_cast<int32_t>(dot - desc.table_name_.ptr());
      db_name.assign_ptr(desc.table_name_.ptr(), db_len);
      table_name.assign_ptr(dot + 1, desc.table_name_.length() - db_len - 1);
    }
    if (OB_SUCC(ret)) {
      SMART_VAR(ObISQLClient::ReadResult, result)
      {
        ObFTDictTableIter iter_table(result);
        if (OB_FAIL(iter_table.init(db_name, table_name))) {
          if (OB_ITER_END == ret) {
            ret = OB_SUCCESS;
          } else {
            LOG_WARN("Failed to init custom dict iterator", K(ret), K(db_name), K(table_name));
          }
        } else if (OB_FAIL(ObFTRangeDict::build_ranges_concurrently_thread_pool(desc,
                                                                                iter_table,
                                                                                range_container))) {
          LOG_WARN("Failed to build custom dict ranges", K(ret), K(db_name), K(table_name));
        }
      }
    }
  }
