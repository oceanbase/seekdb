...
{
  int ret = OB_SUCCESS;
  ObFTDictInfoKey key(desc.cache_id(), desc.type_);
  ObFTDictInfo info;
  container.reset();
    ...
    if (OB_FAIL(ret)) {
      if (OB_ENTRY_NOT_EXIST == ret) {
        ...
        if (is_builtin && OB_FAIL(ObFTRangeDict::build_cache_from_ik_dict(desc, container))) {
          LOG_WARN("Failed to build built-in cache", K(ret));
...
int ObFTDictHub::refresh_dict(const ObString &name, const uint64_t tenant_id)
{
  int ret = OB_SUCCESS;
  const uint64_t cache_id = ObFTDictDesc::calc_cache_id(name, tenant_id);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < ARRAYSIZEOF(types); ++i) {
    const ObFTDictInfoKey key(cache_id, types[i]);
    ObFTDictInfo info;
    ObBucketHashWLockGuard guard(rw_dict_lock_, key.hash());
    if (OB_FAIL(get_dict_info(key, info))) {
      if (OB_HASH_NOT_EXIST == ret) {
        ...
        LOG_WARN("failed to get dictionary info for refresh", K(ret), K(name), K(types[i]));
      }
    } else {
      for (int32_t range_id = 0; OB_SUCC(ret) && range_id < info.range_count_; ++range_id) {
        const ObDictCacheKey cache_key(cache_id, types[i], range_id);
        if (OB_FAIL(ObDictCache::get_instance().erase(cache_key))) {
          if (OB_ENTRY_NOT_EXIST == ret) {
            ret = OB_SUCCESS;
          } else {
            LOG_WARN("failed to erase dictionary cache", K(ret), K(name), K(types[i]), K(range_id));
          }
        }
      }
      if (OB_SUCC(ret) && OB_FAIL(dict_map_.erase_refactored(key)) && OB_HASH_NOT_EXIST != ret) {
        LOG_WARN("failed to erase dictionary metadata", K(ret), K(name), K(types[i]));
