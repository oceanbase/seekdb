...
ObFTDictCacheLoaderBase::ObFTDictCacheLoaderBase()
    : dict_mgr_(nullptr == share::g_mp ? nullptr : share::g_mp->ft_dict_mgr()),
  ...
  SMART_VAR(ObISQLClient::ReadResult, result)
  {
    ...
    if (OB_FAIL(iter_table.init(desc.table_name_, tenant_id_, snapshot_version, desc.need_casedown_, partial_ranges))) {
      if (OB_SNAPSHOT_DISCARDED == ret) {
        ...
        ret = OB_SUCCESS;
        if (OB_ISNULL(partial_ranges)
            && OB_FAIL(build_empty_range(desc, snapshot_version, range_container))) {
          LOG_WARN("Failed to publish empty dictionary cache", K(ret), K(desc));
        }
      }
...
int ObFTDictCacheLoaderBase::build_empty_range(
    const ObFTDictDesc &desc,
    const int64_t snapshot_version,
    ObFTCacheRangeContainer &range_container)
{
  int ret = OB_SUCCESS;
  ObArenaAllocator tmp_alloc(lib::ObMemAttr("FTEmptyDict"));
  ObFTTrie<void> trie(tmp_alloc, desc.coll_type_);
  ObFTDATBuilder<void> builder(tmp_alloc);
  ObFTDAT *dat_block = nullptr;
  ObFTCacheRangeHandle *handle = nullptr;

  if (OB_FAIL(builder.init(trie))) {
    LOG_WARN("Failed to initialize empty dictionary DAT", K(ret), K(desc));
  } else if (OB_FAIL(builder.build_from_trie(trie))) {
    LOG_WARN("Failed to build empty dictionary DAT", K(ret), K(desc));
  } else if (FALSE_IT(builder.get_mem_block(dat_block))) {
  } else if (OB_ISNULL(dat_block)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("Empty dictionary DAT is null", K(ret), K(desc));
  } else if (OB_FAIL(range_container.fetch_info_for_dict(handle))) {
    LOG_WARN("Failed to allocate empty dictionary cache handle", K(ret));
  } else if (OB_FAIL(ObFTCacheDict::put_and_fetch_cache_entry(
                 desc.table_id_, tenant_id_, 0 /* range_id */, dat_block,
                 snapshot_version, 1 /* sentinel range_count */, *handle))) {
    LOG_WARN("Failed to publish empty dictionary cache", K(ret), K(desc));
  }
                                                          ...
                                                          int64_t &row_count)
{
  ...
  ObMySQLProxy *sql_proxy = nullptr == share::g_mp || nullptr == share::g_mp->trans_service()
                            ? nullptr : share::g_mp->trans_service()->get_mysql_proxy();
  ...
  SMART_VAR(ObISQLClient::ReadResult, result)
  {
    if (OB_FAIL(sql_string.append_fmt("SELECT MAX(ORA_ROWSCN) as max_version, COUNT(*) as cnt FROM %.*s",
      ...
      } else if (OB_FAIL(sql_proxy->read(result, sql_string.ptr(), &session_param))) {
                                                      ...
                                                      ObFTCacheRangeContainer &range_container)
{
  ...
  if (OB_FAIL(get_first_range_handle(range_container, first_handle))) {
    ...
    if (OB_UNLIKELY(blk_sz <= 0)) {
      ...
      ObArenaAllocator dup_alloc(lib::ObMemAttr("FTDictSnapDup"));
...
{
  int ret = OB_SUCCESS;
  transaction::ObTransService *txs = nullptr == share::g_mp ? nullptr : share::g_mp->trans_service();
  snapshot_version = 0;
                                           ...
                                           const ObIArray<ObMissingRangeInfo> *partial_ranges)
{
  ...
  ObArenaAllocator tmp_alloc(lib::ObMemAttr("PendingDatBuff"));
...
int ObFTDictCacheLoaderRefresh::load_cache(const ObFTDictDesc &desc)
{
  ObArenaAllocator tmp_alloc(lib::ObMemAttr("FTRangeRefresh"));
...
{
  int ret = OB_SUCCESS;
  ObArenaAllocator allocator(lib::ObMemAttr("FTRangeRefresh"));
