...
protected:
  ...
  int build_empty_range(const ObFTDictDesc &desc,
                        const int64_t snapshot_version,
                        ObFTCacheRangeContainer &range_container);
...
protected:
  ...
  public:
    ...
    {
      int ret = OB_SUCCESS;
      if (OB_ISNULL(dict_mgr_)) {
        ret = OB_ERR_UNEXPECTED;
      } else if (OB_FAIL(dict_mgr_->acquire_slot(table_id_, slot_idx_))) {
        // The caller handles OB_EAGAIN by waiting for the current builder.
      } else {
        acquired_ = true;
      }
      return ret;
    ...
    {
      return OB_ISNULL(dict_mgr_) ? OB_ERR_UNEXPECTED
                                  : dict_mgr_->wait_build_complete(table_id_, slot_idx_);
    }
