...
public:
  ...
  ObFTDictInfoKey(const uint64_t identity)
      : identity_(identity)
  {
  }
  ...
  {
    uint64_t hash = 0;
    hash = common::murmurhash(&identity_, sizeof(identity_), hash);
    return hash;
  }
  ...
  int compare(const ObFTDictInfoKey &other) const
  {
    int ret = 0;
    if (0 == ret) {
      ret = identity_ < other.identity_ ? -1 : (identity_ > other.identity_ ? 1 : 0);
    }
    return ret;
  }
