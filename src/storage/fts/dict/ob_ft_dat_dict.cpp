...
int ObFTDATBuilder<DATA_TYPE>::init(ObFTTrie<DATA_TYPE> &trie)
{
  ...
  if (IS_INIT) {
    ...
    size_t map_size = ObFTDATArrayHashMap::calc_memory_size(trie.node_num());
    // Keep room for FIRST_INDEX even for an empty dictionary sentinel.
    size_t array_size = MAX(static_cast<size_t>(8), trie.node_num() * 6); // by experience.
...
ObFTDATArrayHashMap *ObFTDAT::get_map() { return reinterpret_cast<ObFTDATArrayHashMap *>(buff); }
const ObFTDATArrayHashMap *ObFTDAT::get_map() const { return reinterpret_cast<const ObFTDATArrayHashMap *>(buff); }

int ObFTDATArrayHashMap::find(const ObString &token, ObFTTokenCode &code) const
{
...
int ObFTDATArrayHashMap::insert(const ObString &token, ObFTTokenCode code)
{
  int ret = OB_SUCCESS;
  uint64_t hash = token.hash();
...
int ObFTDATArrayHashMap::init(size_t token_cnt)
{
  int ret = OB_SUCCESS;
...
size_t ObFTDATArrayHashMap::calc_memory_size(size_t token_cnt)
{
  size_t capacity = calc_capacity(token_cnt);
...
size_t ObFTDATArrayHashMap::calc_capacity(size_t token_cnt)
{
