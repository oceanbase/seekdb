...
void ObFTDictRefreshTask::runTimerTask()
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    common::ObArenaAllocator allocator(common::ObMemAttr("FTDictRefresh"));
...
int ObFTDictRefreshTask::get_sql_statement(common::ObSqlString &sql)
{
  ...
  ObFTDictMgr *dict_mgr = nullptr == share::g_mp ? nullptr : share::g_mp->ft_dict_mgr();
