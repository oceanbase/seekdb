                                         ...
                                         int64_t &word_freq)
{
  ...
  if (ascii_cur_ >= ascii_end_) {
    ...
    if (needs_casedown) {
      ...
      if (OB_ISNULL(buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("failed to allocate lowercase ASCII token", K(ret), K(len));
      } else {
...
int ObBEngFTParser::reuse_parser(const char *fulltext, const int64_t fulltext_len)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    // Retain one page to avoid malloc/free churn for mixed-case tokens.
    ...
    ObITokenIterator *&iter) const
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("basic English parser metadata allocator is null", K(ret));
  } else if (OB_ISNULL(parser = OB_NEWx(ObBEngFTParser, metadata_allocator,
                                        *metadata_allocator))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
