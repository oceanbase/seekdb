...
int ObBEngFTParser::reuse_parser(const char *fulltext, const int64_t fulltext_len)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_FAIL(segment(doc_, token_stream_))) {
      LOG_WARN("fail to reopen english token stream", K(ret));
    } else if (OB_ISNULL(token_stream_)) {
      ret = OB_ERR_UNEXPECTED;
    }
  }
...
int ObBEngFTParser::init(ObFTParserParam *param)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    if (OB_FAIL(english_analyzer_.init(analysis_ctx_, metadata_alloc_))) {
    ...
    ObITokenIterator *&iter) const
{
  ...
  } else if (OB_ISNULL(parser = OB_NEWx(ObBEngFTParser, param->metadata_alloc_,
                                        *(param->metadata_alloc_), *(param->scratch_alloc_)))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
