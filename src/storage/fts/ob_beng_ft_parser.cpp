...
namespace
{
using oceanbase::storage::ascii::count_leading_delimiter;
using oceanbase::storage::ascii::count_leading_token;
using oceanbase::storage::ascii::has_ascii_upper;
using oceanbase::storage::ascii::is_ascii_string;
using oceanbase::storage::ascii::lowercase_ascii_copy;
} // namespace
...
bool ObBEngFTParser::is_ascii_document(const char *fulltext, const int64_t fulltext_len) const
{
  ...
  if (is_ascii) {
    is_ascii = storage::ascii::is_ascii_string(fulltext, fulltext_len);
  }
  return is_ascii;
}
