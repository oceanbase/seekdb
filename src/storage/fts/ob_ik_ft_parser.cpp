...
int ObIKFTParser::init_dict(const plugin::ObFTParserParam &param)
{
  ...
  // A non-default dict_table REPLACES the built-in main dictionary (not merged).
  const ObString &main_dict_name = param.ik_param_.main_dict_name_;
  const bool use_custom_main =
      !main_dict_name.empty()
      && (0 != main_dict_name.case_compare(ObFTSLiteral::FT_DEFAULT_IK_DICT_UTF8_TABLE))
      && (0 != main_dict_name.case_compare(ObFTSLiteral::FT_NONE))
      && (0 != main_dict_name.case_compare(ObFTSLiteral::FT_DEFAULT));

  if (OB_FAIL(ret)) {
    // already logged
  } else if (use_custom_main) {
    main_dict_desc.table_id_ = param.ik_param_.main_dict_id_;
    main_dict_desc.table_name_ = main_dict_name;
    if (OB_FAIL(hub_->load_or_build_custom_cache(main_dict_desc, cache_main_))) {
      LOG_WARN("Failed to load custom main dict", K(ret), K(main_dict_name));
    }
