...
class ObTabletTryWashFunc final
{
public:
  explicit ObTabletTryWashFunc(ObTabletMetaMemMgr &t3m);
  ~ObTabletTryWashFunc();
...
private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ObTabletTryWashFunc);
};

class ObIMetaObjPool
{
public:
  ObIMetaObjPool() = default;
  virtual ~ObIMetaObjPool() = default;
...
public:
  ObMetaObjPool(
      ...
      ObTabletTryWashFunc *wash_func = nullptr,
      const bool allow_over_max_free_num = true);
  virtual ~ObMetaObjPool();
...
template <class T>
void ObMetaObjPool<T>::free_obj(void *obj)
{
  release(static_cast<T *>(obj));
}

template <class T>
int ObMetaObjPool<T>::alloc_obj(void *&obj)
{
  int ret = OB_SUCCESS;
  T *t = nullptr;
...
template <class T>
int ObMetaObjPool<T>::acquire(T *&t)
{
  int ret = OB_SUCCESS;
  t = nullptr;
...
template <class T>
void ObMetaObjPool<T>::release(T *t)
{
  BasePool::free(t);
  (void)ATOMIC_AAF(&used_obj_cnt_, -1);
}
...
template <class T>
void ObMetaObjPool<T>::free_node_(typename BasePool::Node *ptr)
{
  if (NULL != ptr) {
    ptr->data.reset();
    ptr->next = NULL;
...
template <class T>
ObMetaObjPool<T>::ObMetaObjPool(
    const int64_t max_free_list_num,
    const lib::ObLabel &label,
    const uint64_t ctx_id,
    ObTabletTryWashFunc *wash_func,
...
#endif /* OCEANBASE_STORAGE_OB_META_OBJ_POOL_H_ */
