...
#ifndef OCEANBASE_STORAGE_OB_STORAGE_META_MEM_MGR_H_
#define OCEANBASE_STORAGE_OB_STORAGE_META_MEM_MGR_H_
...
struct ObStorageMetaMemStatus final
{
public:
  ObStorageMetaMemStatus();
public:
...
class ObStorageMetaMemMgr final
{
public:
  ...
  static const int64_t LOCAL_LS_COUNT = 1;
  static const int64_t MAX_TX_DATA_MEMTABLE_CNT_IN_OBJ_POOL = MAX_MEMSTORE_CNT * LOCAL_LS_COUNT;
  static const int64_t MAX_TX_CTX_MEMTABLE_CNT_IN_OBJ_POOL = LOCAL_LS_COUNT;
  static const int64_t MAX_LOCK_MEMTABLE_CNT_IN_OBJ_POOL = LOCAL_LS_COUNT;
...
private:
  explicit ObStorageMetaMemMgr();
public:
  ~ObStorageMetaMemMgr();
  static int server_module_new(ObStorageMetaMemMgr *&meta_mem_mgr);
  ...
  int get_meta_mem_status(common::ObIArray<ObStorageMetaMemStatus> &info) const;
...
public:
  ...
  {
  public:
    explicit ObT3MResourceLimitCalculatorHandler(ObStorageMetaMemMgr &t3m) : t3m_(t3m) {}
...
private:
  ...
  {
  public:
    explicit TabletGCTask(ObStorageMetaMemMgr *t3m) : t3m_(t3m) {}
  ...
  {
  public:
    explicit TableGCTask(ObStorageMetaMemMgr *t3m) : t3m_(t3m)
    {
  ...
  public:
    TabletGCQueue()
      : gc_head_(nullptr),
        gc_tail_(nullptr),
        tablet_count_(0),
        queue_lock_(common::ObLatchIds::STORAGE_META_MEM_MGR_LOCK)
    {}
...
private:
  ...
  int get_obj_pool_info(
      const ObStorageMetaObjPool<T> &obj_pool,
      const char *name,
...
private:
  ...
  ObStorageMetaObjPool<memtable::ObMemtable> memtable_pool_;
  ObStorageMetaObjPool<ObNormalTabletBuffer> tablet_buffer_pool_;
  ObStorageMetaObjPool<ObLargeTabletBuffer> large_tablet_buffer_pool_;
  ObStorageMetaObjPool<ObDDLKV> ddl_kv_pool_;
  ObStorageMetaObjPool<ObTabletDDLKvMgr> tablet_ddl_kv_mgr_pool_;
  ObStorageMetaObjPool<ObTxDataMemtable> tx_data_memtable_pool_;
  ObStorageMetaObjPool<ObTxCtxMemtable> tx_ctx_memtable_pool_;
  ObStorageMetaObjPool<transaction::tablelock::ObLockMemtable> lock_memtable_pool_;
  ObIStorageMetaObjPool *pool_arr_[ObITable::TableType::MAX_TABLE_TYPE];
...
class ObITabletIterator
{
public:
  ObITabletIterator() = default;
  virtual ~ObITabletIterator() = default;
  virtual int get_next_tablet(ObTabletHandle &handle) = 0;
};

class ObITabletPointerIterator
{
public:
  ObITabletPointerIterator() = default;
  virtual ~ObITabletPointerIterator() = default;
...
public:
  explicit ObT3mTabletMapIterator(ObStorageMetaMemMgr &t3m);
...
class ObTabletIterator : public ObT3mTabletMapIterator,
                         public ObITabletIterator
{
public:
  ObTabletIterator(
      ...
      common::ObArenaAllocator &allocator,
      ObITabletFilterOp *op);
  virtual ~ObTabletIterator() = default;
...
class ObInMemoryTabletIterator : public ObT3mTabletMapIterator,
                                 public ObITabletIterator
{
public:
  explicit ObInMemoryTabletIterator(ObStorageMetaMemMgr &t3m);
  virtual ~ObInMemoryTabletIterator() = default;
  virtual int get_next_tablet(ObTabletHandle &handle) override;
};

class ObTabletPtrWithInMemObjIterator : public ObT3mTabletMapIterator,
                                        public ObITabletPointerIterator
{
public:
  explicit ObTabletPtrWithInMemObjIterator(ObStorageMetaMemMgr &t3m);
  virtual ~ObTabletPtrWithInMemObjIterator() = default;
  virtual int get_next_tablet_pointer(
      ObTabletMapKey &key,
      ObTabletPointerHandle &pointer_handle,
...
template <typename T>
int ObStorageMetaMemMgr::get_obj_pool_info(
    const ObStorageMetaObjPool<T> &obj_pool,
    const char *name,
    common::ObIArray<ObStorageMetaMemStatus> &info) const
{
...
#endif /* OCEANBASE_STORAGE_OB_STORAGE_META_MEM_MGR_H_ */
