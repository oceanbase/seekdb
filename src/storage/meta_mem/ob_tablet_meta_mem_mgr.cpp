...
namespace storage
{
ObTabletMetaMemStatus::ObTabletMetaMemStatus()
  : total_size_(0),
    used_size_(0),
    used_obj_cnt_(0),
    free_obj_cnt_(0),
...
void ObTabletMetaMemMgr::TabletGCTask::runTimerTask()
{
  int ret = OB_SUCCESS;
...
void ObTabletMetaMemMgr::TableGCTask::runTimerTask()
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::TabletGCQueue::push(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(tablet)) {
    ret = OB_INVALID_ARGUMENT;
...
ObTabletMetaMemMgr::ObTabletMetaMemMgr()
  : wash_lock_(common::ObLatchIds::TABLET_META_MEM_MGR_LOCK),
    wash_func_(*this),
    bucket_lock_(),
    ...
    tablet_gc_queue_(),
    free_tables_queue_(),
    gc_queue_lock_(common::ObLatchIds::TABLET_META_MEM_MGR_LOCK),
...
int ObTabletMetaMemMgr::module_new(ObTabletMetaMemMgr *&meta_mem_mgr)
{
  int ret = OB_SUCCESS;


  meta_mem_mgr = OB_NEW(ObTabletMetaMemMgr, ObMemAttr("MetaMemMgr"));
...
int ObTabletMetaMemMgr::init()
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ret = OB_INIT_TWICE;
    LOG_WARN("ObTabletMetaMemMgr has been initialized", K(ret));
    ...
    LOG_WARN("fail to init full tablet creator", K(ret));
    ...
    LOG_WARN("fail to initialize external tablet cnt map", K(ret), K(bucket_num));
...
int ObTabletMetaMemMgr::check_allow_tablet_gc(
    ...
    bool &allow)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTabletMetaMemMgr hasn't been inited", K(ret));
...
void ObTabletMetaMemMgr::init_pool_arr()
{
...
int ObTabletMetaMemMgr::start()
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTabletMetaMemMgr hasn't been inited", K(ret));
  } else if (OB_FAIL(gc_timer_.init("TabletMetaMem", ObMemAttr("TabletMetaMem")))) {
...
void ObTabletMetaMemMgr::stop()
{
  ...
  // so t3m can't stop gc thread until check_all_meta_mem_released is true in ObTabletMetaMemMgr::wait()
}

void ObTabletMetaMemMgr::wait()
{
...
void ObTabletMetaMemMgr::destroy()
{
  gc_timer_.destroy();
...
int ObTabletMetaMemMgr::print_old_chain(
...
int ObTabletMetaMemMgr::push_table_into_gc_queue(ObITable *table, const ObITable::TableType table_type)
{
  int ret = OB_SUCCESS;
  const int64_t size = sizeof(TableGCItem);
  const ObMemAttr attr("TableGCItem");
...
int ObTabletMetaMemMgr::gc_tables_in_queue(bool &all_table_cleaned)
{
  int ret = OB_SUCCESS;
      ...
      if (OB_FAIL(free_tables_queue_.pop(ptr))) {
        ...
        if (OB_ISNULL(table)) {
          ...
          if (OB_UNLIKELY(!ObITable::is_table_type_valid(table_type) || nullptr == pool_arr_[index])) {
            ...
            if (ObITable::TableType::DATA_MEMTABLE == table_type) {
              if (OB_FAIL(push_memtable_into_gc_set_(static_cast<memtable::ObMemtable *>(table)))) {
                LOG_WARN("push memtable into gc set failed", K(ret));
              }
            } else {
...
void ObTabletMetaMemMgr::batch_destroy_memtable_(memtable::ObMemtableSet *memtable_set)
{
...
void ObTabletMetaMemMgr::destroy_gc_memtable_set_(memtable::ObMemtableSet *&memtable_set)
{
  int ret = OB_SUCCESS;
  if (OB_NOT_NULL(memtable_set)) {
    ...
    memtable_set = nullptr;
  }
}

int ObTabletMetaMemMgr::prepare_gc_memtable_set_(memtable::ObMemtableSet *&memtable_set)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(memtable_set)) {
    const ObMemAttr attr("memtable_set");
    void *buf = nullptr;
    ...
    if (OB_FAIL(ret)) {
      if (OB_NOT_NULL(tmp_memtable_set)) {
        int tmp_ret = OB_SUCCESS;
...
void ObTabletMetaMemMgr::batch_gc_memtable_set_(
    memtable::ObMemtableSet *memtable_set,
    const bool remove_tx_callbacks)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  if (OB_NOT_NULL(memtable_set)
      && 0 != memtable_set->size()) {
    if (remove_tx_callbacks) {
      if (OB_TMP_FAIL(ObMemtable::batch_remove_unused_callback_for_uncommited_txn(memtable_set))) {
...
void ObTabletMetaMemMgr::batch_gc_memtable_()
{
  batch_gc_memtable_set_(gc_memtable_set_, true);
  batch_gc_memtable_set_(gc_uninit_memtable_set_, false);
...
int ObTabletMetaMemMgr::push_memtable_into_gc_set_(memtable::ObMemtable *memtable)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(prepare_gc_memtable_set_(gc_memtable_set_))) {
    LOG_WARN("failed to prepare gc memtable set", K(ret), KPC(memtable));
    ...
    LOG_WARN("memtable set insert failed", K(ret), KPC(memtable));
  }

  return ret;
}

int ObTabletMetaMemMgr::inner_push_tablet_into_gc_queue(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::push_tablet_into_gc_queue(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    ...
    const ObDieingTabletMapKey dieing_tablet_key(key);
    ObBucketHashWLockGuard lock_guard(bucket_lock_, key.hash());
...
int ObTabletMetaMemMgr::gc_tablets_in_queue(bool &all_tablet_cleaned)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::has_meta_wait_gc(bool &is_wait)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
void *ObTabletMetaMemMgr::release_tablet(ObTablet *tablet, const bool return_buf_ptr_after_release)
{
...
int ObTabletMetaMemMgr::get_min_end_scn_for_ls(
    const ObTabletMapKey &key,
...
{
  int ret = OB_SUCCESS;
  ObArenaAllocator allocator(common::ObMemAttr("GetMinScn"));
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("ObTabletMetaMemMgr hasn't been initialized", K(ret));
    ...
    LOG_WARN("fail to get pointer from map", KR(ret), K(key), "resource_ptr", ptr_handle.get_resource_ptr());
...
int ObTabletMetaMemMgr::get_min_end_scn_from_single_tablet(ObTablet *tablet,
                                                           const bool is_old,
                                                           const SCN &ls_checkpoint,
                                                           SCN &min_end_scn)
{
  ...
  if (OB_ISNULL(tablet)) {
    ...
    if (contain_uncommitted_row) {
      ...
      cur_recycle_end_scn = SCN::max(tablet->get_tablet_meta().clog_checkpoint_scn_, ls_checkpoint);
      // the clog with scn of checkpoint scn may depend on the tx data with a commit scn of checkpoint scn
      cur_recycle_end_scn = SCN::max(SCN::scn_dec(cur_recycle_end_scn), SCN::min_scn());
    }

    if (cur_recycle_end_scn < min_end_scn) {
      min_end_scn = cur_recycle_end_scn;
    }
  }
  return ret;
}

int ObTabletMetaMemMgr::scan_all_version_tablets(const ObTabletMapKey &key, const ObFunction<int(ObTablet &)> &op)
{
  int ret = OB_SUCCESS;
  ObTablet *cur_tablet = nullptr;
  ObArenaAllocator allocator(common::ObMemAttr("ScanTablets"));
  ObTabletPointerHandle ptr_handle(tablet_map_);
  ObTabletHandle handle;
...
int ObTabletMetaMemMgr::acquire_ddl_kv(ObDDLKVHandle &handle)
{
  int ret = OB_SUCCESS;
...
void ObTabletMetaMemMgr::release_ddl_kv(ObDDLKV *ddl_kv)
{
  if (OB_NOT_NULL(ddl_kv)) {
    if (0 != ddl_kv->get_ref()) {
...
int ObTabletMetaMemMgr::acquire_data_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::acquire_tx_data_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
  ObTxDataMemtable *tx_data_memtable = nullptr;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init ObTabletMetaMemMgr", K(ret));
...
int ObTabletMetaMemMgr::acquire_tx_ctx_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::acquire_lock_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
void ObTabletMetaMemMgr::release_memtable(ObMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
...
void ObTabletMetaMemMgr::release_tx_data_memtable_(ObTxDataMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
int ObTabletMetaMemMgr::acquire_tablet_ddl_kv_mgr(ObDDLKvMgrHandle &handle)
{
  int ret = OB_SUCCESS;
...
void ObTabletMetaMemMgr::release_tablet_ddl_kv_mgr(ObTabletDDLKvMgr *ddl_kv_mgr)
{
...
void ObTabletMetaMemMgr::release_tx_ctx_memtable_(ObTxCtxMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
void ObTabletMetaMemMgr::release_lock_memtable_(ObLockMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
void ObTabletMetaMemMgr::mark_mds_table_deleted_(const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
...
int ObTabletMetaMemMgr::release_memtable_and_mds_table_for_ls_offline(const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
  if (OB_FAIL(tablet_map_.get(key, ptr_handle))) {
...
int ObTabletMetaMemMgr::create_tmp_tablet(
...
int ObTabletMetaMemMgr::acquire_tablet_from_pool(
...
int ObTabletMetaMemMgr::acquire_tablet(
    const ObTabletPoolType type,
    ObTabletHandle &tablet_handle)
{
  int ret = OB_SUCCESS;
  ...
  if (ObTabletPoolType::TP_NORMAL == type) {
    meta_obj.pool_ = static_cast<ObIMetaObjPool *>(&tablet_buffer_pool_);
    header = &normal_tablet_header_;
  } else if (ObTabletPoolType::TP_LARGE == type) {
    meta_obj.pool_ = static_cast<ObIMetaObjPool *>(&large_tablet_buffer_pool_);
...
int ObTabletMetaMemMgr::acquire_tablet(ObIMetaObjPool *pool, ObTablet *&tablet)
{
  int ret = OB_SUCCESS;
  TabletBufferList *header = nullptr;
  void *buf = nullptr;
  ...
  } else if (static_cast<ObMetaObjPool<ObNormalTabletBuffer> *>(pool) == &tablet_buffer_pool_) {
    header = &normal_tablet_header_;
  } else if (static_cast<ObMetaObjPool<ObLargeTabletBuffer> *>(pool) == &large_tablet_buffer_pool_) {
    header = &large_tablet_header_;
  } else {
    ret = OB_NOT_SUPPORTED;
...
int ObTabletMetaMemMgr::acquire_tmp_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    common::ObArenaAllocator &allocator,
    ObTabletHandle &tablet_handle)
...
int ObTabletMetaMemMgr::create_msd_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    ObLS *tenant_ls,
    ObTabletHandle &tablet_handle)
...
int ObTabletMetaMemMgr::create_tablet(
    const ObTabletMapKey &key,
    ObLS *tenant_ls,
    ObTabletHandle &tablet_handle)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    if (OB_FAIL(tenant_ls->get_tablet_svr()->get_tx_ctx_memtable_mgr(memtable_mgr_hdl))) {
      LOG_WARN("fail to get tx data memtable mgr", K(ret));
    }
  } else if (key.tablet_id_.is_ls_lock_tablet()) {
    if (OB_FAIL(tenant_ls->get_tablet_svr()->get_lock_memtable_mgr(memtable_mgr_hdl))) {
      LOG_WARN("fail to get lock memtable mgr", K(ret));
    }
  }
  if (OB_SUCC(ret)) {
    ObTabletPointerHandle ptr_handle(tablet_map_);
    ObTabletPointer tablet_ptr(tenant_ls, memtable_mgr_hdl);
    ObMetaDiskAddr addr;
    addr.set_none_addr();
...
int ObTabletMetaMemMgr::get_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    ObTabletHandle &handle)
{
...
int ObTabletMetaMemMgr::get_tablet_with_filter(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
...
int ObTabletMetaMemMgr::get_tablet_with_allocator(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    common::ObArenaAllocator &allocator,
    ObTabletHandle &handle,
...
int ObTabletMetaMemMgr::build_tablet_handle_for_mds_scan(
    ObTablet *tablet,
    ObTabletHandle &handle)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::get_tablet_buffer_infos(ObIArray<ObTabletBufferInfo> &buffer_infos)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObTabletMetaMemMgr::fill_buffer_infos(
...
int ObTabletMetaMemMgr::get_tablet_addr(const ObTabletMapKey &key, ObMetaDiskAddr &addr)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObTabletMetaMemMgr::get_tablet_pointer_initial_state(const ObTabletMapKey &key, bool &initial_state)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
  if (OB_UNLIKELY(!is_inited_)) {
...
int ObTabletMetaMemMgr::get_tablet_resident_info(
    const ObTabletMapKey &key,
...
int ObTabletMetaMemMgr::get_tablet_ddl_kv_mgr(
    const ObTabletMapKey &key,
...
int64_t ObTabletMetaMemMgr::cal_adaptive_bucket_num()
{
...
int ObTabletMetaMemMgr::get_meta_mem_status(common::ObIArray<ObTabletMetaMemStatus> &info) const
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::push_tablet_pointer_to_fly_map_if_need_(
    const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
  ObTabletPointer *tablet_ptr = nullptr;
...
int ObTabletMetaMemMgr::del_tablet(const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::compare_and_swap_tablet(
    const ObTabletMapKey &key,
...
void ObTabletMetaMemMgr::release_tablet_from_pool(ObTablet *tablet, const bool give_back_buf_into_pool)
{
  if (OB_NOT_NULL(tablet)) {
...
int ObTabletMetaMemMgr::has_tablet(const ObTabletMapKey &key, bool &is_exist)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::check_all_meta_mem_released(bool &is_released, const char *module)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObTabletMetaMemMgr::dump_tablet_info()
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::ObT3MResourceLimitCalculatorHandler::
    ...
    get_resource_constraint_value(share::ObResoureConstraintValue &constraint_value)
{
  ...
  // Calculate memory constraint: (server memory * config_mem_percentage) / 200MB * 20000.
...
int ObTabletMetaMemMgr::TabletMapDumpOperator::operator()(common::hash::HashMapPair<ObTabletMapKey, TabletValueStore *> &entry)
{
  int ret = OB_SUCCESS;
...
ObTabletMetaMemMgr::HeapCompare::HeapCompare(int &ret)
  : ret_(ret)
{
}

bool ObTabletMetaMemMgr::HeapCompare::operator() (
...
int ObTabletMetaMemMgr::get_wash_tablet_candidate(const std::type_info &type_info, CandidateTabletInfo &info)
{
  int ret = OB_SUCCESS;
  TabletBufferList *header = nullptr;
...
int ObTabletMetaMemMgr::do_wash_candidate_tablet(
...
int ObTabletMetaMemMgr::try_wash_tablet_from_gc_queue(
    const int64_t buf_len,
...
int ObTabletMetaMemMgr::update_tablet_buffer_header(ObTablet *old_obj, ObTablet *new_obj)
{
  int ret = OB_SUCCESS;
...
int ObTabletMetaMemMgr::try_wash_tablet(const std::type_info &type_info, void *&free_obj)
{
  int ret = OB_SUCCESS;
...
ObT3mTabletMapIterator::ObT3mTabletMapIterator(ObTabletMetaMemMgr &t3m)
  : tablet_map_(t3m.tablet_map_),
    tablet_items_(),
    idx_(0)
{
  tablet_items_.set_attr(::oceanbase::lib::ObMemAttr("TabletItems"));
}
...
{
  int ret = OB_SUCCESS;
  ObMemAttr attr = ObMemAttr("TabletIterSE");
...
ObTabletIterator::ObTabletIterator(
    ObTabletMetaMemMgr &t3m,
    common::ObArenaAllocator &allocator,
    ObITabletFilterOp *op)
...
int ObTabletIterator::get_next_tablet(ObTabletHandle &handle)
{
  int ret = OB_SUCCESS;
  handle.reset();
...
ObTabletPtrWithInMemObjIterator::ObTabletPtrWithInMemObjIterator(ObTabletMetaMemMgr &t3m)
  : ObT3mTabletMapIterator(t3m)
{
}

int ObTabletPtrWithInMemObjIterator::get_next_tablet_pointer(
...
ObInMemoryTabletIterator::ObInMemoryTabletIterator(ObTabletMetaMemMgr &t3m)
  : ObT3mTabletMapIterator(t3m)
{
}

int ObInMemoryTabletIterator::get_next_tablet(ObTabletHandle &handle)
{
  int ret = OB_SUCCESS;
  if (0 == tablet_items_.count() && OB_FAIL(fetch_tablet_item())) {
    LOG_WARN("fail to fetch value store pointers", K(ret));
...
int ObTabletMetaMemMgr::get_full_tablets_info(
    const ObFullTabletCreator &creator,
    const char *name,
    common::ObIArray<ObTabletMetaMemStatus> &info) const
{
  int ret = OB_SUCCESS;
  ObTabletMetaMemStatus mem_status;
  if (OB_ISNULL(name)) {
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN, "invalid args", K(ret), KP(name));
  } else if (OB_UNLIKELY(STRLEN(name) >= ObTabletMetaMemStatus::STRING_LEN)) {
