...
void ObStorageMetaMemMgr::TabletGCTask::runTimerTask()
{
...
void ObStorageMetaMemMgr::TableGCTask::runTimerTask()
{
...
int ObStorageMetaMemMgr::TabletGCQueue::push(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(tablet)) {
    ret = OB_INVALID_ARGUMENT;
...
int ObStorageMetaMemMgr::server_module_new(ObStorageMetaMemMgr *&meta_mem_mgr)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::init_pool_arr()
{
...
void ObStorageMetaMemMgr::stop()
{
  ...
  // so t3m can't stop gc thread until check_all_meta_mem_released is true in ObStorageMetaMemMgr::wait()
}

void ObStorageMetaMemMgr::wait()
{
...
int ObStorageMetaMemMgr::print_old_chain(
...
int ObStorageMetaMemMgr::push_table_into_gc_queue(ObITable *table, const ObITable::TableType table_type)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::gc_tables_in_queue(bool &all_table_cleaned)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::batch_destroy_memtable_(memtable::ObMemtableSet *memtable_set)
{
...
void ObStorageMetaMemMgr::destroy_gc_memtable_set_(memtable::ObMemtableSet *&memtable_set)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::prepare_gc_memtable_set_(memtable::ObMemtableSet *&memtable_set)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::batch_gc_memtable_set_(
...
void ObStorageMetaMemMgr::batch_gc_memtable_()
{
...
int ObStorageMetaMemMgr::push_memtable_into_gc_set_(memtable::ObMemtable *memtable)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::inner_push_tablet_into_gc_queue(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::push_tablet_into_gc_queue(ObTablet *tablet)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObStorageMetaMemMgr::gc_tablets_in_queue(bool &all_tablet_cleaned)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::has_meta_wait_gc(bool &is_wait)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
void *ObStorageMetaMemMgr::release_tablet(ObTablet *tablet, const bool return_buf_ptr_after_release)
{
...
int ObStorageMetaMemMgr::get_min_end_scn_for_ls(
    const ObTabletMapKey &key,
...
int ObStorageMetaMemMgr::get_min_end_scn_from_single_tablet(ObTablet *tablet,
                                                           const bool is_old,
                                                           const SCN &ls_checkpoint,
                                                           SCN &min_end_scn)
{
...
int ObStorageMetaMemMgr::scan_all_version_tablets(const ObTabletMapKey &key, const ObFunction<int(ObTablet &)> &op)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::release_ddl_kv(ObDDLKV *ddl_kv)
{
  if (OB_NOT_NULL(ddl_kv)) {
    if (0 != ddl_kv->get_ref()) {
...
int ObStorageMetaMemMgr::acquire_data_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::acquire_tx_data_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::acquire_tx_ctx_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::acquire_lock_memtable(ObTableHandleV2 &handle)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::release_memtable(ObMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
...
void ObStorageMetaMemMgr::release_tx_data_memtable_(ObTxDataMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
int ObStorageMetaMemMgr::acquire_tablet_ddl_kv_mgr(ObDDLKvMgrHandle &handle)
{
  int ret = OB_SUCCESS;
...
void ObStorageMetaMemMgr::release_tablet_ddl_kv_mgr(ObTabletDDLKvMgr *ddl_kv_mgr)
{
...
void ObStorageMetaMemMgr::release_tx_ctx_memtable_(ObTxCtxMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
void ObStorageMetaMemMgr::release_lock_memtable_(ObLockMemtable *memtable)
{
  if (OB_NOT_NULL(memtable)) {
    if (0 != memtable->get_ref()) {
...
void ObStorageMetaMemMgr::mark_mds_table_deleted_(const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
...
int ObStorageMetaMemMgr::release_memtable_and_mds_table_for_ls_offline(const ObTabletMapKey &key)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
  if (OB_FAIL(tablet_map_.get(key, ptr_handle))) {
...
int ObStorageMetaMemMgr::create_tmp_tablet(
...
int ObStorageMetaMemMgr::acquire_tablet_from_pool(
...
int ObStorageMetaMemMgr::acquire_tablet(
    const ObTabletPoolType type,
    ObTabletHandle &tablet_handle)
{
  int ret = OB_SUCCESS;
  ...
  if (ObTabletPoolType::TP_NORMAL == type) {
    meta_obj.pool_ = static_cast<ObIStorageMetaObjPool *>(&tablet_buffer_pool_);
    header = &normal_tablet_header_;
  } else if (ObTabletPoolType::TP_LARGE == type) {
    meta_obj.pool_ = static_cast<ObIStorageMetaObjPool *>(&large_tablet_buffer_pool_);
...
int ObStorageMetaMemMgr::acquire_tablet(ObIStorageMetaObjPool *pool, ObTablet *&tablet)
{
  int ret = OB_SUCCESS;
  TabletBufferList *header = nullptr;
  void *buf = nullptr;
...
int ObStorageMetaMemMgr::acquire_tmp_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    common::ObArenaAllocator &allocator,
    ObTabletHandle &tablet_handle)
...
int ObStorageMetaMemMgr::create_msd_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    ObLS *tenant_ls,
    ObTabletHandle &tablet_handle)
...
int ObStorageMetaMemMgr::create_tablet(
    const ObTabletMapKey &key,
    ObLS *tenant_ls,
    ObTabletHandle &tablet_handle)
{
...
int ObStorageMetaMemMgr::get_tablet(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    ObTabletHandle &handle)
{
...
int ObStorageMetaMemMgr::get_tablet_with_filter(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
...
int ObStorageMetaMemMgr::get_tablet_with_allocator(
    const WashTabletPriority &priority,
    const ObTabletMapKey &key,
    common::ObArenaAllocator &allocator,
    ObTabletHandle &handle,
...
int ObStorageMetaMemMgr::build_tablet_handle_for_mds_scan(
    ObTablet *tablet,
    ObTabletHandle &handle)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::get_tablet_buffer_infos(ObIArray<ObTabletBufferInfo> &buffer_infos)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObStorageMetaMemMgr::fill_buffer_infos(
...
int ObStorageMetaMemMgr::get_tablet_addr(const ObTabletMapKey &key, ObMetaDiskAddr &addr)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObStorageMetaMemMgr::get_tablet_pointer_initial_state(const ObTabletMapKey &key, bool &initial_state)
{
  int ret = OB_SUCCESS;
  ObTabletPointerHandle ptr_handle(tablet_map_);
  if (OB_UNLIKELY(!is_inited_)) {
...
int ObStorageMetaMemMgr::get_tablet_resident_info(
    const ObTabletMapKey &key,
...
int ObStorageMetaMemMgr::get_tablet_ddl_kv_mgr(
    const ObTabletMapKey &key,
...
int ObStorageMetaMemMgr::get_meta_mem_status(common::ObIArray<ObStorageMetaMemStatus> &info) const
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::del_tablet(const ObTabletMapKey &key)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    } else if (OB_FAIL(tablet_map_.erase(key, handle))) {
    } else {
...
int ObStorageMetaMemMgr::compare_and_swap_tablet(
    const ObTabletMapKey &key,
    ...
    const bool set_pool /* whether to set tablet pool */)
{
  ...
  if (OB_UNLIKELY(!is_inited_)) {
    ...
    ObIStorageMetaObjPool *pool = nullptr;
    ObBucketHashWLockGuard lock_guard(bucket_lock_, key.hash());
    if (OB_FAIL(has_tablet(key, is_exist))) {
    } else if (OB_UNLIKELY(!is_exist)) {
      ret = OB_ENTRY_NOT_EXIST;
...
void ObStorageMetaMemMgr::release_tablet_from_pool(ObTablet *tablet, const bool give_back_buf_into_pool)
{
  if (OB_NOT_NULL(tablet)) {
...
int ObStorageMetaMemMgr::has_tablet(const ObTabletMapKey &key, bool &is_exist)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::check_all_meta_mem_released(bool &is_released, const char *module)
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!is_inited_)) {
    ret = OB_NOT_INIT;
...
int ObStorageMetaMemMgr::dump_tablet_info()
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::ObT3MResourceLimitCalculatorHandler::
...
int ObStorageMetaMemMgr::TabletMapDumpOperator::operator()(common::hash::HashMapPair<ObTabletMapKey, TabletValueStore *> &entry)
{
  int ret = OB_SUCCESS;
...
ObStorageMetaMemMgr::HeapCompare::HeapCompare(int &ret)
  : ret_(ret)
{
}

bool ObStorageMetaMemMgr::HeapCompare::operator() (
...
int ObStorageMetaMemMgr::get_wash_tablet_candidate(const std::type_info &type_info, CandidateTabletInfo &info)
{
  int ret = OB_SUCCESS;
  TabletBufferList *header = nullptr;
...
int ObStorageMetaMemMgr::do_wash_candidate_tablet(
...
int ObStorageMetaMemMgr::try_wash_tablet_from_gc_queue(
    const int64_t buf_len,
...
int ObStorageMetaMemMgr::update_tablet_buffer_header(ObTablet *old_obj, ObTablet *new_obj)
{
  int ret = OB_SUCCESS;
...
int ObStorageMetaMemMgr::try_wash_tablet(const std::type_info &type_info, void *&free_obj)
{
  int ret = OB_SUCCESS;
...
ObT3mTabletMapIterator::ObT3mTabletMapIterator(ObStorageMetaMemMgr &t3m)
  : tablet_map_(t3m.tablet_map_),
    tablet_items_(),
    idx_(0)
{
  tablet_items_.set_attr(lib::ObMemAttr("TabletItems"));
}
...
ObTabletPtrWithInMemObjIterator::ObTabletPtrWithInMemObjIterator(ObStorageMetaMemMgr &t3m)
  : ObT3mTabletMapIterator(t3m)
{
}
...
ObInMemoryTabletIterator::ObInMemoryTabletIterator(ObStorageMetaMemMgr &t3m)
  : ObT3mTabletMapIterator(t3m)
{
}
...
int ObStorageMetaMemMgr::get_full_tablets_info(
    const ObFullTabletCreator &creator,
    const char *name,
    common::ObIArray<ObStorageMetaMemStatus> &info) const
{
  int ret = OB_SUCCESS;
  ObStorageMetaMemStatus mem_status;
  if (OB_ISNULL(name)) {
    ret = OB_INVALID_ARGUMENT;
    STORAGE_LOG(WARN, "invalid args", K(ret), KP(name));
  } else if (OB_UNLIKELY(STRLEN(name) >= ObStorageMetaMemStatus::STRING_LEN)) {
