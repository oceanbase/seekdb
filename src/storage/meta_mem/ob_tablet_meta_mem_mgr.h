...
#ifndef OCEANBASE_STORAGE_OB_TABLET_META_MEM_MGR_H_
#define OCEANBASE_STORAGE_OB_TABLET_META_MEM_MGR_H_
...
#include "storage/meta_mem/ob_tablet_pointer.h"
#include "storage/meta_mem/ob_meta_obj_pool.h"
...
struct ObTabletMetaMemStatus final
{
public:
  ObTabletMetaMemStatus();
public:
...
class ObTabletMetaMemMgr final
{
public:
...
private:
  explicit ObTabletMetaMemMgr();
public:
  ~ObTabletMetaMemMgr();
  static int module_new(ObTabletMetaMemMgr *&meta_mem_mgr);
  ...
  int get_meta_mem_status(common::ObIArray<ObTabletMetaMemStatus> &info) const;
...
public:
  ...
  {
  public:
    explicit ObT3MResourceLimitCalculatorHandler(ObTabletMetaMemMgr &t3m) : t3m_(t3m) {}
...
private:
  ...
  {
  public:
    explicit TabletGCTask(ObTabletMetaMemMgr *t3m) : t3m_(t3m) {}
  ...
  {
  public:
    explicit TableGCTask(ObTabletMetaMemMgr *t3m) : t3m_(t3m)
    {
...
private:
  ...
  int acquire_tablet(ObIMetaObjPool *pool, ObTablet *&tablet);
  ...
  int get_obj_pool_info(
      const ObMetaObjPool<T> &obj_pool,
      const char *name,
...
private:
  ...
  ObTabletTryWashFunc wash_func_;
  ...
  ObMetaObjPool<memtable::ObMemtable> memtable_pool_;
  ObMetaObjPool<ObNormalTabletBuffer> tablet_buffer_pool_;
  ObMetaObjPool<ObLargeTabletBuffer> large_tablet_buffer_pool_;
  ObMetaObjPool<ObDDLKV> ddl_kv_pool_;
  ObMetaObjPool<ObTabletDDLKvMgr> tablet_ddl_kv_mgr_pool_;
  ObMetaObjPool<ObTxDataMemtable> tx_data_memtable_pool_;
  ObMetaObjPool<ObTxCtxMemtable> tx_ctx_memtable_pool_;
  ObMetaObjPool<transaction::tablelock::ObLockMemtable> lock_memtable_pool_;
  ObIMetaObjPool *pool_arr_[ObITable::TableType::MAX_TABLE_TYPE];
...
{
  friend class ObTabletMetaMemMgr;
public:
  explicit ObT3mTabletMapIterator(ObTabletMetaMemMgr &t3m);
...
{
public:
  explicit ObInMemoryTabletIterator(ObTabletMetaMemMgr &t3m);
...
{
public:
  explicit ObTabletPtrWithInMemObjIterator(ObTabletMetaMemMgr &t3m);
...
template <typename T>
int ObTabletMetaMemMgr::get_obj_pool_info(
    const ObMetaObjPool<T> &obj_pool,
    const char *name,
    common::ObIArray<ObTabletMetaMemStatus> &info) const
{
...
#endif /* OCEANBASE_STORAGE_OB_TABLET_META_MEM_MGR_H_ */
