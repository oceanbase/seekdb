...
{
public:
  explicit TryWashTabletFunc(ObStorageMetaMemMgr &t3m);
  ~TryWashTabletFunc();
...
{
public:
  ObIStorageMetaObjPool() = default;
  virtual ~ObIStorageMetaObjPool() = default;
...
template <class T>
void ObStorageMetaObjPool<T>::free_obj(void *obj)
{
  release(static_cast<T *>(obj));
}

template <class T>
int ObStorageMetaObjPool<T>::alloc_obj(void *&obj)
{
  int ret = OB_SUCCESS;
  T *t = nullptr;
...
template <class T>
int ObStorageMetaObjPool<T>::acquire(T *&t)
{
  int ret = OB_SUCCESS;
  t = nullptr;
...
template <class T>
void ObStorageMetaObjPool<T>::release(T *t)
{
  BasePool::free(t);
  (void)ATOMIC_AAF(&used_obj_cnt_, -1);
}
...
template <class T>
void ObStorageMetaObjPool<T>::free_node_(typename BasePool::Node *ptr)
{
  if (NULL != ptr) {
    ptr->data.reset();
    ptr->next = NULL;
...
#endif /* OCEANBASE_STORAGE_OB_STORAGE_META_OBJ_POOL_H_ */
