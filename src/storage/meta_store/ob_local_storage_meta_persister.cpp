...
#include "ob_local_storage_meta_persister.h"
...
int ObLocalStorageMetaPersister::init(
    ...
    ObLocalStorageCheckpointSlogHandler &ckpt_slog_handler)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ret = OB_INIT_TWICE;
    LOG_WARN("ObLocalStorageMetaPersister has inited", K(ret));
  } else {
...
void ObLocalStorageMetaPersister::destroy()
{
  if (IS_INIT) {
    slogger_ = nullptr;
...
int ObLocalStorageMetaPersister::batch_update_tablet(const ObIArray<ObUpdateTabletLog> &slog_arr)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObLocalStorageMetaPersister::update_tablet(
...
int ObLocalStorageMetaPersister::write_empty_shell_tablet(ObTablet *tablet, ObMetaDiskAddr &tablet_addr)
{
  int ret = OB_SUCCESS;
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
...
int ObLocalStorageMetaPersister::remove_tablet(
...
int ObLocalStorageMetaPersister::remove_tablets(
...
int ObLocalStorageMetaPersister::write_update_tablet_slog_(
    const common::ObTabletID &tablet_id,
...
int ObLocalStorageMetaPersister::write_remove_tablet_slog_(
    const common::ObTabletID &tablet_id)
{
  int ret = OB_SUCCESS;
...
int ObLocalStorageMetaPersister::write_remove_tablets_slog_(
...
int ObLocalStorageMetaPersister::safe_batch_write_remove_tablets_slog_(
    const common::ObIArray<ObTabletID> &tablet_ids)
{
  int ret = OB_SUCCESS;
  const int64_t tablet_count = tablet_ids.count();
  const int32_t cmd = ObIRedoModule::gen_cmd(ObRedoLogMainType::OB_REDO_LOG_LOCAL_STORAGE,
      ObRedoLogSubType::OB_REDO_LOG_DELETE_TABLET);
