...
LogSlidingWindow::LogSlidingWindow()
  : self_(),
    ...
    match_lsn_info_lock_(common::ObLatchIds::PALF_SW_MATCH_LSN_MAP_LOCK),
    match_lsn_info_(),
    last_truncate_lsn_(),
...
int LogSlidingWindow::after_flush_log(const FlushLogCbCtx &flush_cb_ctx)
{
  ...
  if (OB_SUCC(ret) && can_exec_cb) {
    ...
    if (LEADER == state_mgr_->get_role()) {
      // Leader updates local match lsn.
      (void) try_update_match_lsn_info_(self_, log_end_lsn);
    }
    time_guard.click();
...
int LogSlidingWindow::to_follower_pending(LSN &last_lsn)
{
  ...
  } else if (OB_FAIL(reset_match_lsn_info_())) {
    PALF_LOG(WARN, "reset_match_lsn_info_ failed", K(ret), K_(palf_id), K_(self));
  } else {
    reconfirm_fetch_dest_.reset();
...
int LogSlidingWindow::to_leader_active()
{
  ...
  if (IS_NOT_INIT) {
    ...
    // Reset local match lsn after reconfirm.
    PALF_LOG(WARN, "reset_match_lsn_info_ failed", K(ret), K_(palf_id), K_(self));
  } else {
    reconfirm_fetch_dest_.reset();
...
int LogSlidingWindow::get_server_ack_info(const common::ObAddr &server, LsnTsInfo &ack_info) const
{
  ...
  } else if (OB_UNLIKELY(server != self_)) {
    ret = OB_ENTRY_NOT_EXIST;
  } else {
    ObSpinLockGuard guard(match_lsn_info_lock_);
    if (OB_UNLIKELY(!match_lsn_info_.is_valid())) {
      ret = OB_ENTRY_NOT_EXIST;
    } else {
      ack_info = match_lsn_info_;
    }
  }
  return ret;
}
...
int LogSlidingWindow::get_ack_info_array(LogMemberAckInfoList &ack_info_array) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < member_list.get_member_number(); ++i) {
      ...
      } else if (OB_UNLIKELY(tmp_server.get_server() != self_)) {
        ret = OB_ERR_UNEXPECTED;
        PALF_LOG(WARN, "unexpected non-local log sync member", K(ret), K_(palf_id), K_(self), K(tmp_server));
      } else if (OB_UNLIKELY(!match_lsn_info_.is_valid())) {
        ret = OB_ERR_UNEXPECTED;
        PALF_LOG(WARN, "match lsn info is invalid", K(ret), K_(palf_id), K_(self), K_(match_lsn_info));
      } else {
        LogMemberAckInfo ack_info;
        ack_info.member_ = tmp_server;
        ack_info.last_ack_time_us_ = match_lsn_info_.last_ack_time_us_;
        ack_info.last_flushed_end_lsn_ = match_lsn_info_.lsn_;
        ack_info_array.push_back(ack_info);
        PALF_LOG(TRACE, "push ack info success", K(ack_info));
      }
    }
  }
  return ret;
...
int LogSlidingWindow::get_lagged_member_list(const LSN &dst_lsn, ObMemberList &lagged_list)
{
  ...
  if (!dst_lsn.is_valid()) {
    ...
    ObSpinLockGuard guard(match_lsn_info_lock_);
    if (match_lsn_info_.is_valid() && match_lsn_info_.lsn_ < dst_lsn &&
        OB_FAIL(lagged_list.add_server(self_))) {
      PALF_LOG(WARN, "add self to lagged_list failed", K(ret), K_(palf_id), K_(self), K(dst_lsn), K_(match_lsn_info));
    } else {
                                        ...
                                        LSN &result_lsn) const
{
  ...
  ObAddr tmp_server;
  if (OB_UNLIKELY(replica_num != 1 || member_list.get_member_number() != 1)) {
    ret = OB_ERR_UNEXPECTED;
    PALF_LOG(WARN, "unexpected multi-member log sync config", K(ret), K_(palf_id), K_(self),
        K(member_list), K(replica_num));
  } else if (OB_FAIL(member_list.get_server_by_index(0, tmp_server))) {
    PALF_LOG(WARN, "get_server_by_index failed", K(ret), K_(palf_id), K_(self), K(member_list));
  } else if (OB_UNLIKELY(tmp_server != self_)) {
    ret = OB_ERR_UNEXPECTED;
    PALF_LOG(WARN, "unexpected non-local log sync member", K(ret), K_(palf_id), K_(self), K(tmp_server));
  } else {
    ObSpinLockGuard guard(match_lsn_info_lock_);
    if (OB_UNLIKELY(!match_lsn_info_.is_valid())) {
      ...
      result_lsn = match_lsn_info_.lsn_;
      PALF_LOG(TRACE, "current matched lsn", K_(palf_id), K_(self), K(result_lsn));
    }
  }
  return ret;
}
...
int LogSlidingWindow::reset_match_lsn_info_()
{
  int ret = OB_SUCCESS;
  LSN max_flushed_end_lsn;
  const int64_t now_us = ObTimeUtility::current_time();
  {
    ObSpinLockGuard guard(match_lsn_info_lock_);
    get_max_flushed_end_lsn(max_flushed_end_lsn);
    match_lsn_info_ = LsnTsInfo(max_flushed_end_lsn, now_us);
  }
  PALF_LOG(INFO, "reset_match_lsn_info_ finished", K(ret), K_(palf_id), K_(self), K(max_flushed_end_lsn));
  return ret;
}

// need caller hold wlock in PalfHandleImpl
int LogSlidingWindow::config_change_update_match_lsn_info(
    ...
    const int64_t new_replica_num)
{
  ...
  // Try to advance committed_end_lsn after updating local match lsn.
  if (OB_SUCC(ret)) {
    if (state_mgr_->is_leader_active()) {
  ...
  PALF_LOG(INFO, "config_change_update_match_lsn_info_ finished", K(ret), K_(palf_id), K_(self), K(added_memberlist), K(removed_memberlist));
  return ret;
}

int LogSlidingWindow::try_update_match_lsn_info_(const common::ObAddr &server, const LSN &end_lsn)
{
  int ret = OB_SUCCESS;
  if (!server.is_valid() || !end_lsn.is_valid()) {
    ...
    ret = OB_ENTRY_NOT_EXIST;
    PALF_LOG(WARN, "unexpected non-local match lsn update", K(ret), K_(palf_id), K_(self), K(server), K(end_lsn));
  } else {
    const int64_t now_us = ObTimeUtility::current_time();
    ObSpinLockGuard guard(match_lsn_info_lock_);
    if (OB_UNLIKELY(!match_lsn_info_.is_valid())) {
      match_lsn_info_ = LsnTsInfo(end_lsn, now_us);
    } else if (match_lsn_info_.lsn_ <= end_lsn) {
      const LSN old_end_lsn = match_lsn_info_.lsn_;
      const int64_t old_advance_time_us = match_lsn_info_.last_advance_time_us_;
      if (match_lsn_info_.lsn_ < end_lsn) {
        match_lsn_info_.last_advance_time_us_ = now_us;
        match_lsn_info_.last_ack_time_us_ = now_us;
      }
      match_lsn_info_.lsn_ = end_lsn;
      if (old_end_lsn < end_lsn &&
          now_us - old_advance_time_us > MATCH_LSN_ADVANCE_DELAY_THRESHOLD_US) {
        PALF_LOG(WARN, "[MATCH LSN ADVANCE DELAY]match_lsn advance delay too much time",
            K(ret), K_(palf_id), K_(self), K(server), K(end_lsn),
            "old_end_lsn", old_end_lsn, "old_advance_time_us", old_advance_time_us,
            "new_ack_time_us", now_us, "advance delay(us)", now_us - old_advance_time_us);
      }
    }
  }
  PALF_LOG(TRACE, "try_update_match_lsn_info_ finished", K(ret), K_(palf_id), K_(self), K(server), K(end_lsn));
  return ret;
}
...
int LogSlidingWindow::ack_log(const common::ObAddr &src_server, const LSN &end_lsn)
{
  ...
  if (IS_NOT_INIT) {
    ...
    // Why record end_lsn of degraded learners?
  ...
  } else if (OB_FAIL(try_update_match_lsn_info_(src_server, end_lsn))) {
    PALF_LOG(WARN, "try_update_match_lsn_info_ failed", K(ret), K_(palf_id), K_(self), K(src_server), K(end_lsn));
  } else {
    LSN old_committed_end_lsn;
    get_committed_end_lsn_(old_committed_end_lsn);
    LSN new_committed_end_lsn;
