...
PalfLogBuffer::PalfLogBuffer()
  : allocation_(NULL), data_(NULL), size_(0), capacity_(0), prefix_size_(0), is_sealed_(false)
{}
...
int PalfLogBuffer::init(const int64_t capacity, const int64_t prefix_size)
{
  ...
  if (NULL != allocation_) {
    ...
    ret = OB_INVALID_ARGUMENT;
                 ...
                 server_malloc(alloc_size, "PalfLogBuffer")))) {
                                   ...
                                   const int64_t valid_size)
{
  ...
  if (NULL == allocation_ || is_sealed_ || new_capacity <= capacity_
    ...
    ret = OB_INVALID_ARGUMENT;
                 ...
                 server_malloc(new_capacity + prefix_size_, "PalfLogBuffer")))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
  } else {
...
LogPendingBufferLimiter::LogPendingBufferLimiter(const int64_t limit)
  : state_(alloc_state_(limit))
{}
...
void LogPendingBufferLimiter::reset(const int64_t limit)
{
  release_state_ref_(state_);
  state_ = alloc_state_(limit);
  if (NULL == state_) {
    const int ret = OB_ALLOCATE_MEMORY_FAILED;
...
LogBufferSegment::LogBufferSegment()
  : begin_lsn_(), owner_(), entry_size_(0), valid_size_(0), is_padding_(false),
    data_checksum_(0), limiter_state_(NULL),
    reserved_bytes_(0), next_(NULL)
{}
...
LogGroupWriteBuf::LogGroupWriteBuf()
  : head_(NULL), tail_(NULL), data_len_(0), is_inited_(false)
{
