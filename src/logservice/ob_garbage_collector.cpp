...
void ObGCHandler::try_check_and_set_wait_gc_(ObGarbageCollector::LSStatus &ls_status)
{
  ...
  if (OB_FAIL(ls_->get_gc_state(gc_state))) {
  ...
  } else if (OB_FAIL(try_check_and_set_wait_gc_when_log_archive_is_off_(gc_state, readable_scn, offline_scn, ls_status))) {
    ...
    CLOG_LOG(INFO, "try_check_and_set_wait_gc_when_log_archive_is_off_ success", K(ls_id), K(gc_state), K(offline_scn));
  }
}
