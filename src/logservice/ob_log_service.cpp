...
int ObLogService::check_need_do_checkpoint(bool &need_do_checkpoint)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_ISNULL(ls_service)) {
      ...
      auto get_unrecycable_log_disk_size = [&](ObLS &ls) -> int {
      ...
      if (OB_FAIL(ls_service->foreach_ls(get_unrecycable_log_disk_size, ObLSGetMod::LOG_MOD))) {
        CLOG_LOG(WARN, "foreach ls failed", KP(ls_service));
      } else {
