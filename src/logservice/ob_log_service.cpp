...
int ObLogService::attach_log_io_callback_background_executor(
    share::ObBackgroundTaskExecutor *background_executor)
{
  ...
  if (OB_ISNULL(palf_env_)) {
    ret = OB_NOT_INIT;
      ...
      palf_env_->attach_log_io_callback_background_executor(
          background_executor))) {
    CLOG_LOG(WARN, "attach log io callback background source failed",
        K(ret));
  }
  return ret;
}

int ObLogService::detach_log_io_callback_background_executor()
{
  int ret = OB_SUCCESS;
  if (OB_NOT_NULL(palf_env_)
      && OB_FAIL(
          palf_env_->detach_log_io_callback_background_executor())) {
    CLOG_LOG(WARN, "detach log io callback background source failed",
        K(ret));
  }
  return ret;
}
