...
int LSFetchCtx::init_group_iterator_(const palf::LSN &start_lsn)
{
  ...
  if (OB_UNLIKELY(! start_lsn.is_valid())) {
    ...
    // get_file_end_lsn should return the actual data end in MemoryStorage, not LOG_MAX_LSN_VAL
    // This ensures IteratorStorage::pread correctly limits read size to actual available data
    palf::GetFileEndLSN group_iter_end_func = [&](){ return mem_storage_.get_log_tail(); };
