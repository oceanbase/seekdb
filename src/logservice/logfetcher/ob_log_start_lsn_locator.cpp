...
int64_t ObLogStartLSNLocator::g_batch_count =
    ObLogFetcherConfig::default_start_lsn_locator_batch_count;
int64_t ObLogStartLSNLocator::g_rpc_timeout =
    ObLogFetcherConfig::default_start_lsn_locator_rpc_timeout_sec * _SEC_;
...
void ObLogStartLSNLocator::configure(const ObLogFetcherConfig &config)
{
    ...
    if (OB_SUCCESS == rpc_err && OB_SUCCESS == svr_err && OB_SUCCESS == new_ls_err) {
      ...
      if (! cur_max_start_lsn_.is_valid()) {
        ...
        if (OB_UNLIKELY(! is_consistent)) {
          ...
          if (OB_ISNULL(cfg_)) {
            ret = OB_ERR_UNEXPECTED;
            LOG_ERROR("cfg_ is nullptr", K(cfg_));
          } else if (cfg_->skip_start_lsn_locator_result_consistent_check) {
