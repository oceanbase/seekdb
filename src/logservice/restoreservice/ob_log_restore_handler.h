...
// When creating network standby tenant, need to wait for tenant to become NORMAL state before returning success to user.
// Tenant becoming Normal state requires system log stream to be readable, but during this waiting process, log stream synchronization
// may encounter some errors, these errors will cause inability to continue fetching logs, so when encountering these errors
// directly return failure to client, no need to wait for timeout
// These states are all considered as states that can continue waiting for tenant to become NORMAL
