...
int ObUpdateAutoincSequenceTask::process()
{
  ...
  MANAGEMENT_EVENT_ADD("ddl scheduler", "update autoinc sequence task process",
    "ret", ret,
    K_(trace_id),
    K_(task_id),
...
int ObModifyAutoincTask::deserialize_params_from_message(const char *buf, const int64_t data_len, int64_t &pos)
{
  int ret = OB_SUCCESS;
