  ...
  if (OB_UNLIKELY(OB_INVALID_ID == data_table_id || OB_INVALID_ID == dest_table_id
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCC(ret)) {
        ...
        // FTS next-stage optimization (reference d325f95): FTS auxiliary
        // rowkeys are supported by the order-preserving encoder. Materialize
        // the collation key once per row instead of recomputing UTF-8 weights
        // in each sort comparison; preserve the old DDL behavior elsewhere.
