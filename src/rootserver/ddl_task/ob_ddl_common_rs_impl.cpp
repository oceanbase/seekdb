  ...
  if (OB_UNLIKELY(OB_INVALID_ID == data_table_id || OB_INVALID_ID == dest_table_id
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCC(ret)) {
        ...
        // FTS auxiliary tables sort a large number of repeated variable-length
        // tokens.  Their rowkey types are supported by the order-preserving
        // encoder, so materialize the collation key once per row instead of
        // recomputing UTF-8 weights in every comparison.  Keep the historical
        // setting for all other online DDL paths.
