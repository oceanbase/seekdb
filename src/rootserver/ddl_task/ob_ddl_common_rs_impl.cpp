  ...
  if (OB_UNLIKELY(OB_INVALID_ID == data_table_id || OB_INVALID_ID == dest_table_id
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCC(ret)) {
        ...
        // The FTS auxiliary tables sort large token streams during backfill.
        // Use the vectorized sorter for those two internal builds while keeping
        // the established DDL behavior for every other index type.
