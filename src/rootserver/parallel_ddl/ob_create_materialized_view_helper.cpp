...
int ObCreateMaterializedViewHelper::append_generate_schema_for_mv_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    new_view_schema_->get_view_schema().set_mv_refresh_info(&(arg_.mv_ainfo_.at(0).mv_refresh_info_));
  }
  return ret;
}
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < arg_.mv_ainfo_.count(); i ++) {
      ...
      if (OB_SUCC(ret)) { // check same table_name
        ...
        if (OB_FAIL(schema_guard_wrapper_.get_table_id(container_table_schema.get_database_id(),
...
int ObCreateMaterializedViewHelper::create_schemas_for_mv_()
{
  ...
  if (OB_FAIL(check_inner_stat_())) {
    ...
    const ObMVRefreshInfo *mv_refresh_info = new_view_schema_->get_view_schema().get_mv_refresh_info();
    if (OB_ISNULL(mv_refresh_info)) {
      ret = OB_ERR_UNEXPECTED;
