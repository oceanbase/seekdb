...
#include "ob_context_allocator.h"
...
ObContextAllocatorV2::ObContextAllocatorV2(uint64_t ctx_id,
...
ObContextAllocatorV2::~ObContextAllocatorV2()
{
  allocator_ = NULL;
}

void ObContextAllocatorV2::set_req_chunkmgr_parallel(int32_t parallel)
{
...
void *ObContextAllocator::alloc(const int64_t size, const ObMemAttr &attr)
{
  return common_realloc(NULL, size, attr, *this, obj_mgr_);
}

int64_t ObContextAllocator::get_obj_hold(void *ptr)
{
...
void* ObContextAllocator::realloc(const void *ptr, const int64_t size, const ObMemAttr &attr)
{
  return common_realloc(ptr, size, attr, *this, obj_mgr_);
}

void ObContextAllocator::free(void *ptr)
{
  common_free(ptr);
}
int ObContextAllocatorV2::iter_label(VisitFunc func) const
{
  int ret = OB_SUCCESS;
  struct ItemWrapper
  {
  ...
  if (OB_UNLIKELY(!mem_dump.is_inited())) {
    ...
    SMART_VAR(ItemWrapper[item_cap], items) {
      ...
      auto it = std::lower_bound(tcrs, tcrs + tcr_cnt, ctx_id_,
                                  &ObMemoryDump::ContextRange::compare);
      if (ObCtxIds::KVSTORE_CACHE_ID == ctx_id_) {
        ...
        IGNORE_RETURN with_resource_handle_invoke([&](const ObServerMemoryManager *mgr) {
...
void ObContextAllocatorV2::print_usage() const
{
  int ret = OB_SUCCESS;
...
AChunk *ObContextAllocator::pop_chunk()
{
...
void ObContextAllocator::push_chunk(AChunk *chunk)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  lib::ObMutexGuard guard(chunk_freelist_mutex_);
...
AChunk *ObContextAllocator::alloc_chunk(const int64_t size, const ObMemAttr &attr)
{
  int ret = OB_SUCCESS;
...
void ObContextAllocator::free_chunk(AChunk *chunk, const ObMemAttr &attr)
{
  if (chunk != nullptr) {
    ObDisableDiagnoseGuard disable_diagnose_guard;
...
void ObContextAllocator::dec_hold(const int64_t size)
{
  ctx_allocator_.dec_hold(size);
}

void ObContextAllocatorV2::dec_hold(const int64_t size)
{
...
int ObContextAllocator::set_idle(const int64_t set_size, const bool reserve/*=false*/)
{
  int ret = OB_SUCCESS;
...
void ObContextAllocatorV2::get_chunks(AChunk **chunks, int cap, int &cnt)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  allocator_->get_chunks_(chunks, cap, cnt);
}

int64_t ObContextAllocatorV2::get_used() const
{
  int64_t used = 0;
...
ObLabelItem ObContextAllocatorV2::get_label_usage(ObLabel &label) const
{
  ObLabelItem item;
  item.reset();
  IGNORE_RETURN iter_label([&](ObLabel &label_, LabelItem *l_item)
...
void ObContextAllocatorV2::update_wash_stat(int64_t related_chunks, int64_t blocks, int64_t size)
{
...
void ObContextAllocator::on_alloc(AObject& obj, const ObMemAttr& attr, const bool light_backtrace_allowed)
{
...
void ObContextAllocator::on_free(AObject& obj, ABlock& block)
{
...
void ObContextAllocator::common_free(void *ptr)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  if (NULL != ptr) {
    AObject *obj = reinterpret_cast<AObject*>((char*)ptr - AOBJECT_HEADER_SIZE);
