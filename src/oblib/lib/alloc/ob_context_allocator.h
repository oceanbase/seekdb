...
#ifndef _OB_CONTEXT_ALLOCATOR_H_
#define _OB_CONTEXT_ALLOCATOR_H_
...
class ObContextAllocator;

class ObContextAllocatorV2 : private common::ObLink
{
public:
  friend class ObContextAllocator;
  friend class ObMallocAllocator;
  using VisitFunc = std::function<int(ObLabel &label, common::LabelItem *l_item)>;
  using InvokeFunc = std::function<int (const ObServerMemoryManager*)>;
  ObContextAllocatorV2(uint64_t ctx_id,
      ObContextAllocator *allocator);
  ~ObContextAllocatorV2();
  ObContextAllocatorV2 *&get_next()
  {
    return reinterpret_cast<ObContextAllocatorV2*&>(next_);
  }
  ...
  // statistic related
  int set_server_memory_manager()
  {
    int ret = common::OB_SUCCESS;
    if (resource_handle_.is_valid()) {
      ret = common::OB_INIT_TWICE;
      LIB_LOG(WARN, "resource_handle is already valid", K(ret), K_(ctx_id));
    } else if (OB_FAIL(ObResourceMgr::get_instance().get_server_resource_manager(
        resource_handle_))) {
      LIB_LOG(ERROR, "get_server_resource_manager failed", K(ret));
    }
    return ret;
  }
  ...
  int64_t get_limit() const
  {
    ...
    with_resource_handle_invoke([&ctx_id, &limit](const ObServerMemoryManager *mgr) {
  ...
  int64_t get_hold() const
  {
    int64_t hold = 0;
    uint64_t ctx_id = ctx_id_;
    with_resource_handle_invoke([&ctx_id, &hold](const ObServerMemoryManager *mgr) {
  ...
  {
    int64_t limit = 0;
    with_resource_handle_invoke([&limit](const ObServerMemoryManager *mgr) {
      limit = mgr->get_limit();
      return common::OB_SUCCESS;
    });
    return limit;
  ...
  int64_t get_memory_hold() const
  {
    int64_t hold = 0;
    with_resource_handle_invoke([&hold](const ObServerMemoryManager *mgr) {
...
private:
  ...
  ObContextAllocator *get_allocator()
  {
    return allocator_;
  }
  ObServerResourceManagerHandle &get_resource_handle() { return resource_handle_; }
  int64_t inc_ref_cnt(int64_t cnt) { return ATOMIC_FAA(&ref_cnt_, cnt); }
  int64_t get_ref_cnt() const { return ATOMIC_LOAD(&ref_cnt_); }
private:
  ObServerResourceManagerHandle resource_handle_;
  int64_t ref_cnt_;

  uint64_t ctx_id_;
  ObContextAllocator *allocator_;
...
{
friend class ObContextAllocatorGuard;
friend class ObMallocAllocator;
friend class ObContextAllocatorV2;

class ChunkMgr : public IChunkMgr
{
public:
  ChunkMgr(ObContextAllocator &ta) : ta_(ta) {}
...
public:
  ...
  ReqChunkMgr(ObContextAllocator &ta)
...
public:
  explicit ObContextAllocator(ObContextAllocatorV2 &ctx_allocator, uint64_t ctx_id)
  ...
  virtual ~ObContextAllocator()
  {}
  ObContextAllocatorV2 *get_ctx_allocator()
  {
  ...
  inline ObContextAllocator *&get_next()
  {
    return reinterpret_cast<ObContextAllocator*&>(next_);
  }
private:
  // will delete it
...
public:
  ...
  int64_t get_memory_limit() const { return ctx_allocator_.get_memory_limit(); }

  int64_t get_memory_hold() const { return ctx_allocator_.get_memory_hold(); }
...
public:
  ...
  static void* common_realloc(const void *ptr, const int64_t size, const ObMemAttr &attr,
      ObContextAllocator& ta, T &allocator)
  {
    ...
    if (NULL == nptr) {
      print_alloc_failed_msg(ta.get_ctx_id(),
                             ...
                             ta.get_memory_hold(), ta.get_memory_limit());
    }
    return nptr;
  }
...
private:
  ObContextAllocatorV2 &ctx_allocator_;
...
}; // end of class ObContextAllocator

} // end of namespace lib
} // end of namespace oceanbase

#endif /* _OB_CONTEXT_ALLOCATOR_H_ */
