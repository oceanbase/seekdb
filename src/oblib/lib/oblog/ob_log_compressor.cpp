...
{
  log_compressor_.run_timer_task();
}
...
int ObLogCompressor::init()
{
  ...
  } else if (0 != (sys_err = regcomp(&regex_uncompressed_, OB_UNCOMPRESSED_SYSLOG_FILE_PATTERN, REG_EXTENDED))) {
...
void ObLogCompressor::run_timer_task()
{
  ...
  if (!stopped_) {
    // wait until stoped or needing to work
    {
      ...
      if (!stopped_ && !is_enable_compress() && max_disk_size_ <= 0 && OB_LOGGER.get_max_file_index() <= 0) {
        idle = true;
      }
    }
