     ...
     int16_t *precision, int16_t *scale, const bool do_rounding, const bool catch_trunc_err)
{
  ...
  if (OB_SUCC(ret) && (has_digit || 0 < i_nth) 
    ...
    if (0 == i || i >= length - 1) {
      ...
      /* The legal number exponent range is [-130, 125],
       * so e_value's valid range cannot be larger than 3 digits. */
      int e_cnt = 0;
              ...
              const bool need_to_sci/*false*/) const
{
  ...
  if (OB_ISNULL(buf) || OB_UNLIKELY(max_need_size < 0)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      // Keep a leading zero for decimal output in MySQL-only mode.
      if (nmb->is_decimal()) {
        buf[pos++] = '0';
      }
...
int ObNumber::get_npi_(double n, ObNumber& out, ObIAllocator &alloc, const bool do_rounding) const
{
  ...
  (void)ob_gcvt_opt(n, OB_GCVT_ARG_DOUBLE, static_cast<int32_t>(sizeof(buf) - 1), buf, NULL, TRUE);
