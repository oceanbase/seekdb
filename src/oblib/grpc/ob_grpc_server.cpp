/*
 * Copyright (c) 2025 OceanBase.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#define USING_LOG_PREFIX LIB

#include "grpc/ob_grpc_server.h"
#include <memory>
#include <string>
#include <vector>
#include <grpcpp/grpcpp.h>
#include <grpcpp/support/interceptor.h>
#include <grpcpp/support/server_interceptor.h>
#include "grpc/ob_grpc_context.h"
#include "lib/oblog/ob_log.h"
#include "lib/oblog/ob_log_module.h"
#include "lib/profile/ob_trace_id.h"
#include "lib/utility/ob_macro_utils.h"

namespace oceanbase
{
namespace obgrpc
{
namespace
{

void extract_trace_id_from_metadata(std::multimap<grpc::string_ref, grpc::string_ref> *metadata)
{
  if (OB_ISNULL(metadata)) {
    return;
  }

  std::multimap<grpc::string_ref, grpc::string_ref>::iterator it = metadata->find("custom-header");
  if (it != metadata->end()) {
    std::string header(it->second.data(), it->second.length());
    const size_t comma_pos = header.find(',');
    if (comma_pos != std::string::npos && comma_pos + 1 < header.length()) {
      const std::string trace_id_str = header.substr(comma_pos + 1);
      common::ObCurTraceId::set(trace_id_str.c_str());
    }
  }
}

class ObGrpcTraceIdInterceptor : public grpc::experimental::Interceptor {
public:
  void Intercept(grpc::experimental::InterceptorBatchMethods *methods) override
  {
    if (methods->QueryInterceptionHookPoint(
            grpc::experimental::InterceptionHookPoints::POST_RECV_INITIAL_METADATA)) {
      extract_trace_id_from_metadata(methods->GetRecvInitialMetadata());
    }
    methods->Proceed();
    common::ObCurTraceId::reset();
  }
};

class ObGrpcTraceIdInterceptorFactory : public grpc::experimental::ServerInterceptorFactoryInterface {
public:
  grpc::experimental::Interceptor *CreateServerInterceptor(grpc::experimental::ServerRpcInfo *info) override
  {
    UNUSED(info);
    return new ObGrpcTraceIdInterceptor();
  }
};

std::vector<std::unique_ptr<grpc::experimental::ServerInterceptorFactoryInterface> >
create_server_interceptor_factories()
{
  std::vector<std::unique_ptr<grpc::experimental::ServerInterceptorFactoryInterface> > creators;
  creators.push_back(std::unique_ptr<grpc::experimental::ServerInterceptorFactoryInterface>(
      new ObGrpcTraceIdInterceptorFactory()));
  return creators;
}

} // namespace

ObGrpcServer::ObGrpcServer()
  : thread_(), server_(), services_(), cert_provider_()
{
}

void ObGrpcServer::register_service(grpc::Service *service)
{
  if (NULL != service) {
    services_.push_back(service);
  }
}

int ObGrpcServer::start(int port)
{
  int ret = OB_SUCCESS;
  if (NULL != server_.get()) {
    // already running
  } else if (services_.empty()) {
    ret = OB_NOT_INIT;
    LOG_WARN("no gRPC services registered, cannot start server", K(ret), K(port));
  } else {
    grpc::ServerBuilder builder;
    std::vector<std::unique_ptr<grpc::experimental::ServerInterceptorFactoryInterface> > interceptor_factories =
        create_server_interceptor_factories();
    builder.experimental().SetInterceptorCreators(std::move(interceptor_factories));
    for (int64_t i = 0; i < static_cast<int64_t>(services_.size()); ++i) {
      builder.RegisterService(services_[i]);
    }

    const std::string addr = std::string("0.0.0.0:") + std::to_string(port);
    builder.AddChannelArgument(GRPC_ARG_ALLOW_REUSEPORT, 0);
    const int MAX_MESSAGE_SIZE = 512 * 1024 * 1024;
    builder.SetMaxReceiveMessageSize(MAX_MESSAGE_SIZE);
    builder.SetMaxSendMessageSize(MAX_MESSAGE_SIZE);

    std::shared_ptr<grpc::ServerCredentials> creds;
    if (ob_grpc_is_rpc_tls_enabled()) {
      creds = create_server_credentials(cert_provider_);
      if (!creds) {
        ret = OB_INIT_FAIL;
        LOG_DBA_ERROR(OB_INIT_FAIL, "msg", "failed to create gRPC TLS server credentials", K(port));
      }
    } else {
      creds = grpc::InsecureServerCredentials();
    }
    if (OB_SUCC(ret)) {
      builder.AddListeningPort(addr, creds);
      server_ = builder.BuildAndStart();
      if (NULL == server_.get()) {
        ret = OB_ERROR;
        LOG_DBA_ERROR(OB_ERROR, "msg", "failed to build and start gRPC server", K(port));
      } else {
        thread_ = std::thread([this] {
          server_->Wait();
        });
        LOG_INFO("gRPC server started successfully", K(port));
      }
    }
  }
  return ret;
}

void ObGrpcServer::stop()
{
  if (NULL != server_.get()) {
    server_->Shutdown();
    if (thread_.joinable()) {
      thread_.join();
    }
    server_.reset();
    cert_provider_.reset();
    LOG_INFO("gRPC server stopped");
  }
}

void ObGrpcServer::wait()
{
  if (thread_.joinable()) {
    thread_.join();
  }
}

} // end namespace obgrpc
} // end namespace oceanbase
