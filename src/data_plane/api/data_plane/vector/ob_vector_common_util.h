...
struct ObCentersBuffer
{
  ...
  int init(const int64_t dim, const int64_t capacity, data_plane::ObIVectorMemory &ivf_build_mem_ctx);
  void destroy();
  ...
  int32_t total_cnt_;
  int64_t capacity_;
  data_plane::ObIVectorMemory *ivf_build_mem_ctx_;
...
public:
  ObVectorCenterClusterHelper(ObIAllocator &allocator, const VEC_T *const_vec, ObVectorDistanceType dis_type, int64_t dim, int64_t nprobe, float similarity_threshold)
 ...
 public:
  ...
  struct HeapCompare {
    ...
    explicit HeapCompare(ObVectorDistanceType dis_type) {
      if (dis_type == ObVectorDistanceType::DOT) {
        is_max_heap_ = false;
      } else {
        is_max_heap_ = true;
      }
...
template <typename T>
int ObCentersBuffer<T>::init(const int64_t dim, const int64_t capacity, data_plane::ObIVectorMemory &ivf_build_mem_ctx)
{
  int ret = OB_SUCCESS;
  if (0 >= dim || 0 >= capacity) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (OB_ISNULL(vectors_ = static_cast<T*>(ivf_build_mem_ctx_->allocate(total_size)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      SHARE_LOG(WARN, "failed to alloc vectors memory", K(ret), K(total_size), K(ivf_build_mem_ctx_->allocated_bytes()));
    } else {
  ...
  VEC_T *center_vec /*= nullptr*/)
{
  ...
  if (heap_.count() < nprobe_) {
    ...
    if (NULL == ptr) {
      ...
      OB_ASSERT(center_with_buf != nullptr);
...
bool ObVectorCenterClusterHelper<VEC_T, CENTER_T>::is_satify_similarity_threshold(const double& distance)
{
  ...
  if (similarity_threshold_ != 0.0 && OB_FAIL(vector_similarity_from_distance(dis_type_, distance, similarity))){
