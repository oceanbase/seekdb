...
public:
  ...
  {
    TASK_TYPE_UT,
    TASK_TYPE_MACROMERGE,
    TASK_TYPE_INDEX_FINISH,
    TASK_TYPE_MAIN_FINISH,
    TASK_TYPE_MINOR_MERGE,
    TASK_TYPE_INDEX_PERPARE,
    TASK_TYPE_INDEX_LOCAL_SORT,
    TASK_TYPE_INDEX_MERGE,
    TASK_TYPE_NORMAL_MINOR_MERGE,
    TASK_TYPE_BUILD_INDEX_NORMAL_MERGE,
    TASK_TYPE_UNIQUE_INDEX_CHECKING,
    TASK_TYPE_REPORT_INDEX_STATUS,
    TASK_TYPE_MERGE_PREPARE_TASK,
    TASK_TYPE_INDEX_MERGE_TO_LATEST_FINISH,
    TASK_TYPE_COMPACT_TO_LASTEST,
    TASK_TYPE_SSTABLE_MERGE_PREPARE,
    TASK_TYPE_SSTABLE_MERGE_FINISH,
    TASK_TYPE_UNIQUE_CHECKING_PREPARE,
    TASK_TYPE_SIMPLE_UNIQUE_CHECKING,
    TASK_TYPE_FAKE,
    TASK_TYPE_STANDBY_RESTORE,
    TASK_TYPE_MAJOR_MERGE_FINISH,
    TASK_TYPE_SQL_BUILD_INDEX, // build index by sql plan.
    TASK_TYPE_COMPLEMENT_PREPARE,
    TASK_TYPE_COMPLEMENT_WRITE,
    TASK_TYPE_COMPLEMENT_MERGE,
    TASK_TYPE_DDL_KV_DUMP,
    TASK_TYPE_DDL_KV_MERGE,
    TASK_TYPE_MDS_MINI_MERGE,
    TASK_TYPE_BATCH_FREEZE_TABLETS,
    TASK_TYPE_LOB_BUILD_MAP,
    TASK_TYPE_LOB_MERGE_MAP,
    TASK_TYPE_LOB_WRITE_DATA,
    TASK_TYPE_TABLE_FINISH_BACKFILL,
    TASK_TYPE_VERIFY_CKM,
    TASK_TYPE_VECTOR_INDEX_MEMDATA_SYNC,
    TASK_TYPE_DELETE_LOB_META_ROW,
...
public:
  ...
  static ObDagWorker *self() { return self_; }
...
private:
  RLOCAL_STATIC(ObDagWorker *, self_);
...
// TODO(@DanLing) parameters in ObDagScheduler
...
private:
  ...
  ObIAllocator* reserved_allocator_;
  ...
  DagNetIdMap dag_net_id_map_; // search dag net by specified dag id; protected by dag_net_map_lock_
  int64_t dag_net_cnts_[ObDagNetType::DAG_NET_TYPE_MAX + 1];  // lock by dag_net_map_lock_
};
...
public:
  ...
  int init(
      ...
      ObIAllocator &reserved_allocator,
      ObDagScheduler &scheduler);
  // Sets only the priority field. Used by ObDagScheduler::init to keep
  ...
  int deal_with_finish_task(ObITask *&task, ObIDag *&dag, ObDagWorker &worker, int error_code);
...
private:
  ...
  void pause_worker_(ObDagWorker &worker);
...
public:
  ...
  int check_dag_net_exist(
      const ObDagId &dag_id, bool &exist);
  int cancel_dag_net(const ObDagId &dag_id);
  int deal_with_finish_task(ObITask *&task, ObDagWorker &worker, int error_code);
  bool try_switch(ObDagWorker &worker);
  int dispatch_task(ObITask &task, ObDagWorker *&ret_worker, const int64_t priority);
  void finish_dag_net(ObIDagNet *dag_net);
  template <typename T>
...
public:
  template<typename T>
  static int alloc_dag(ObIAllocator &allocator, const bool use_reserved_allocator, T *&dag);
...
private:
  ...
  lib::MemoryContext reserved_mem_context_;
...
int ObDagScheduler::alloc_dag(T *&dag)
{
  ...
  if (IS_NOT_INIT) {
    ...
    const bool use_reserved_allocator = tmp_dag.uses_reserved_allocator();
    ObIAllocator &allocator = get_allocator(use_reserved_allocator);
    if (OB_FAIL(alloc_dag(allocator, use_reserved_allocator, dag))) {
    }
  }
  return ret;
}
...
template<typename T>
int ObDagScheduler::alloc_dag(
    ObIAllocator &allocator,
    const bool use_reserved_allocator,
    T *&dag)
{
  int ret = OB_SUCCESS;
  void *buf = nullptr;
  if (DAG_SIZE_LIMIT < sizeof(T)){
    ...
    if (new_dag->uses_reserved_allocator() != use_reserved_allocator) {
      ret = OB_ERR_UNEXPECTED;
      COMMON_LOG(WARN, "dag allocator class does not match", K(ret), KPC(new_dag), K(use_reserved_allocator));
...
template <typename  T>
int ObDagScheduler::alloc_dag_with_priority(
...
template<typename T>
void ObDagScheduler::free_dag_net(T *&dag_net)
{
  if (OB_NOT_NULL(dag_net)) {
    ObIAllocator &allocator = get_allocator(dag_net->uses_reserved_allocator());
    dag_net->~T();
...
template <typename T>
int ObDagScheduler::create_and_add_dag_net(const ObIDagInitParam *param)
{
  int ret = common::OB_SUCCESS;
  void *buf = nullptr;
  T *dag_net = nullptr;
  ...
  if (IS_NOT_INIT) {
    ...
    T tmp_dag_net;
    ObIAllocator &allocator = get_allocator(tmp_dag_net.uses_reserved_allocator());
...
template<typename T>
int ObDagScheduler::create_and_add_dag(
    const ObIDagInitParam *param,
    const bool emergency/* = false*/,
    const bool check_size_overflow/* = true*/)
{
...
#define SET_RESERVE_MODE()                                               \
  ({                                                                     \
    share::ObDagWorker *worker = share::ObDagWorker::self(); \
