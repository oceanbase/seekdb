...
#ifndef OCEANBASE_DATA_PLANE_BLOCKSSTABLE_OB_STORAGE_DATUM_H_
#define OCEANBASE_DATA_PLANE_BLOCKSSTABLE_OB_STORAGE_DATUM_H_
...
// Stable data-plane value representation shared by query and storage.
// Storage-specific comparison/schema helpers remain in the Storage module.
struct ObStorageDatum : public common::ObDatum
{
  ...
  OB_INLINE void shallow_copy_from_datum(const common::ObDatum &src);
  ...
  OB_INLINE bool need_copy_for_encoding_column_with_flat_format(const common::ObObjDatumMapType map_type) const;
  OB_INLINE const char *to_cstring2(const bool for_dump = false) const;
  // only for unittest
  OB_INLINE bool operator==(const ObStorageDatum &other) const;
  OB_INLINE bool operator==(const common::ObObj &other) const;
...
OB_INLINE int ObStorageDatum::deep_copy(
...
OB_INLINE void ObStorageDatum::shallow_copy_from_datum(const common::ObDatum &src)
{
  if (this != &src) {
    reuse();
    pack_ = src.pack_;
...
OB_INLINE int64_t ObStorageDatum::storage_to_string(
    char *buf,
    int64_t buf_len,
    const bool for_dump) const
{
  int64_t pos = 0;
  if (is_ext()) {
    if (is_nop()) {
    ...
    if (null_) {
      ...
      if (len_ > 0) {
        OB_ASSERT(NULL != ptr_);
        const int64_t plen = std::min(
            static_cast<int64_t>(len_),
...
OB_INLINE bool ObStorageDatum::need_copy_for_encoding_column_with_flat_format(
    const common::ObObjDatumMapType map_type) const
{
  return common::OBJ_DATUM_STRING == map_type && sizeof(uint64_t) == len_ && is_local_buf();
}

} // namespace blocksstable
} // namespace oceanbase

#endif // OCEANBASE_DATA_PLANE_BLOCKSSTABLE_OB_STORAGE_DATUM_H_
