...
#include "share/table/ob_table_rpc_struct.h"  // ObTableDirectLoad* types (Table-API RPC removed)
...
#define OB_DEFINE_TABLE_DIRECT_LOAD_RPC_CALL_1(prio, name, pcode, Arg)                \
  ...
  {                                                                                   \
    ...
    } else if (OB_FAIL(direct_load_send_stub(request, result))) { /* Table-API RPC removed (feature decommissioned) */ \
...
public:
  // Table-API RPC removed (feature decommissioned); proxy no longer held.
  ObTableDirectLoadRpcProxy()
...
private:
  ...
  int direct_load_send_stub(const table::ObTableDirectLoadRequest &request,
                            table::ObTableDirectLoadResult &result)
  {
    UNUSEDx(request, result);
