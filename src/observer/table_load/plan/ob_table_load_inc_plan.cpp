...
private:
  // 数据表插入算子
  OB_DEFINE_TABLE_LOAD_DATA_TABLE_OP(INC_DATA_TABLE_INSERT_OP, IncDataTableEputOp, WRITE_INPUT,
...
int ObTableLoadIncPKTablePlan::generate()
{
  ...
  if (OB_UNLIKELY(is_generated())) {
    ...
    // 数据表插入算子
    ObTableLoadIncDataTableEputOp *inc_minor_data_table_insert_op = nullptr;
...
private:
  ...
  // ------------ AckPhase ------------ //
  // 数据表确认算子
  OB_DEFINE_TABLE_LOAD_DATA_TABLE_OP(INC_DATA_TABLE_ACK_OP, IncDataTableAckOp, CHANNEL_INPUT,
                                     ROWKEY, MERGE_WITH_ORIGIN_QUERY_FOR_DATA, INC,
                                     ObTableLoadNormalRowHandler,
                                     OB_TABLE_LOAD_INC_BASIC_OPEN_FLAG);
...
#define OB_TABLE_LOAD_TABLE_CHANNEL_DEF(DEF)                                                   \
  ...
  DEF(INC_UNIQUE_INDEX_TABLE_INSERT_OP, INC_DATA_TABLE_ACK_OP,                                 \
      ObTableLoadUniqueIndexToAckDataTableChannel, "unique_index_insert -> data_ack")          \
  ...
  if (OB_UNLIKELY(is_generated())) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < store_ctx_->index_store_table_ctxs_.count(); ++i) {
      ...
      if (index_table_ctx->schema_->is_local_unique_index()) {
        // inc minor
        ObTableLoadIncUniqueIndexTableInsertOp *inc_minor_unique_index_table_insert_op =
          nullptr;
