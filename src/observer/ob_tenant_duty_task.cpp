...
void ObTenantDutyTask::update_all_tenants()
{
  int ret = OB_SUCCESS;
  {
    if (OB_FAIL(update_tenant_wa_percentage())) {
...
int ObTenantDutyTask::read_tenant_wa_percentage(int64_t &pctg)
{
  int ret = OB_SUCCESS;
...
void ObTenantSqlMemoryTimerTask::runTimerTask()
{
  ...
  {
    MOD_SCOPE {
      ObTenantSqlMemoryManager *sql_mem_mgr = share::g_mp->tenant_sql_memory_manager();
...
int ObTenantDutyTask::update_tenant_wa_percentage()
{
  int ret = OB_SUCCESS;
  int64_t wa_pctg = 0;
...
int ObTenantDutyTask::read_obj(
    ObSysVarClassType sys_var, common::ObObj &obj)
{
  int ret = OB_SUCCESS;
  ObSchemaGetterGuard schema_guard;
  const ObSysVarSchema *var_schema = NULL;
...
int ObTenantDutyTask::read_int64(
    ObSysVarClassType sys_var, int64_t &val)
{
  int ret = OB_SUCCESS;
  ObObj obj;
...
int ObTenantDutyTask::read_double(
    ObSysVarClassType sys_var, double &val)
{
  int ret = OB_SUCCESS;
  ObObj obj;
  number::ObNumber num;
