...
#include "observer/virtual_table/ob_all_virtual_all_tables.h"
...
namespace observer
{
/*ObAllVirtualAllTables is deprecated, please modify NEW_TABLE_STATUS_SQL or NEW_TABLE_STATUS_SQL_ORA directly*/
...
ObAllVirtualAllTables::ObAllVirtualAllTables()
...
int ObAllVirtualAllTables::inner_open()
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualAllTables::get_sequence_value()
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualAllTables::get_table_stats()
{
  int ret = OB_SUCCESS;
  for(int64_t i = 0; OB_SUCC(ret) && i < table_schemas_.count(); ++i) {
    const ObTableSchema *table_schema = table_schemas_.at(i);
...
void ObAllVirtualAllTables::reset()
{
  session_ = NULL;
  sql_proxy_ = NULL;
  database_id_ = OB_INVALID_ID;
...
int ObAllVirtualAllTables::inner_get_next_row(common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualAllTables::inner_get_next_row()
{
  int ret = OB_SUCCESS;
              ...
              switch(col_id) {
                ...
                case ENGINE: {
                  if (table_schema->is_user_view()) {
                    ...
                    cells[cell_idx].set_varchar(ObString::make_string("InnoDB"));
                    cells[cell_idx].set_collation_type(ObCharset::get_default_collation(ObCharset::get_default_charset()));
                  }
                  break;
                }
