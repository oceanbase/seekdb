...
void ObVirtualOutlineBase::reset()
{
...
int ObVirtualOutlineBase::inner_open()
{
  int ret = OB_SUCCESS;
...
int ObVirtualOutlineBase::set_database_infos_and_get_value(uint64_t database_id, bool &is_recycle)
{
  int ret = OB_SUCCESS;
  is_recycle = false;
  DBInfo db_info;
...
int ObVirtualOutlineBase::is_database_recycle(uint64_t database_id, bool &is_recycle)
{
  int ret = OB_SUCCESS;
  DBInfo db_info;
  is_recycle = false;
...
void ObVirtualOutline::reset()
{
  ObVirtualOutlineBase::reset();
}

int ObVirtualOutline::fill_cells(const ObOutlineInfo *outline_info)
{
  int ret = OB_SUCCESS;
  ObObj *cells = NULL;
...
int ObVirtualOutline::is_output_outline(const ObOutlineInfo *outline_info, bool &is_output)
{
  int ret = OB_SUCCESS;
  is_output = false;
...
int ObVirtualOutline::inner_get_next_row(common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
