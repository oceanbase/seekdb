...
#include "observer/virtual_table/ob_all_virtual_concurrent_limit_sql.h"
...
void ObAllVirtualConcurrentLimitSql::reset()
{
  ObAllVirtualOutlineBase::reset();
}

int ObAllVirtualConcurrentLimitSql::is_need_output(const ObOutlineInfo *outline_info, bool &is_output)
{
  int ret = OB_SUCCESS;
  is_output = false;
...
int ObAllVirtualConcurrentLimitSql::inner_open()
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(ObAllVirtualOutlineBase::inner_open())) {
...
int ObAllVirtualConcurrentLimitSql::get_next_concurrent_limit_row(const ObOutlineInfo *outline_info,
                                                                     bool &is_iter_end)
{
  int ret = OB_SUCCESS;
  is_iter_end = false;
...
int ObAllVirtualConcurrentLimitSql::inner_get_next_row(common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
int ObAllVirtualConcurrentLimitSql::fill_cells(const ObOutlineInfo *outline_info,
      ...
      switch (col_id) {
        ...
        case LIMIT_TARGET : {
          ...
          if (outline_info->get_sql_text_str().empty()) {
            ...
            LOG_WARN("fail to get visibale signature", K(ret));
          } else if (OB_FAIL(sql::ObSQLUtils::gen_limit_sql(visible_signature, param, *session_, *allocator_, limit_sql))) {
