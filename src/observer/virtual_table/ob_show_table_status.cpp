...
namespace observer
{
/*ObShowTableStatus is deprecated, please modify NEW_TABLE_STATUS_SQL or NEW_TABLE_STATUS_SQL_ORA directly*/
...
ObShowTableStatus::ObShowTableStatus()
...
int ObShowTableStatus::inner_open()
{
  int ret = OB_SUCCESS;
...
int ObShowTableStatus::get_sequence_value()
{
  int ret = OB_SUCCESS;
  ObArray<AutoincKey> autoinc_keys;
  ObArray<int64_t> autoinc_versions;
...
int ObShowTableStatus::get_table_stats()
{
  int ret = OB_SUCCESS;
  for(int64_t i = 0; OB_SUCC(ret) && i < table_schemas_.count(); ++i) {
    const ObTableSchema *table_schema = table_schemas_.at(i);
...
int ObShowTableStatus::inner_get_next_row(common::ObNewRow *&row)
{
  int ret = OB_SUCCESS;
...
int ObShowTableStatus::inner_get_next_row()
{
  int ret = OB_SUCCESS;
              ...
              switch(col_id) {
                ...
                case ENGINE: {
                  if (table_schema->is_user_view()) {
                    ...
                    // ENGINE is accepted only as MySQL-compatible syntax. Keep
                    // SHOW TABLE STATUS consistent with the historical server
                    // behavior instead of exposing a per-table engine value.
