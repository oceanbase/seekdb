 ...
 */
 ...
 int ObShowCreateLocation::calc_show_location_id(uint64_t &show_location_id)
 {
   int ret = OB_SUCCESS;
   for (int64_t i = 0;
        OB_SUCCESS == ret && OB_INVALID_ID == show_location_id && i < key_ranges_.count(); ++i) {
     ...
     if (start_key.get_obj_cnt() > 0 && start_key.get_obj_cnt() == end_key.get_obj_cnt()) {
       if (OB_UNLIKELY(NULL == start_key_obj_ptr || NULL == end_key_obj_ptr)) {
         ...
         show_location_id = start_key_obj_ptr[0].get_int();
       } else {/*do nothing*/}
     }
   }
   return ret;
                                          ...
                                          const ObString &location_name)
 {
   ...
   if (OB_ISNULL(cur_row_.cells_)
     ...
     uint64_t cell_idx = 0;
     char *loc_def_buf = NULL;
     int64_t loc_def_buf_size = OB_MAX_VARCHAR_LENGTH;
     if (OB_UNLIKELY(NULL == (loc_def_buf = static_cast<char *>(allocator_->alloc(loc_def_buf_size))))) {
     ...
     for (int64_t i = 0; OB_SUCC(ret) && i < output_column_ids_.count(); ++i) {
       ...
       switch(col_id) {
         case OB_APP_MIN_COLUMN_ID: {
           // location_id
           cur_row_.cells_[cell_idx].set_int(show_location_id);
           break;
         }
         case OB_APP_MIN_COLUMN_ID + 1: {
           // location_name
           cur_row_.cells_[cell_idx].set_varchar(location_name);
