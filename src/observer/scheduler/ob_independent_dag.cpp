...
int ObITaskWithMonitor::add_monitor_info()
{
  ...
  if (OB_ISNULL(dag) || !dag->is_independent()) {
    ...
    storage::ObDDLDagMonitorNode *node = ind_dag->get_monitor_node();
    if (OB_ISNULL(node)) {
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_NOT_NULL(monitor_node_)) {
      for (int64_t i = 0; OB_SUCC(ret) && i < task_array.count(); ++i) {
        ...
        if (OB_ISNULL(task)) {
          ret = OB_ERR_UNEXPECTED;
          COMMON_LOG(WARN, "task is null", K(ret), K(i));
    ...
    const bool inherit_trace_id /*= false*/)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    COMMON_LOG(WARN, "init monitor node failed", K(ret));
...
int ObIndependentDag::init_monitor_node()
{
  int ret = OB_SUCCESS;
  monitor_node_ = nullptr;
  storage::ObDDLDagMonitorMgr *mgr = share::g_mp->ddl_dag_monitor_mgr();
  if (OB_ISNULL(mgr)) {
    COMMON_LOG(INFO, "ddl dag monitor manager is unavailable; monitoring disabled", KP(this));
  } else if (OB_FAIL(mgr->register_node(this, get_dag_id(), monitor_node_))) {
    COMMON_LOG(WARN, "register ddl dag monitor node failed", K(ret), KPC(this));
  }
  return ret;
}
