...
void ObDagWorker::run1()
{
  ...
  while (!has_set_stop()) {
    ...
    if (DWS_RUNNABLE == status_ && NULL != task_) {
      ...
      task_ = NULL;
      reset_compaction_thread_locals();
      if (OB_FAIL(share::g_mp->dag_scheduler()->deal_with_finish_task(cur_task, *this, ret/*task error_code*/))) {
...
int ObDagWorker::yield()
{
  ...
  if (!((++counter) & CHECK_INTERVAL)) {
    ...
    if (last_check_time_ + check_period_ <= curr_time) {
      ...
      } else if (DWS_RUNNING == status_ && share::g_mp->dag_scheduler()->try_switch(*this)) {
        status_ = DWS_WAITING;
...
int ObDagPrioScheduler::sys_task_start(ObIDag &dag)
{
  ...
  if (dag.get_dag_status() != ObIDag::DAG_STATUS_READY) {
    ...
    sys_task_status.comment_.assign_ptr(comment, static_cast<ObString::obstr_size_t>(strlen(comment)));
