    ...
    ObSchemaGetterGuard &schema_guard, const ObTableSchema &index_schema, const int64_t row_count)
{
  ...
  SERVER_MODULE_SCOPE {
    ...
    if (OB_ISNULL(service) || OB_ISNULL(shared_mem_mgr)) {
      ...
      {
        OB_ASSERT_SUCC(ret = ObPluginVectorIndexHelper::get_vector_memory_limit_size(mem_limited_size));
        if (OB_FAIL(estimate_vector_memory_used(schema_guard, index_schema, row_count, estimate_memory))) {
        } else if (OB_FALSE_IT(estimate_memory = ceil(
                                   estimate_memory * VEC_ESTIMATE_MEMORY_FACTOR *
                                   VEC_MEMORY_HOLD_FACTOR))) { // multiple 2.0， and need to consider the hold memory.
...
bool ObVectorIndexUtil::check_ivf_vector_index_memory(ObSchemaGetterGuard &schema_guard, const ObTableSchema &index_schema, const int64_t row_count)
{
  ...
  if (row_count <= 0) {
    ...
    if (data_table_id == OB_INVALID_ID) {
      ...
      OB_ASSERT_SUCC(ret = ObPluginVectorIndexHelper::get_vector_memory_limit_size(mem_limited_size));
      if (OB_FAIL(estimate_ivf_memory(row_count, param, construct_mem, buff_mem))) {
