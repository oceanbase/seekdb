...
#ifdef TG_DEF
#ifdef _WIN32
#define OB_NPROCESSORS_ONLN() ([]() -> long { SYSTEM_INFO _si; GetSystemInfo(&_si); return (long)_si.dwNumberOfProcessors; }())
#define GET_THREAD_NUM_BY_NPROCESSORS(factor) (OB_NPROCESSORS_ONLN() / (factor) > 0 ? OB_NPROCESSORS_ONLN() / (factor) : 1)
#define GET_THREAD_NUM_BY_NPROCESSORS_WITH_LIMIT(factor, limit) (OB_NPROCESSORS_ONLN() / (factor) > 0 ? min(static_cast<int64_t>(OB_NPROCESSORS_ONLN() / (factor)), static_cast<int64_t>(limit)) : 1)
#else
...
TG_DEF(MergeMemPool, MergeMemPool, TIMER)
...
TG_DEF(LogIOTaskCbThreadPool, LogIOCb, LINK_QUEUE_THREAD,
...
TG_DEF(LogSharedQueueTh, LogSharedQueueThread, QUEUE_THREAD,
       ...
       palf::LogSharedQueueTh::MINI_MODE_THREAD_NUM),
       palf::LogSharedQueueTh::MAX_LOG_HANDLE_TASK_NUM)
TG_DEF(ReplayService, ReplaySrv, LINK_QUEUE_THREAD, 1, (common::REPLAY_TASK_QUEUE_SIZE + 1) * OB_MAX_LS_NUM_PER_TENANT_PER_SERVER_CAN_BE_SET)
...
TG_DEF(CONFIG_MGR, ConfigMgr, TIMER)
...
TG_DEF(SyslogPerErrLimiter, LogLimiterRefresher, TIMER)
...
TG_DEF(TableLockService, OBJLockGC, TIMER)
TG_DEF(TenantMdsServiceRecyle, MdsTRecyle, TIMER)
TG_DEF(TenantMdsServiceDumpStatus, MdsTDump, TIMER)
TG_DEF(MultiVersionGarbageCollector, MultiVersionGC, TIMER)
