...
int ObServerTableOperator::update_start_info(
    ObMySQLTransaction &trans,
    const common::ObAddr &server,
    ...
    const ObServerInfoInTable::ObBuildVersion &new_build_version)
{
  ...
  if (OB_UNLIKELY(new_start_service_time < 0
      || new_build_version.is_empty()
      || !server.is_valid()
      || !server.ip_to_string(ip, sizeof(ip)))) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), K(server), K(new_start_service_time), K(new_build_version));
  } else {
    ObSqlString sql;
    if (OB_FAIL(sql.assign_fmt(
        "UPDATE %s SET start_service_time = %ld, build_version = '%s' "
        "WHERE svr_ip = '%s' AND svr_port = %d",
        OB_ALL_SERVER_TNAME, new_start_service_time, new_build_version.ptr(), ip, server.get_port()))) {
    ...
    ObDMLSqlSplicer &dml)
{
  ...
  if (OB_UNLIKELY(!server_status.is_valid()
    ...
    if (OB_FAIL(dml.add_pk_column(K(svr_ip)))
      ...
      // in version 4.2, we add a new column last_offline_time
