...
#ifndef OCEANBASE_SHARE_OB_LOCAL_MERGE_INFO_
#define OCEANBASE_SHARE_OB_LOCAL_MERGE_INFO_
...
public:
  ...
  ObLocalMergeInfo();
  ObLocalMergeInfo &operator =(const ObLocalMergeInfo &other) = delete;
  int assign(const ObLocalMergeInfo &other);
  // differ from assign, only exclude 'need_update_' copy
  int assign_value(const ObLocalMergeInfo &other);
  void reset();
  bool is_valid() const;
...
#endif  //OCEANBASE_SHARE_OB_LOCAL_MERGE_INFO_
