...
void ObBGThreadMonitorTimerTask::runTimerTask()
{
  monitor_.run_task_();
}
...
int ObBGThreadMonitor::start()
{
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
  } else if (OB_FAIL(TG_START(lib::TGDefIDs::BGThreadMonitor))) {
    SHARE_LOG(WARN, "fail to start BGThreadMonitor timer", K(ret));
  } else if (OB_FAIL(TG_SCHEDULE(lib::TGDefIDs::BGThreadMonitor, monitor_task_,
                                 CHECK_INTERVAL, true/*repeat*/, false/*immediate*/))) {
    SHARE_LOG(WARN, "fail to schedule BGThreadMonitor timer", K(ret));
  }
  return ret;
}
...
void ObBGThreadMonitor::wait()
{
  ...
  TG_WAIT(lib::TGDefIDs::BGThreadMonitor);
}

void ObBGThreadMonitor::stop()
{
  SHARE_LOG(INFO, "ObBGThreadMonitor stop");
  TG_STOP(lib::TGDefIDs::BGThreadMonitor);
}
...
void ObBGThreadMonitor::destroy()
{
  ...
  TG_DESTROY(lib::TGDefIDs::BGThreadMonitor);
  is_inited_ = false;
}
...
void ObBGThreadMonitor::run_task_()
{
