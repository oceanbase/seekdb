...
int ObGAISClient::get_leader_(ObAddr &leader)
{
  int ret = OB_SUCCESS;
  if (gais_cache_leader_valid_) {
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(GCTX.location_service_)) {
      ...
      gais_cache_leader_valid_ = true;
      LOG_INFO("succ to refresh leader", K(cluster_id), K(leader));
    }
  } else {
