...
#include "share/storage/ob_tablet_compaction_checksum_table_storage.h"
...
ObTabletCompactionChecksumTableStorage::ObTabletCompactionChecksumTableStorage()
  : pool_(nullptr)
{
}
...
int ObTabletCompactionChecksumTableStorage::init(ObSQLiteConnectionPool *pool)
{
  int ret = OB_SUCCESS;
...
int ObTabletCompactionChecksumTableStorage::create_table_if_not_exists()
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(pool_)) {
    ret = OB_NOT_INIT;
...
int ObTabletCompactionChecksumTableStorage::batch_get(
  ...
  if (!is_inited()) {
    ...
    if (OB_SUCC(ret)) {
      ...
      auto row_processor = [&](ObSQLiteRowReader &reader) -> int {
        ObTabletCompactionChecksumItem item;
...
int ObTabletCompactionChecksumTableStorage::range_get(const common::ObTabletID &start_tablet_id,
...
int ObTabletCompactionChecksumTableStorage::get_min_compaction_scn(uint64_t &min_compaction_scn)
{
  int ret = OB_SUCCESS;
...
int ObTabletCompactionChecksumTableStorage::get_max_row_count(const common::ObTabletID &tablet_id,
...
int ObTabletCompactionChecksumTableStorage::batch_check_checksum(const ObIArray<common::ObTabletID> &tablet_ids,
