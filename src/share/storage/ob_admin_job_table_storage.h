...
#ifndef OCEANBASE_SHARE_STORAGE_OB_ADMIN_JOB_TABLE_STORAGE_H_
#define OCEANBASE_SHARE_STORAGE_OB_ADMIN_JOB_TABLE_STORAGE_H_
...
class ObAdminJobTableStorage
{
public:
  ObAdminJobTableStorage();
  virtual ~ObAdminJobTableStorage();

  int init(ObSQLiteConnectionPool *pool);
  bool is_inited() const { return nullptr != pool_; }

  int create_job(const ObAdminJobEntry &entry);
...
private:
  ...
  DISALLOW_COPY_AND_ASSIGN(ObAdminJobTableStorage);
};

} // namespace share
} // namespace oceanbase

#endif // OCEANBASE_SHARE_STORAGE_OB_ADMIN_JOB_TABLE_STORAGE_H_
