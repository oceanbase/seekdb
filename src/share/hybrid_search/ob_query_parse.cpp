...
int ObESQueryParser::construct_hybrid_query(ObQueryReqFromJson *fts, ObQueryReqFromJson *knn, ObQueryReqFromJson *&hybrid)
{
  ...
  // NOTE: RRF fusion logic has been moved to ObHybridSearchFusionEngine (execution layer)
  // SQL generation now produces unified FULL OUTER JOIN without fusion calculations
  // Fusion strategy selection and execution is delegated to the fusion engine
  if (OB_FAIL(ret)) {
