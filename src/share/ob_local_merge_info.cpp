...
///////////////////////////////////////////////////////////////////////////////
#define CONSTRUCT_LOCAL_MERGE_INFO() \
...
ObLocalMergeInfo::ObLocalMergeInfo()
  : CONSTRUCT_LOCAL_MERGE_INFO(),
    start_merge_fail_times_(0)
{
}

int ObLocalMergeInfo::assign(const ObLocalMergeInfo &other)
{
  int ret = OB_SUCCESS;
  if (this != &other) {
...
int ObLocalMergeInfo::assign_value(
    const ObLocalMergeInfo &other)
{
  int ret = OB_SUCCESS;
  if (this != &other) {
    start_merge_fail_times_ = other.start_merge_fail_times_;
...
void ObLocalMergeInfo::reset()
{
  this->~ObLocalMergeInfo();
  new(this) ObLocalMergeInfo();
}

bool ObLocalMergeInfo::is_valid() const
{
  bool is_valid = true;
...
{
  return (ObLocalMergeInfo::MERGE_STATUS_VERIFYING == merge_status_.get_value());
}
