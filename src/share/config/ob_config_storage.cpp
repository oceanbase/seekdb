  ...
  if (OB_ISNULL(pool_)) {
    ...
    if (!guard) {
      ...
      LOG_WARN("failed to acquire connection", K(ret));
        ...
        "CREATE TABLE IF NOT EXISTS __all_namespace_worker_parameter ("
        "name TEXT NOT NULL PRIMARY KEY, value TEXT NOT NULL, "
        "gmt_modified INTEGER NOT NULL);", nullptr))) {
    }
  }
  return ret;
}

int ObConfigStorage::load_namespace_worker_configs(
    std::map<std::string, std::string> &configs)
{
  ...
  if (!is_inited()) {
    ...
    auto row_processor = [&](share::ObSQLiteRowReader &reader) -> int {
      int name_len = 0;
      int value_len = 0;
      const char *name = reader.get_text(&name_len);
      const char *value = reader.get_text(&value_len);
      if (OB_ISNULL(name) || OB_ISNULL(value) || name_len <= 0) {
        ret = OB_ERR_UNEXPECTED;
      } else {
        configs[std::string(name, name_len)] = std::string(value, value_len);
      }
      return ret;
    };
    share::ObSQLiteConnectionGuard guard(pool_);
    if (!guard) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("failed to acquire connection", K(ret));
    } else if (OB_FAIL(guard->query(
        "SELECT name, value FROM __all_namespace_worker_parameter;",
        nullptr, row_processor))) {
      if (OB_ENTRY_NOT_EXIST == ret) {
        ret = OB_SUCCESS;
      } else {
        LOG_WARN("failed to load namespace worker configs", K(ret));
      }
    }
  }
  return ret;
...
int ObConfigStorage::upsert_namespace_worker_config(
    const char *name,
    const char *value)
{
  int ret = OB_SUCCESS;
  if (!is_inited()) {
    ret = OB_NOT_INIT;
    LOG_WARN("not init", K(ret));
  } else if (OB_ISNULL(name) || OB_ISNULL(value) || '\0' == name[0]) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid namespace worker config", K(ret), KP(name), KP(value));
  } else {
    const char *upsert_sql =
        "INSERT OR REPLACE INTO __all_namespace_worker_parameter "
        "(name, value, gmt_modified) VALUES (?, ?, ?);";
    const int64_t current_time = ObTimeUtility::current_time();
    auto binder = [&](share::ObSQLiteBinder &b) -> int {
      int bind_ret = b.bind_text(name);
      if (OB_SUCC(bind_ret)) { bind_ret = b.bind_text(value); }
      if (OB_SUCC(bind_ret)) { bind_ret = b.bind_int64(current_time); }
      return bind_ret;
    };
    share::ObSQLiteConnectionGuard guard(pool_);
    if (!guard) {
      ...
      LOG_WARN("failed to persist namespace worker config", K(ret), K(name));
    }
  }
  return ret;
}
