CREATE OR REPLACE PACKAGE BODY dbms_ischeduler
  ...
  BEGIN
    ...
    IF my_job_name IS NOT NULL THEN
      SELECT MAX(job) INTO jid FROM OCEANBASE.__ALL_SCHEDULER_JOB
  ...
  BEGIN
    ...
    SELECT 1 INTO DUMMY FROM OCEANBASE.__ALL_SCHEDULER_JOB
      WHERE job_name = my_job_name AND POWNER = PUSER
            AND job != (SELECT MAX(job) FROM OCEANBASE.__ALL_SCHEDULER_JOB);
