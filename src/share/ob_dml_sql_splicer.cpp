...
int ObDMLSqlSplicer::append_value(const char *str, bool &is_null)
{
  ...
  int ret = OB_SUCCESS;
  if (!is_null) {
    if (mode_ == NAKED_VALUE_MODE) {
      ret = values_.append(str);
    } else {
      ret = values_.append("'");
      ret = ret != OB_SUCCESS ? ret : values_.append(str);
      ret = ret != OB_SUCCESS ? ret : values_.append("'");
    }
    if (OB_FAIL(ret)) {
...
int ObDMLSqlSplicer::append_value(const ObString &str, bool &is_null)
{
  ...
  if (!is_null) {
    if (mode_ == NAKED_VALUE_MODE) {
      ret = values_.append(str.ptr(), str.length());
    } else {
      ret = values_.append("'");
      ret = ret != OB_SUCCESS ? ret : values_.append(str.ptr(), str.length());
      ret = ret != OB_SUCCESS ? ret : values_.append("'");
    }
    if (OB_FAIL(ret)) {
      LOG_WARN("append string to values failed", K(ret), K(str));
