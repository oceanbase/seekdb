...
int get_real_file_path(const common::ObString &uri, char *buf, const int64_t buf_size, int device_type)
{
  ...
  if (OB_STORAGE_FILE == device_type) {
    ...
    const uint64_t sys_module_id)
{
  ...
  io_info.fd_ = fd;
  const int64_t real_timeout_ms = OB_IO_MANAGER.get_object_storage_io_timeout_ms();
    ...
    const uint64_t sys_module_id)
{
  ...
  if (ObStorageAccessType::OB_STORAGE_ACCESS_DIRECT_MULTIPART_WRITER == flag) {
    ...
    if (!is_complete_mode) {
      if (OB_FAIL(fd.device_handle_->buf_append_part(fd, buf, size, is_full))) {
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_FAIL(ObIOManager::get_instance().get_tenant_io_manager(tenant_holder))) {
      OB_LOG(WARN, "fail to get tenant io manager", KR(ret));
