    ...
    bool &trans_happened)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    // COUNT(*) observes only the number of rows produced by LIMIT.  The
    // particular equally-sized subset and its implicit relevance order are
    // unobservable, so the text iterator may stream and stop at the limit.
    if (can_prune_counted_projection) {
      ...
      if (OB_ISNULL(ctx_) || OB_ISNULL(ctx_->expr_factory_)) {
        ...
        // The parent does not consume the generated-table column.  Avoid a
        // base-table lookup whose value COUNT(*) immediately discards.
