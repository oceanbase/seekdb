  ...
  for (int64_t i = 0; OB_SUCC(ret) && !need_calc && i < relation_exprs.count(); ++i) {
    ...
    if (OB_FAIL(ret) || !contains_match_expr) {
      ...
      if (OB_FAIL(ret)) {
        ...
        need_calc = true;
      } else if (T_OP_BOOL != relation_exprs.at(i)->get_expr_type()) {
        // Functional lookup still needs the score projection when MATCH is
        // wrapped by a numeric or arithmetic expression. Only the implicit
        // boolean predicate has a fully score-free execution path.
        need_calc = true;
