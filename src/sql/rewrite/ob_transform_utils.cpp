                                                        ...
                                                        ObIArray<ObExprConstraint> &constraints)
{
  ...
  if (OB_ISNULL(expr) || OB_ISNULL(match_expr)) {
    ...
    } else if (expr->get_param_expr(0) == match_expr
               || expr->get_param_expr(1) == match_expr) {
      // A comparison consumes the numeric MATCH value even when the constant is zero.
      need_calc = true;
      need_check_child = false;
    }
      ...
      if (ObOptimizerUtil::find_item(stmt->get_condition_exprs(), root_expr)) {
        used_as_condition = true;
                 ...
                 && ObOptimizerUtil::find_item(
                        ...
                        root_expr)) {
        used_as_condition = true;
      } else if (OB_FAIL(check_expr_used_as_condition(
                     stmt, root_expr, match_expr, used_as_condition))) {
        LOG_WARN("failed to classify match expression usage", K(ret), K(i));
      }
      if (OB_FAIL(ret)) {
      } else if (!used_as_condition) {
        need_calc = true;
      } else if (OB_FAIL(inner_check_need_calc_match_score(
                     exec_ctx,
                     root_expr,
                     match_expr,
                     root_needs_score,
