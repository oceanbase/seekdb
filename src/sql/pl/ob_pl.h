...
#include "sql/pl/parser/parse_stmt_node.h"
#include "sql/pl/ob_pl_type.h"
#include "sql/pl/ob_pl_package_manager.h"
#include "sql/pl/ob_pl_interface_pragma.h"
...
struct ObPLExecCtx : public ObPLINS
{
  ObPLExecCtx(common::ObIAllocator *allocator,
              ...
              query::ObIAiEndpointAdmin *ai_endpoint_admin = NULL,
              share::ObResourceLimitCalculator *resource_limit_calculator = NULL) :
  ...
  query::ObIAiEndpointAdmin *ai_endpoint_admin_;
...
struct ExecCtxBak
{
#define PL_EXEC_CTX_BAK_ATTRS phy_plan_ctx_,expr_op_ctx_store_,expr_op_size_,has_non_trivial_expr_op_ctx_,frames_,frame_cnt_,pl_expr_allocator_,resource_limit_calculator_
...
class ObPLContext : public sql::ObIProceduralContext
{
...
public:
      ...
      parent_stack_(nullptr)
  {
    ...
    exec_ctx_.set_pl_stack_ctx(&pl_stack, &pl_stack);
  }
  ~LinkPLStackGuard()
  {
    //pop the pl stack from current execution stack
    exec_ctx_.set_pl_stack_ctx(parent_stack_, parent_stack_);
  }
                                         ...
                                         ObPLContext *snapshot)
  {
    if (exec_ctx.get_pl_stack_ctx() != snapshot) {
      exec_ctx.set_pl_stack_ctx(snapshot, snapshot);
    }
  }
private:
  sql::ObExecContext &exec_ctx_;
