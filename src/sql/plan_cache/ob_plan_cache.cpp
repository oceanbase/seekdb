...
int ObPlanCache::cache_evict_all_obj()
{
  ...
  ObKVEntryTraverseOp get_ids_op(&to_evict_keys);
...
int ObPlanCache::cache_evict_by_ns(ObLibCacheNameSpace ns)
{
  int ret = OB_SUCCESS;
  LCKeyValueArray to_evict_keys;
  ObGetKVEntryByNsOp get_ids_op(ns, &to_evict_keys);
  if (OB_FAIL(foreach_cache_evict(get_ids_op))) {
  }
  return ret;
}
...
int ObPlanCache::cache_evict_plan_by_sql_id(uint64_t db_id, common::ObString sql_id)
{
  int ret = OB_SUCCESS;
  LCKeyValueArray to_evict_keys;
  ObGetKVEntryBySQLIDOp get_ids_op(db_id, sql_id, &to_evict_keys);
  if (OB_FAIL(foreach_cache_evict(get_ids_op))) {
  }
  return ret;
}
...
int ObPlanCache::cache_evict()
{
  ...
  if (get_mem_hold() > get_mem_high()) {
    if (calc_evict_num(cache_evict_num) && cache_evict_num > 0) {
      ...
      ObNodeStatFilterOp filter(cache_evict_num, &to_evict_keys,
      ...
      if (OB_SUCC(ret) && idle_evict_keys.count() > 0) {
        ...
        for (int64_t i = 0; i < idle_evict_keys.count(); i++) {
          if (OB_NOT_NULL(idle_evict_keys.at(i).node_)) {
            idle_evict_keys.at(i).node_->dec_ref_count();
          }
        }
      }
    ...
    if (to_evict.count() > 0) {
      ...
      for (int64_t i = 0; i < to_evict.count(); i++) {
        if (OB_NOT_NULL(to_evict.at(i).node_)) {
          to_evict.at(i).node_->dec_ref_count();
        }
      }
    }
  }
...
int ObPlanCache::ref_plan(const ObCacheObjID plan_id, ObCacheObjGuard& guard)
{
  ...
  ObILibCacheObject *cache_obj = NULL;
  query::check_plan_cache_access(access_service());
  if (OB_FAIL(ref_cache_obj(plan_id, guard))) {
  } else {
    cache_obj = guard.cache_obj_;
    if (OB_ISNULL(cache_obj)) {
      ret = OB_ERR_UNEXPECTED;
...
int ObPlanCache::ref_alloc_plan(const ObCacheObjID plan_id, ObCacheObjGuard& guard)
{
  ...
  ObILibCacheObject *cache_obj = NULL;
  query::check_plan_cache_access(access_service());
  if (OB_FAIL(ref_alloc_obj(plan_id, guard))) {
  } else {
    cache_obj = guard.cache_obj_;
    if (OB_ISNULL(cache_obj)) {
      ret = OB_ERR_UNEXPECTED;
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(co_mgr_.erase_cache_obj(cache_obj_id))) {
  }
  return ret;
}
...
int ObPlanCache::alloc_cache_obj(ObCacheObjGuard& guard, ObLibCacheNameSpace ns)
{
  ...
  if (OB_FAIL(co_mgr_.alloc(guard, ns, root_context_))) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("cache object manager returned null object", K(ret), K(ns));
  } else {
    guard.cache_obj_->plan_cache_ = this;
  }
  return ret;
}

void ObPlanCache::free_cache_obj(ObILibCacheObject *&cache_obj)
{
  co_mgr_.free(cache_obj);
  cache_obj = NULL;
}
...
int ObPlanCache::flush_plan_cache()
{
  ...
  if (OB_FAIL(ret)) {
    // do nothing
  } else if (OB_FAIL(query::get_plan_cache_safe_timestamp(
      access_service(), safe_timestamp))) {
...
void ObPlanCacheEliminationTask::run_plan_cache_task()
{
  ...
  if (OB_FAIL(plan_cache_->update_memory_conf())) {
  }
