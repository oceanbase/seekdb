...
int ObForkDatabaseResolver::resolve(const ParseNode &parse_tree)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(dst_database_node) && OB_ISNULL(src_database_node)) {
      ret = OB_ERR_UNEXPECTED;
      SQL_RESV_LOG(WARN, "invalid parse tree!", K(ret));
    } else if ((dst_database_node != NULL && T_IDENT != dst_database_node->type_)
      ...
      if (dst_database_node == NULL) {
        dst_database_name = ObString::make_string("__drop__");
      } else {
      ...
      if ((dst_database_node != NULL && dst_database_name.prefix_match("__"))
        ...
        } else if (dst_database_node != NULL && OB_FAIL(ObSQLUtils::check_and_convert_db_name(
