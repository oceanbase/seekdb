                                                         ...
                                                         ObIArray<ObString> &column_names)
{
  ...
  if (T_FUN_SYS_AI_SPLIT_DOCUMENT == table_expr->get_expr_type()) {
    // sys_func table function: no PL UDT; return the 4 fixed column names.
    static const char *AI_SPLIT_COLS[4] = {"chunk_id", "chunk_offset", "chunk_length", "chunk_text"};
    for (int64_t i = 0; OB_SUCC(ret) && i < 4; ++i) {
      ObString name(AI_SPLIT_COLS[i]);
      OZ (column_names.push_back(name));
    }
  } else if (T_FUN_SYS_GENERATOR == table_expr->get_expr_type()) {
