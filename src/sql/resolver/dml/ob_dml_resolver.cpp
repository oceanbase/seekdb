                                                               ...
                                                               ObIArray<ColumnItem> &col_items)
{
  ...
  if (OB_FAIL(ret)) { // do nothing ...
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < 4; ++i) {
      col_item = NULL;
      if (NULL != (col_item = stmt->get_column_item(table_item.table_id_, column_names[i]))) {
        // exist, ignore resolve...
      } else {
        ObObjMeta meta;
        ObAccuracy accuracy;
        if (i < 3) {
          meta.set_int();
          accuracy.set_accuracy(ObAccuracy::DDL_DEFAULT_ACCURACY[ObIntType]);
        } else {
          meta.set_varchar();
          meta.set_collation_type(ObCharset::get_system_collation());
          meta.set_collation_level(CS_LEVEL_IMPLICIT);
          accuracy.set_length(OB_MAX_VARCHAR_LENGTH);
        }
        OZ (resolve_function_table_column_item(table_item,
                                               meta,
                                               accuracy,
