  ...
  if (OB_FAIL(ret)) {
    ...
    if (0 == col_name.case_compare("CHUNK_ID") ||
      ...
      if (OB_ISNULL(table_item) || OB_ISNULL(table_item->json_table_def_)
        ...
        // seekdb: accept any utf8mb4 collation (e.g. the MySQL 8.0 default utf8mb4_0900_ai_ci),
        // not just utf8mb4_bin / utf8mb4_general_ci. Non-utf8mb4 (e.g. binary BLOB) still rejected.
        if (!res_type.is_null() && ObCharset::charset_type_by_coll(coll_type) != CHARSET_UTF8MB4) {
          ret = OB_NOT_SUPPORTED;
