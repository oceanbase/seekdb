...
int ObDMLResolver::ai_split_document_table_add_columns(TableItem *&table_item)
{
  INIT_SUCC(ret);
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < 4; ++i) {
    ...
    if (OB_ISNULL(col_def = static_cast<ObDmlJtColDef*>(allocator_->alloc(sizeof(ObDmlJtColDef))))) {
      ...
      col_def->col_base_info_.col_name_.assign_ptr(col_names[i], static_cast<int32_t>(strlen(col_names[i])));
      ...
      if (i < 3) {
        ...
        if (OB_FAIL(session_info_->get_collation_connection(coll_type))
            || OB_FAIL(session_info_->get_character_set_connection(charset_type))) {
          LOG_WARN("failed to get connection charset/collation", K(ret));
        } else {
      ...
      if (OB_FAIL(generate_json_table_output_column_item(table_item,
                                                         ...
                                                         col_item))) {
        LOG_WARN("failed to generate output column item", K(ret), K(i));
        ...
        LOG_WARN("failed to push back column def", K(ret));
      }
    }
  }
  return ret;
...
int ObDMLResolver::resolve_ai_split_document_item(const ParseNode &parse_tree, TableItem *&tbl_item)
{
  ...
  } else if (OB_FAIL(ai_split_document_table_add_columns(item))) {
    LOG_WARN("failed to add ai split document columns", K(ret));
  } else {
    tbl_item = item;
  }
  return ret;
