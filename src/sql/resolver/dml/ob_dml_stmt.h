...
public:
  ...
  // Extract all expression relation ids from this stmt
  ...
  // For example: select c1+1 from t1 where c1>1 and c2<1; In this query,
  // This expression contains the relation expr node which is the '+' expression in the select item c1+1,
  // Condition expression '>' in c1>1 and '<' in c2<1, some stmt will generate some intermediate expressions,
  // Intermediate expression is not recorded in relation expr,
  // For example, if there are equality conditions in a select statement, it will generate an equal set,
  //expressions in equal set are not added to relation exprs,
  // relation expr expressions will be deduplicated, the deduplication data structure uses hash set,
  // Make the flag of relation expr trees, deduce type, and relation id analysis more performance-efficient,
  // Avoid doing some redundant analysis, consuming less memory and time

  int get_relation_exprs(common::ObIArray<ObRawExpr *> &relation_exprs,
                         ObStmtExprGetter &visitor) const;
  // If you want to modify the pointer's pointing, you need to use the following interface; ignore_scope indicates that the relation does not need to be acquired
...
protected:
  ...
  // Get all query-related expressions (expressions generated by attributes specified in the query statement) root expr from stmt
