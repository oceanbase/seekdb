  ...
  if (OB_ISNULL(grant_role) || OB_ISNULL(grant_stmt)
    ...
    if (OB_SUCC(ret)) {
      if (user_name_array.count() != host_name_array.count()) {
        ...
        for (int i = 0; OB_SUCC(ret) && i < user_name_array.count(); ++i) {
          ...
          OZ (schema_ck->get_user_info(user_name, host_name, grantee_info), user_name, host_name);
    ...
    if (OB_SUCC(ret)) {
      ...
      for (int i = 0; OB_SUCC(ret) && i < role_list->num_child_; ++i) {
        ...
        OZ (schema_ck->get_user_info(user_name, host_name, role_info), user_name, host_name);
        ...
        if (OB_SUCC(ret)) {
          ...
          if (OB_SUCC(ret)) {
            if (OB_FAIL(check_and_convert_name(db, table))) {
              ...
              OB_ASSERT_SUCC(ret = grant_stmt->set_database_name(db));
              OB_ASSERT_SUCC(ret = grant_stmt->set_table_name(table));
            }
          }
