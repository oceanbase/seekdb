...
{
  int ret = OB_SUCCESS;
  bool is_nullable = ((flags & share::ObSysVarFlag::NULLABLE) && (value.get_type() == ObNullType));
...
void ObBasicSysVar::reset()
{
  ...
  clean_value();
  type_ = ObUnknownType;
  flags_ = share::ObSysVarFlag::NONE;
...
{
  return share::ObSysVarMeta::get_sys_var_name_by_id(get_type());
}
...
int ObBasicSysVar::is_need_serialize(bool &need_serialize) const
{
  ...
  if (OB_FAIL(share::ObSysVarMeta::calc_sys_var_store_idx(get_type(), sys_var_idx))) {
    LOG_WARN("fail to calc sys var store idx", K(ret), K(sys_var_idx));
  } else if (OB_UNLIKELY(sys_var_idx < 0) ||
             OB_UNLIKELY(sys_var_idx >= share::ObSysVariables::get_all_sys_var_count())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("invalid sys var idx", K(ret),
              K(sys_var_idx), K(share::ObSysVariables::get_all_sys_var_count()));
  } else {
    need_serialize = (0 != (share::ObSysVariables::get_flags(sys_var_idx) & share::ObSysVarFlag::NEED_SERIALIZE));
  }
  return ret;
}
*/
...
{
  int ret = OB_SUCCESS;
  // In order to behave like MySQL, we do not check if flags_ contains share::ObSysVarFlag::NULLABLE here,
                                           ...
                                           const ObObj &in_val, ObObj &out_val)
{
  ...
  int ret = OB_SUCCESS;
  if (true == set_var.is_set_default_
      || (0 != (flags_ & share::ObSysVarFlag::NULLABLE) && ObNullType == in_val.get_type())) {
    // do nothing
  } else {
    ObString str_val;
    int32_t offset = 0;
  ...
  if (true == set_var.is_set_default_) {
    ...
    if (ObVarcharType == in_val.get_type()) {
      ...
      if (CS_TYPE_INVALID == (coll_type = ObCharset::collation_type(coll_name))) {
        ...
        ret = OB_NOT_SUPPORTED;
        LOG_USER_ERROR(OB_NOT_SUPPORTED,"only utf8mb4_general_ci and utf8mb4_unicode_ci are supported, other collation");
      } else {
        out_val.set_int(static_cast<int64_t>(coll_type));
      }
    } else if (ObIntType == in_val.get_type()) {
      ...
      } else if (int64_val != CS_TYPE_UTF8MB4_GENERAL_CI
                 && int64_val != CS_TYPE_UTF8MB4_UNICODE_CI) {
        ret = OB_NOT_SUPPORTED;
        LOG_USER_ERROR(OB_NOT_SUPPORTED,"only utf8mb4_general_ci and utf8mb4_unicode_ci are supported, other collation");
      } else {
        out_val = in_val;
...
int ObPreProcessSysVars::change_base_values(const ObIArray<std::pair<ObString, ObString>> &sys_vars)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < sys_vars.count(); ++i) {
    ...
    if (OB_FAIL(share::ObSysVariables::set_value(sys_var.first, sys_var.second))) {
