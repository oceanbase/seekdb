...
// This array contains essential system variables that must be preloaded during initialization, including:
// 1. compatibility mode related vars (ob_compatibility_mode, sql_mode, ob_read_consistency)  
        ...
        &sys_var_delta_ids, char *buf, const int64_t &buf_len, int64_t &pos)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < sys_var_delta_ids.count(); ++i) {
    ...
    if (OB_FAIL(share::ObSysVarMeta::calc_sys_var_store_idx(sys_var_delta_ids.at(i), sys_var_idx))) {
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("sys var idx is invalid", K(sys_var_idx), K(get_sys_var_count()), K(ret));
    } else if (ensure_sys_var_loaded(sys_var_delta_ids.at(i)) != OB_SUCCESS) {
      LOG_WARN("fail to ensure sys var loaded", K(ret), K(sys_var_delta_ids.at(i)));
    } else if (OB_ISNULL(sys_vars_[sys_var_idx])) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("sys var is NULL", K(ret), K(sys_var_idx), K(get_sys_var_count()));
    } else {
...
int ObBasicSessionInfo::set_session_state_(ObSQLSessionState state)
{
  ...
  if (OB_UNLIKELY(SESSION_KILLED == thread_data_.state_)) {
    ...
    LOG_WARN("session is killed", K(ret), K(sessid_), K(client_sessid_), K(state));
