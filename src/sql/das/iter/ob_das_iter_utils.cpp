    ...
    ObDASIter *&retrieval_result)
{
  ...
  if (OB_FAIL(ObDASTRMergeIter::build_query_tokens(ir_scan_ctdef, ir_scan_rtdef, alloc,
    ...
    if (ir_scan_ctdef->has_pushdown_topk() && !has_duplicate_boolean_tokens) {
      ...
      // accumulator used by TaaT.  Keep streaming posting lists with DaaT
      // even for large token sets; the DaaT iterators are dynamically sized,
      // and this also avoids constructing a relevance-only fallback subtree.
      merge_iter_param.daat_mode_ = 1;
      ...
      // Boolean evaluation depends on per-token match state maintained by TaaT.
      merge_iter_param.taat_mode_ = 1;
    } else {
      // DaaT streams naturally ordered posting lists and can stop at LIMIT
      // without materializing the complete candidate set.
      merge_iter_param.daat_mode_ = 1;
    }
