...
{
  int ret = OB_SUCCESS;
  sr_iter_param_.dim_weights_ = ir_ctdef_->need_calc_relevance() && dim_weights_.count() >= 1
  ...
  if (ir_ctdef_->need_calc_relevance() && OB_NOT_NULL(ir_ctdef_->field_boost_expr_)) {
  ...
  if (!ir_ctdef_->need_calc_relevance() || query_tokens_.count() == 0) {
    // do nothing
      ...
      || OB_UNLIKELY(1 != ir_ctdef_->get_doc_agg_ctdef()
          ->pd_expr_spec_.pd_storage_aggregate_output_.count())) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid document aggregate definition", K(ret), KP(ir_ctdef_->get_doc_agg_ctdef()));
...
int ObDASTRMergeIter::init_taat_iter_param(ObTextTaaTParam &iter_param)
{
  ...
  if (OB_ISNULL(iter_param.dim_iter_ = dim_iter_)) {
    ...
    // Boolean filtering does not consume corpus statistics.
  } else if (OB_ISNULL(ir_ctdef_->get_doc_agg_ctdef())
      || OB_UNLIKELY(1 != ir_ctdef_->get_doc_agg_ctdef()
          ->pd_expr_spec_.pd_storage_aggregate_output_.count())) {
    ret = OB_ERR_UNEXPECTED;
