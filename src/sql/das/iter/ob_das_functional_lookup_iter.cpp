...
{
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(storage_count > index_scan_rowsize_ - data_scan_read_rowsize_)) {
    ret = OB_ERR_UNEXPECTED;
...
int ObDASFuncLookupIter::do_index_lookup()
{
  ...
  if (merge_iter->has_main_lookup_iter()) {
    ...
    const int64_t doc_id_count = merge_iter->get_doc_id_count();
    const int64_t start_pos = data_scan_read_rowsize_;
    const int64_t end_pos = data_scan_read_rowsize_ + merge_iter->get_doc_id_count();
    if (OB_FAIL(ret)) {
      // do nothing
