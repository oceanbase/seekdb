                                                             ...
                                                             ObFTWordMap *reusable_word_map)
{
  ...
  ObFTWordMap *ft_word_map = reusable_word_map;
  void *rows_buf = nullptr;
  blocksstable::ObDatumRow *rows = nullptr;
  if (OB_ISNULL(helper) || OB_UNLIKELY(!ft_obj_meta.is_valid())) {
    ...
    ret = OB_ITER_END;
             ...
             && OB_FAIL(local_word_map.create(ft_word_bkt_cnt, common::ObMemAttr("FTWordMap")))) {
    LOG_WARN("fail to create ft word map", K(ret), K(ft_word_bkt_cnt));
  } else if (OB_ISNULL(ft_word_map) && FALSE_IT(ft_word_map = &local_word_map)) {
  } else if (OB_NOT_NULL(ft_word_map)
             && !ft_word_map->created()
             && OB_FAIL(ft_word_map->create(ft_word_bkt_cnt, common::ObMemAttr("FTWordMap")))) {
    LOG_WARN("fail to lazily create reusable full-text word map", K(ret), K(ft_word_bkt_cnt));
