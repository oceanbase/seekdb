                                            ...
                                            IndexDMLInfo *index_dml_info)
{
  ...
  if (OB_ISNULL(top) || OB_ISNULL(table_partition_info)) {
    ...
    if (OB_FAIL(allocate_stat_collector_as_top(top,
      ...
      LOG_WARN("failed to allocate exchange as top", K(ret));
    } else if ((exch_info.need_exchange() || session->get_ddl_info().is_partition_local_ddl())
               && OB_FAIL(allocate_sort_as_top(top, sort_keys))) {
...
int ObDelUpdLogPlan::prepare_inverted_sort_keys(ObLogStatCollector &stat_collector)
{
  ...
  if (OB_ISNULL(insert_stmt) || OB_ISNULL(schema_guard) || OB_ISNULL(session)) {
    ...
    if (OB_ISNULL(insert_table)) {
    ...
    } else if (OB_FAIL(schema_guard->get_table_schema(insert_table->ddl_table_id_, table_schema))) {
      LOG_WARN("failed to get table schema", K(ret));
    } else if (OB_ISNULL(table_schema)) {
      ret = OB_ERR_UNEXPECTED;
      ...
      } else if (OB_FAIL(schema_guard->get_table_schema(table_schema->get_database_id(),
                                                 inverted_table_name,
                                                 true /*is_index*/,
                                                 inverted_schema,
                                                 table_schema->is_offline_ddl_table() /*with_hidden_flag*/))) {
        LOG_WARN("failed to get inverted index schema by name", K(ret), K(inverted_table_name), K(table_schema->get_database_id()));
