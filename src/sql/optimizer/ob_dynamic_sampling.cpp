...
int ObDynamicSampling::add_table_clause(ObSqlString &table_str)
{
  ...
  if (OB_UNLIKELY(is_big_table_ && sample_big_table_rown_cnt_ < MAGIC_MAX_AUTO_SAMPLE_SIZE)) {
    ...
    LOG_WARN("get unexpected error", K(ret), K(sample_big_table_rown_cnt_), K(is_big_table_));
  } else if (is_big_table_ && OB_FAIL(table_str.append_fmt("(SELECT * FROM `%.*s`.`%.*s` %.*s %.*s LIMIT %ld) %s%.*s%s" ,
                                                           db_name_.length(),
                                                           db_name_.ptr(),
                                                           table_name_.length(),
                                                           table_name_.ptr(),
                                                           ...
                                                           alias_name_.empty() ? " " : ("`")))) {
    LOG_WARN("failed to append fmt", K(ret));
  } else if (!is_big_table_ && OB_FAIL(table_str.append_fmt("`%.*s`.`%.*s` %.*s %.*s %s%.*s%s" ,
                                                            db_name_.length(),
                                                            db_name_.ptr(),
                                                            table_name_.length(),
                                                            table_name_.ptr(),
...
//   if (OB_FAIL(gen_select_filed(select_fields))) {
//     LOG_WARN("failed to generate select filed", K(ret));
//   } else if (OB_FAIL(raw_sql_str.append_fmt("SELECT %.*s %.*s FROM `%.*s`.`%.*s` %.*s %.*s %.*s %.*s `%.*s`.`%.*s` %.*s %.*s %.*s %s %.*s %s %.*s",
//                                             basic_hints_.length(),
//                                             basic_hints_.ptr(),
//                                             static_cast<int32_t>(select_fields.length()),
//                                             select_fields.ptr(),
