...
namespace sql
{
inline double revise_sel_ndv(double ndv) { return ndv < 1.0 ? 1.0 : ndv; }
                                                ...
                                                bool is_semi)
{
  ...
  if (max1 - min1 > OB_DOUBLE_EPSINON) {
    overlap_ndv1 = revise_sel_ndv(ndv1 * overlap / (max1 - min1)) ;
  } else {
    overlap_ndv1 = 1;
  }
  if (max2 - min2 > OB_DOUBLE_EPSINON) {
    overlap_ndv2 = revise_sel_ndv(ndv2 * overlap / (max2 - min2)) ;
  } else {
    overlap_ndv2 = 1;
  }
  if (is_semi) {
