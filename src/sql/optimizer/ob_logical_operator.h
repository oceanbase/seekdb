...
public:
  ...
  // In the MANUAL_TABLE_DOP case, the exchange operator can only be called in alloc_gi_pre
  int push_current_dfo_dop(int64_t dop);
  // In the MANUAL_TABLE_DOP case, the exchange operator can only be called in alloc_gi_post
  ...
  // The number of multi-branch operators on the path from the current dfo's root op to the current processing op
  int64_t multi_child_op_above_count_in_dfo_;
  // Recorded the number of partitions for the current GI direct TSC
  int64_t partition_count_;
  // Recorded whether the current GI direct TSC is a hash/key partition table
  bool hash_part_;
...
public:
  ...
  ObRawExpr *expr_;
  // Generally you can ignore this variable, simply think of it as a bool variable,
  // Indicates whether the expression has been generated, does not affect understanding the entire expression architecture
  // If it is really unavoidable to use this value, you need to thoroughly understand it, @Xifeng
  uint64_t producer_branch_;
  // A plan contains many expressions, each expression must have two attributes:
  //  - Who is responsible for calculating/generating this expression
  //  - Who is responsible for consuming this expression
  //
  // In OB:
  //  - producer id indicates the id of the expression generator
  //  - consumer id indicates the id of the consumer operator of the expression
  //
  // - For regular expressions (such as c1+1), in the allocate_expr_pre stage
  //   First encounter of these expressions, their consumer id and producer id will be set to the current operator,
  // - For fix expr, allocate_expr_pre stage first encounters these expressions, it will only set consumer id
  //   and producer id is delayed to be generated by the subsequent operator (there must be an operator later that can generate this expression)
  //
  // The expression generation process is very convoluted, here is a document to help you get started:
  //  - 
  // If you still don't understand, please directly @Xifeng
...
public:
  ...
  /**
   * Currently the optimizer uses two phases to generate the plan:
   * The first phase is to generate join order and other operators
   * The second phase is to allocate exchange nodes and handle other logic (various plan_traverse_loop)
   * In the first phase, to avoid copying the plan tree and save memory usage of the optimizer, the optimizer only sets child nodes, not parent nodes, that is, all parent_ are NULL,
   * Therefore, at any time during the first phase, the get_parent interface cannot be called to obtain the parent node (the parent node can be obtained by passing in the root node).
   * In the second phase, because the get_parent interface is used in many places, at the end of the first phase, we set the parent correctly according to the child nodes, so that the get_parent interface can be called to obtain the parent node in the second phase.
   * Regarding why the parent is not set in the first phase:
   * Consider the following example, when we allocate the group by operator, for the current plan, there are usually two ways: hash group and merge group, because a child node can only have one parent,
   * So we must copy the current plan, then allocate hash group by and merge group by on these two plans respectively, and set the corresponding parent, this copying method will occupy a large amount of memory space,
   * If the parent structure is not maintained, this plan copying can be avoided.
   */
  ...
  /**
   * The average length of rows generated by the operator (in bytes)
   */
  ...
  /**
   * The depth of the operator in the plan tree, with top-level operators being 0
   */
  ...
  // Indicates that only one child's data is iterated at a time, and data from one child is received before starting to receive the next child's data
  ...
  // Generate partition id expression
...
protected:
  ...
  common::ObSEArray<ObRawExpr *, 8, common::ModulePageAllocator, true> pushdown_filter_exprs_; // pushdown filtering exprs, used for calculating condition pushdown of nl join's join keys
...
protected:
  ...
  void *traverse_ctx_;
  // Strict partition wise join constraint, requires that the base table partitions within a group are logically and physically the same
  ObPwjConstraint strict_pwj_constraint_;
  // Non-strict partition wise join constraint, requires the base table partitions within the group to be physically identical
  ObPwjConstraint non_strict_pwj_constraint_;
  // The physical distribution of the dependent tables
  //ObLocationConstraint if there is only one element, then only need to constrain whether the location type is consistent;
  //                    If there are multiple elements, then each location's corresponding physical distribution needs to be verified to be the same
  ObLocationConstraint location_constraint_;
  // Used for constraint extraction of multi part dml
  // multi_part_location_constraint_ can only have one constraint element
  ...
  int64_t interesting_order_info_;  // Record where the operator's order is used in stmt e.g. join, group by, order by
  int64_t parallel_;
