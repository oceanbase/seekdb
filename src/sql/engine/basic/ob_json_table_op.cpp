  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(params_expr)) {
      ...
      if (params_type == ObNullType) {
        is_null = true;
      } else if (!(params_type == ObJsonType
                    || ob_is_string_type(params_type))) {
        // seekdb is MySQL-only: Oracle types (ObNCharType/ObRawType) do not exist;
        // ob_is_string_type already covers string + text/lob type classes.
