...
class ObStorageVecSortImpl
{
  ...
  typedef ObStorageVecSortImpl<Compare, Store_Row, has_addon> ThisType;
public:
...
private:
  ...
  friend class ObFullSortStrategy;
...
private:
  ...
  ObFullSortStrategy<ThisType> *sort_strategy_;
...
int ObStorageVecSortImpl<Compare, Store_Row, has_addon>::init_sort_strategy(const bool enable_encode_sortkey)
{
  ...
  void *buf = allocator.alloc(sizeof(ObFullSortStrategy<ThisType>));

  if (OB_ISNULL(buf)) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    SQL_ENG_LOG(WARN, "failed to allocate memory", K(ret));
  } else {
    UNUSED(enable_encode_sortkey);
    sort_strategy_ = new (buf) ObFullSortStrategy<ThisType>();
...
template <typename Compare, typename Store_Row, bool has_addon>
int ObStorageVecSortImpl<Compare, Store_Row, has_addon>::do_full_sort_strategy(const int64_t begin)
{
  int ret = OB_SUCCESS;
  if (!is_inited_) {
    ret = OB_NOT_INIT;
  } else if (OB_ISNULL(comp_)) {
    ret = OB_ERR_UNEXPECTED;
  } else if (begin < 0 || begin > rows_.count()) {
    ret = OB_INVALID_ARGUMENT;
  } else if (rows_.count() - begin > 1) {
    lib::ob_sort(&rows_.at(begin), &rows_.at(0) + rows_.count(), CopyableComparer(*comp_));
    if (OB_SUCCESS != comp_->get_error_code()) {
      ret = comp_->get_error_code();
    }
  }
  return ret;
}
...
int ObStorageVecSortImpl<Compare, Store_Row, has_addon>::sort_inmem_data(const bool need_force_dump)
{
  ...
  } else if (OB_FAIL(sort_strategy_->sort(*this, 0))) {
    SQL_ENG_LOG(WARN, "failed to sort in-memory data", K(ret));
  } else {
    if (0 != sort_chunks_.get_size() || need_force_dump) {
      ObArray<Store_Row *> ties_rows;
      ObNormalDumpStrategy<Store_Row, has_addon> input(rows_, ties_rows, *sk_row_meta_);
    ...
    const int64_t begin, const int64_t end)
{
  ...
  if (!is_inited_) {
    ...
    UNUSED(end);
    ret = sort_strategy_->sort(*this, begin);
  }
  return ret;
}
...
void ObStorageVecSortImpl<Compare, Store_Row, has_addon>::cleanup_sort_strategy()
{
  if (OB_NOT_NULL(sort_strategy_)) {
    ObIAllocator &allocator = mem_context_.ref_context()->get_malloc_allocator();
    sort_strategy_->~ObFullSortStrategy<ThisType>();
