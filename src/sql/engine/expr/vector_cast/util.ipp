...
struct DecintRangeChecker
{
  ...
  static int check(CAST_CHECKER_ARG_DECL)
  {
    ...
    if (OB_UNLIKELY(precision < OB_MIN_DECIMAL_PRECISION
...
struct StringRangeChecker
{
  ...
  static int check(CAST_CHECKER_ARG_DECL)
  {
    ...
    if (OB_ISNULL(session)) {
      ...
      if (ObVarcharType == out_type || ObCharType  == out_type) // ObVarcharType, ObCharType
      {
      ...
      if (res_datum.is_null()) {
        ...
        if (OB_SUCC(ret) && !has_result) {
          ...
          if (mysql_nopadding_opt && (accuracy_length >= res_vec->get_length(idx))) {
            ...
            if (accuracy_length < text_length) {
              int64_t acc_len = accuracy_length;
  ...
  {
    bool no_padding = (ObCharType != out_type)
