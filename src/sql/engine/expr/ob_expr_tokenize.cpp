...
// TOKENIZE is deterministic for a given parser configuration and dictionary
// generation.  A small direct-mapped cache avoids rebuilding the same JSON for
// repeated literals while keeping memory and synchronization strictly bounded.
struct ObTokenizeMemoKey
{
  ObTokenizeMemoKey()
      : fulltext_(), parser_name_(), collation_(CS_TYPE_INVALID), dict_epoch_(0), hash_(0)
  {}
...
public:
  ...
  {
    SLOT_COUNT = 16,
    MAX_FULLTEXT_LENGTH = 4096,
    MAX_PARSER_NAME_LENGTH = 128,
           ...
           bool &found)
  {
    ...
    if (slot.matches(key)) {
      ...
      if (slot.result_length_ > 0
          && OB_ISNULL(buffer = static_cast<char *>(allocator.alloc(slot.result_length_)))) {
        ...
        result.assign_ptr(buffer, slot.result_length_);
        found = true;
      }
    }
    return ret;
...
private:
  ...
  {
    Slot()
        : lock_(), hash_(0), collation_(CS_TYPE_INVALID), dict_epoch_(0),
          fulltext_length_(0), parser_name_length_(0), result_length_(0), valid_(false)
    {}
...
int prepare_memo_key(const ObExpr &expr,
                     ObEvalCtx &ctx,
                     ...
                     bool &memoizable)
{
  ...
  if (expr.arg_cnt_ > 2 || expr.args_[0]->obj_meta_.has_lob_header()) {
    // Parser properties and LOB locators require the regular evaluation path.
    ...
    // Keep NULL handling on the regular path.
    ...
    if (!is_memoizable_parser(key.parser_name_)) {
      // User-provided parsers can have state outside the dictionary hub.
...
int ObExprTokenize::eval_tokenize(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &expr_datum)
{
  ...
  if (OB_UNLIKELY(expr.arg_cnt_ < 1 || expr.arg_cnt_ > 3)) {
  ...
  } else if (OB_FAIL(prepare_memo_key(expr, ctx, memo_key, memoizable))) {
    LOG_WARN("fail to prepare tokenize memo key", K(ret));
  } else if (memoizable
      && OB_FAIL(ObTokenizeMemo::instance().find(
          memo_key, temp_allocator, memo_result, memo_hit))) {
    LOG_WARN("fail to find tokenize memo result", K(ret));
  } else if (memo_hit) {
    if (OB_FAIL(ObJsonExprHelper::pack_json_str_res(expr, ctx, expr_datum, memo_result))) {
      LOG_WARN("fail to pack memoized tokenize result", K(ret));
    }
    ...
    if (OB_FAIL(ObJsonWrapper::get_raw_binary(json_result, raw_binary, &temp_allocator))) {
      LOG_WARN("fail to serialize tokenize result for memo", K(ret));
    } else {
      ObTokenizeMemo::instance().store(memo_key, raw_binary);
