                                                ...
                                                ObIAllocator &allocator)
{
  ...
  for (int64_t i = 0; i < namespace_arr.count() && OB_SUCC(ret); i += 2) {
    if ((i + 1) >= namespace_arr.count()) {
      ...
      } else if (OB_FAIL(ob_write_string(allocator, namespace_arr.at(i + 1), prefix_str))) {
  ...
  if (src_obj.is_null()) {
    ...
    if (OB_ISNULL(session = ctx.exec_ctx_.get_my_session())) {
      ...
      if (OB_FAIL(ObObjCaster::to_type(obj_type, cs_type, cast_ctx, src_obj, dst_obj))) {
        ...
        if (!xt_need_acc_check && (ob_is_varchar_or_char(obj_type, cs_type)) && ret == OB_ERR_DATA_TOO_LONG) {
