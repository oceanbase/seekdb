  ...
  if (OB_FAIL(expr.eval_vector_param_value(ctx, skip, bound))) {
    ...
    for (int64_t idx = bound.start(); OB_SUCC(ret) && idx < bound.end(); ++idx) {
      ...
      if (OB_FAIL(ret)) {
        ...
        // seekdb lacks the idx-based string_to_templob_result wrapper; inline its body
        // using ObTextStringVectorResult (the vector-slot LOB result helper that seekdb has).
        ObTextStringVectorResult<ObIVector> blob_res(expr.datum_meta_.type_, &expr, &ctx, res_vec, idx);
        if (OB_FAIL(blob_res.init_with_batch_idx(file_data.length(), idx))) {
          LOG_WARN("fail to init lob result for vector", K(ret), K(idx), K(file_data.length()));
        } else if (OB_FAIL(blob_res.append(file_data.ptr(), file_data.length()))) {
          LOG_WARN("fail to append file data to vector lob", K(ret), K(idx), K(file_data.length()));
        } else {
          blob_res.set_result();
        }
      }
                                            ...
                                            ObString &file_data)
{
  ...
  // seekdb: local-file-only port — use the fixed default cap instead of the
  // document_ai_file_max_size tenant config (not present in seekdb).
  int64_t document_ai_file_max_size = ObExprLoadFile::DEFAULT_DOCUMENT_AI_FILE_MAX_SIZE;
