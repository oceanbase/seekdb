...
// Wrapper class to handle a SQL expression string/text datum result.
...
public:
  ObTextStringDatumResult(const ObObjType type, const ObExpr* expr, ObEvalCtx *ctx, ObDatum *res_datum) :
    common::ObTextStringResult(type, expr->obj_meta_.has_lob_header(), NULL),
    expr_(expr), ctx_(ctx), res_datum_(res_datum)
  {}
  ObTextStringDatumResult(const ObObjType type, bool has_header, ObDatum *res_datum) :
    common::ObTextStringResult(type, has_header, NULL),
    expr_(NULL), ctx_(NULL), res_datum_(res_datum)
  {}
