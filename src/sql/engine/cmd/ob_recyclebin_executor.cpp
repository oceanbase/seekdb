...
//  obcall::Int64 expire_time = current_time - GCONF.schema_history_expire_time;
  ...
  if (OB_FAIL(ret)) {
    ...
    while (OB_SUCC(ret) && !is_finished) {
      // Purge a bounded batch so the local DDL thread remains responsive.
      ...
      if (OB_FAIL(GSCHEMASERVICE.cal_purge_need_timeout(purge_recyclebin_arg, cal_timeout))) {
        ...
        is_finished = obcall::ObPurgeRecycleBinArg::DEFAULT_PURGE_EACH_TIME != affected_rows;
      ...
      LOG_INFO("purge recycle objects", KR(ret), K(cost_time), K(cal_timeout),
               K(total_purge_count), K(purge_recyclebin_arg), K(affected_rows), K(is_finished));
    }
