...
{ \
  ...
  const obcall::ObSequenceDDLArg &sequence_arg = stmt.get_arg(); \
  ObString first_stmt; \
  if (OB_FAIL(stmt.get_first_stmt(first_stmt))) { \
    LOG_WARN("fail to get first stmt" , K(ret)); \
  } else { \
    const_cast<obcall::ObSequenceDDLArg&>(sequence_arg).ddl_stmt_str_ = first_stmt; \
  } \
  if (OB_FAIL(ret)) { \
  } else if (OB_ISNULL(task_exec_ctx = GET_TASK_EXECUTOR_CTX(ctx))) { \
    ret = OB_NOT_INIT; \
    LOG_WARN("get task executor context failed"); \
  } else if (OB_FAIL(rootserver::serial_call([&]{ return GCTX.root_service_->func(sequence_arg); }))) { \
