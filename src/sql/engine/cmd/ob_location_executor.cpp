...
{
  int ret = OB_SUCCESS;
  const obcall::ObCreateLocationArg &create_location_arg = stmt.get_create_location_arg();
  const_cast<obcall::ObCreateLocationArg&>(create_location_arg).ddl_stmt_str_ = stmt.get_masked_sql();
  if (OB_FAIL(ret)) {
    // do nothing.
...
{
  int ret = OB_SUCCESS;
  const obcall::ObDropLocationArg &drop_location_arg = stmt.get_drop_location_arg();
  if (OB_ISNULL(ctx.get_stmt_factory()) || OB_ISNULL(ctx.get_stmt_factory()->get_query_ctx())) {
    ret = OB_ERR_UNEXPECTED;
    SQL_ENG_LOG(WARN, "query ctx is null", K(ret));
  } else {
    const_cast<obcall::ObDropLocationArg&>(drop_location_arg).ddl_stmt_str_ =
