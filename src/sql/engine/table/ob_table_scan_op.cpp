...
int ObTableScanOp::inner_open()
{
  ...
  if (OB_ISNULL(my_session = GET_MY_SESSION(ctx_))) {
    ...
    LOG_WARN("fail to cache fts output expressions", K(ret));
  } else {
...
int ObTableScanOp::inner_get_next_batch(const int64_t max_row_cnt)
{
  ...
  if (OB_UNLIKELY(rand_scan_processor_.use_rand_scan())) {
    ...
    if (OB_FAIL(inner_get_next_fts_index_batch(max_row_cnt))) {
      LOG_WARN("failed to get next fts ddl batch", K(ret), K(max_row_cnt));
    }
...
{
  int ret = OB_SUCCESS;
  const int64_t capacity = MIN(max_row_cnt, MY_SPEC.max_batch_size_);
  ...
  if (OB_UNLIKELY(capacity <= 0)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid fts ddl batch capacity", K(ret), K(max_row_cnt), K(MY_SPEC.max_batch_size_));
  }

  // A returned batch never crosses a source-document boundary. Token strings
  // may point into the row cache, which is recycled for the next document.
  while (OB_SUCC(ret) && token_count < capacity) {
    blocksstable::ObDatumRow *token_row = nullptr;
    if (OB_FAIL(fts_index_.get_next_row(token_row))) {
      if (OB_ITER_END != ret) {
        LOG_WARN("failed to read cached fts token", K(ret));
      } else if (token_count > 0) {
        ret = OB_SUCCESS;
        break;
      } else {
        ret = OB_SUCCESS;
        if (OB_FAIL(prepare_next_fts_document())) {
          if (OB_ITER_END != ret) {
            LOG_WARN("failed to prepare next fts document", K(ret));
          }
        } else if (iter_end_) {
          break;
        } else if (OB_FAIL(fts_index_.get_next_row(token_row))) {
          if (OB_ITER_END == ret) {
            ret = OB_SUCCESS;
            continue;
          } else {
            LOG_WARN("failed to read first token of fts document", K(ret));
          }
        }
      }
    }

    if (OB_SUCC(ret) && OB_NOT_NULL(token_row)) {
      ObEvalCtx::BatchInfoScopeGuard guard(eval_ctx_);
      guard.set_batch_idx(token_count);
      guard.set_batch_size(capacity);
      if (OB_FAIL(fill_generated_fts_cols(token_row))) {
        LOG_WARN("failed to materialize fts token", K(ret), K(token_count));
      } else {
        ++token_count;
      }
    }
  ...
  if (OB_SUCC(ret) && OB_FAIL(replicate_fts_input_columns(token_count))) {
    LOG_WARN("failed to replicate fts input columns", K(ret), K(token_count));
  } else if (OB_SUCC(ret)) {
    brs_.size_ = token_count;
    brs_.end_ = iter_end_ && 0 == token_count;
    brs_.reset_skip(token_count);
    brs_.all_rows_active_ = true;
  }
  return ret;
}

int ObTableScanOp::prepare_next_fts_document()
{
  int ret = OB_SUCCESS;
  const int64_t word_idx = MY_SPEC.is_fts_index_aux_ ? 0 : 1;
  const int64_t doc_id_idx = MY_SPEC.is_fts_index_aux_ ? 1 : 0;
  bool has_tokens = false;
  while (OB_SUCC(ret) && !iter_end_ && !has_tokens) {
    if (OB_FAIL(inner_get_next_batch_for_tsc(1))) {
      LOG_WARN("failed to fetch fts source document", K(ret));
    } else if (iter_end_) {
      // Source scan is exhausted.
    } else if (OB_UNLIKELY(brs_.size_ != 1)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("unexpected fts source batch size", K(ret), K(brs_.size_));
    } else if (OB_NOT_NULL(brs_.skip_) && brs_.skip_->at(0)) {
      // Fetch another source row if this one was filtered.
    } else {
      ...
      if (OB_ISNULL(ft_expr) || OB_ISNULL(doc_id_expr)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("fts source expressions are not initialized", K(ret), KP(ft_expr), KP(doc_id_expr));
      } else if (OB_FAIL(ft_expr->eval(eval_ctx_, ft_datum))) {
        LOG_WARN("failed to evaluate fts source text", K(ret));
      } else if (OB_FAIL(doc_id_expr->eval(eval_ctx_, doc_id_datum))) {
        LOG_WARN("failed to evaluate fts source doc id", K(ret));
      } else if (OB_ISNULL(ft_datum) || OB_ISNULL(doc_id_datum)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("fts source datum is null", K(ret), KP(ft_datum), KP(doc_id_datum));
      } else {
        ...
        if (OB_FAIL(ObTextStringHelper::read_real_string_data(fts_lob_allocator_,
                                                              *ft_datum,
                                                              ...
                                                              fulltext))) {
          LOG_WARN("failed to materialize fts source text", K(ret));
        } else if (OB_FAIL(fts_index_.segment(ft_expr->obj_meta_, *doc_id_datum, fulltext))) {
          if (OB_ITER_END == ret) {
            ret = OB_SUCCESS;
          } else {
            LOG_WARN("failed to segment fts source document", K(ret));
          }
        } else {
          has_tokens = true;
        }
...
int ObTableScanOp::replicate_fts_input_columns(const int64_t batch_size)
{
  int ret = OB_SUCCESS;
  for (int64_t i = 0; OB_SUCC(ret) && batch_size > 0 && i < MY_SPEC.output_.count(); ++i) {
    ObExpr *expr = MY_SPEC.output_.at(i);
    if (OB_ISNULL(expr)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("fts output expression is null", K(ret), K(i));
      ...
      // Generated columns were written independently for every token above.
    } else {
      ObDatum *first_datum = nullptr;
      ObEvalCtx::BatchInfoScopeGuard guard(eval_ctx_);
      guard.set_batch_idx(0);
      guard.set_batch_size(batch_size);
      if (OB_FAIL(expr->eval(eval_ctx_, first_datum))) {
        LOG_WARN("failed to evaluate fts input expression", K(ret), K(i));
      } else if (OB_ISNULL(first_datum)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("fts input datum is null", K(ret), K(i));
      } else {
        ObDatum *datums = expr->locate_batch_datums(eval_ctx_);
        for (int64_t row_idx = 1; row_idx < batch_size; ++row_idx) {
          datums[row_idx] = datums[0];
        }
        expr->set_evaluated_projected(eval_ctx_);
      }
    }
...
int ObTableScanOp::fetch_next_fts_index_rows()
{
  ...
  while (OB_SUCC(ret) && !has_segment_word) {
    ...
    } else if (OB_ISNULL(ft_expr = fts_output_exprs_[word_idx])
               || OB_ISNULL(doc_id_expr = fts_output_exprs_[doc_id_idx])) {
      ret = OB_ERR_UNEXPECTED;
...
int ObTableScanOp::init_fts_output_exprs()
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < share::ObFtsIndexBuilderUtil::OB_FTS_INDEX_OR_DOC_WORD_TABLE_COL_CNT; ++i) {
    if (OB_FAIL(get_output_fts_col_expr_by_type(expr_types[i], fts_output_exprs_[i]))) {
      LOG_WARN("failed to locate fts output expression", K(ret), K(i), K(expr_types[i]));
    }
  }
  return ret;
}
...
int ObTableScanOp::fill_generated_fts_cols(blocksstable::ObDatumRow *row)
{
  ...
  if (OB_ISNULL(row)) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < share::ObFtsIndexBuilderUtil::OB_FTS_INDEX_OR_DOC_WORD_TABLE_COL_CNT; ++i) {
      ...
      if (OB_ISNULL(expr)) {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("fts output expression is not initialized", K(ret), K(i), K(expr_types[i]));
      } else {
