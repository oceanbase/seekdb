...
int ObTableScanOp::inner_get_next_fts_index_batch(const int64_t max_row_cnt)
{
  ...
  // Keep one source document in each output batch so its source datums remain
  // stable while downstream vector operators consume a dense token batch.
  ...
  if (batch_size > 0) {
    for (int64_t i = 0; OB_SUCC(ret) && i < MY_SPEC.output_.count(); ++i) {
      ...
      if (OB_ISNULL(expr)) {
        ...
        // Generated columns have already been filled token by token.
      } else {
