...
int ObContinuousVector<BasicOp>::default_hash(BATCH_EVAL_HASH_ARGS) const
{
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::murmur_hash(BATCH_EVAL_HASH_ARGS) const {
  BatchHashResIter hash_iter(hash_values);
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::murmur_hash_v3(BATCH_EVAL_HASH_ARGS) const
{
  BatchHashResIter hash_iter(hash_values);
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::murmur_hash_v3_for_one_row(EVAL_HASH_ARGS_FOR_ROW) const
{
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::null_first_cmp(VECTOR_ONE_COMPARE_ARGS) const
{
  return VecOpUtil::template ns_cmp<true>(expr.obj_meta_, *this, row_idx, r_null, r_v, r_len, cmp_ret);
}

template<typename BasicOp>
int ObContinuousVector<BasicOp>::null_last_cmp(VECTOR_ONE_COMPARE_ARGS) const
{
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::null_first_mul_cmp(VECTOR_MUL_COMPARE_ARGS) const
{
  int ret = OB_SUCCESS;
  cmp_ret = 0;
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::null_last_mul_cmp(VECTOR_MUL_COMPARE_ARGS) const
{
  int ret = OB_SUCCESS;
  cmp_ret = 0;
  uint16_t start_idx = bound.start();
...
template<typename BasicOp>
int ObContinuousVector<BasicOp>::null_first_cmp_batch_rows(VECTOR_COMPARE_BATCH_ROWS_ARGS) const
{
  int ret = OB_SUCCESS;
  ObLength r_len = 0;
  const char *r_v = NULL;
...
template <typename BasicOp>
int ObContinuousVector<BasicOp>::no_null_cmp_batch_rows(VECTOR_COMPARE_BATCH_ROWS_ARGS) const
{
  int ret = OB_SUCCESS;
  ObLength r_len = 0;
  const char *r_v = NULL;
...
template class ObContinuousVector<VectorBasicOp<VEC_TC_ENUM_SET_INNER>>;
template class ObContinuousVector<VectorBasicOp<VEC_TC_RAW>>;
template class ObContinuousVector<VectorBasicOp<VEC_TC_ROWID>>;
template class ObContinuousVector<VectorBasicOp<VEC_TC_LOB>>;
