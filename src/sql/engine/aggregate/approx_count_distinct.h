...
#include "sql/engine/aggregate/iaggregate.h"
...
namespace aggregate
{
static const constexpr int8_t APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS = 10;
static const constexpr int64_t APPROX_COUNT_DISTINCT_LLC_NUM_BUCKETS =
  (1 << APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS);

OB_INLINE int approx_count_distinct_llc_add_value(const uint64_t value, char *llc_bitmap_buf,
                                                  int64_t size)
{
  // copy from `ObAggregateProcessor::llv_add_value`
  int ret = OB_SUCCESS;
  uint64_t bucket_index = value >> (64 - APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS);
  uint64_t pmax = 0;
  if (0 == value << APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS) {
    // do nothing
  } else {
    pmax = ObExprEstimateNdv::llc_leading_zeros(
             value << APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS,
             64 - APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS) +
           1;
  }
  ObString::obstr_size_t llc_num_buckets = size;
  OB_ASSERT(size == APPROX_COUNT_DISTINCT_LLC_NUM_BUCKETS);
...
OB_INLINE bool approx_count_distinct_is_supported_vec_tc(const VecValueTypeClass vec_tc)
{
#define APPROX_COUNT_DISTINCT_VEC_TC_CASE(tc)                                                      \
  case (tc): {                                                                                     \
    return true;                                                                                   \
  }
  switch (vec_tc) {
    LST_DO_CODE(APPROX_COUNT_DISTINCT_VEC_TC_CASE, AGG_VEC_TC_LIST);
    default: {
      return false;
    }
  }
#undef APPROX_COUNT_DISTINCT_VEC_TC_CASE
}
...
{
public:
  static const constexpr int8_t LLC_BUCKET_BITS = APPROX_COUNT_DISTINCT_LLC_BUCKET_BITS;
  static const constexpr int64_t LLC_NUM_BUCKETS = APPROX_COUNT_DISTINCT_LLC_NUM_BUCKETS;
public:
  static const constexpr VecValueTypeClass IN_TC = in_tc;
  static const constexpr VecValueTypeClass OUT_TC = out_tc;
public:
  ApproxCountDistinct(): runtime_in_tc_(in_tc) {}
                  ...
                  ObIAllocator &allocator) override
  {
    ...
    if (!aggr_info.param_exprs_.empty() && OB_NOT_NULL(aggr_info.param_exprs_.at(0))) {
      ObDatumMeta &param_meta = aggr_info.param_exprs_.at(0)->datum_meta_;
      runtime_in_tc_ = get_vec_value_tc(param_meta.type_, param_meta.scale_, param_meta.precision_);
    }
    return OB_SUCCESS;
  }
  ...
  {
    int ret = OB_SUCCESS;
    if constexpr (agg_func == T_FUN_APPROX_COUNT_DISTINCT_SYNOPSIS_MERGE) {
      ...
      if (OB_ISNULL(tmp_res)) {
        ...
        } else if (OB_FAIL(approx_count_distinct_llc_add_value(
                     hash_val, reinterpret_cast<char *>(tmp_res), LLC_NUM_BUCKETS))) {
                           ...
                           const int32_t agg_cell_len)
  {
    ...
    if (OB_FAIL(ret)) {
    } else if constexpr (agg_func == T_FUN_APPROX_COUNT_DISTINCT_SYNOPSIS_MERGE) {
      ...
      ret = OB_ERR_UNEXPECTED;
      SQL_LOG(WARN , "invalid null llc bitmap", K(ret));
    } else if constexpr (agg_func == T_FUN_APPROX_COUNT_DISTINCT_SYNOPSIS) {
      char *res_buf = agg_expr.get_str_res_mem(agg_ctx.eval_ctx_, LLC_NUM_BUCKETS);
      if (OB_ISNULL(res_buf)) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        SQL_LOG(WARN, "allocate memory failed", K(ret));
    ...
    if (OB_FAIL(ret)) {
      ...
      if (row_sel.is_empty()) {
        for (int i = bound.start(); OB_SUCC(ret) && i < bound.end(); i++) {
          ...
          if (OB_SUCC(ret)) {
            if (OB_FAIL(approx_count_distinct_llc_add_value(hash_val, llc_bitmap_buf,
                                                            LLC_NUM_BUCKETS))) {
                  ...
                  char *agg_cell) override
  {
    ...
    if (OB_FAIL(ret)) {
        ...
        if (OB_LIKELY(!is_null)) {
          ...
          if (OB_FAIL(hash_func(tmp_datum, hash_val, hash_val))) {
            SQL_LOG(WARN, "hash calculation failed", K(ret));
                       ...
                       hash_val, reinterpret_cast<char *>(llc_bitmap_buf), LLC_NUM_BUCKETS))) {
            SQL_LOG(WARN, "llc add value failed", K(ret));
          } else {
      ...
      if (!has_null) {
        ...
        if (OB_SUCC(ret)) {
          if (OB_FAIL(approx_count_distinct_llc_add_value(hash_val, (char *)llc_bitmap_buf,
                                                          LLC_NUM_BUCKETS))) {
            SQL_LOG(WARN, "llc add value failed", K(ret));
          } else {
            agg_ctx.locate_notnulls_bitmap(agg_col_idx, agg_cell).set(agg_col_idx);
  ...
  inline void *get_tmp_res(RuntimeContext &agg_ctx, int32_t agg_col_id, char *agg_cell) override
  {
    ...
    if constexpr (agg_func == T_FUN_APPROX_COUNT_DISTINCT) {
  ...
  TO_STRING_KV("aggregate", "approx_count_distinct", K_(runtime_in_tc), K(out_tc), K(agg_func));
private:
  VecValueTypeClass runtime_in_tc_;
};
