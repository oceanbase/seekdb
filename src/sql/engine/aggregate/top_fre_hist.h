...
namespace aggregate
{
inline constexpr bool is_top_fre_hist_lob_vec_tc(const VecValueTypeClass vec_tc)
{
...
OB_INLINE bool is_top_fre_hist_supported_vec_tc(const VecValueTypeClass vec_tc)
{
#define TOP_FRE_HIST_VEC_TC_CASE(tc)                                                              \
  case (tc): {                                                                                    \
    return true;                                                                                  \
  }
  switch (vec_tc) {
    LST_DO_CODE(TOP_FRE_HIST_VEC_TC_CASE, AGG_VEC_TC_LIST);
    default: {
      return false;
    }
  }
#undef TOP_FRE_HIST_VEC_TC_CASE
}
...
public:
  TopFreHist(): runtime_in_tc_(in_tc) {}
                                         ...
                                         int32_t len)
  {
    ...
    } else if constexpr (is_lob_vec_tc()) {
    ...
    } else if constexpr (in_tc == VEC_TC_STRING) {
                         ...
                         int32_t agg_col_idx, char *agg_cell) override
  {
    ...
    if (OB_LIKELY(agg_ctx.aggr_infos_.at(agg_col_idx).param_exprs_.count() == 1)) {
      HashFuncTypeForTc hash_func = get_hashfunc_by_tc(runtime_in_tc_);
      ...
      if (OB_LIKELY(!is_null)) {
        if (OB_FAIL(add_top_k_frequency_item(agg_ctx, 
                                             ...
                                             data_len))) {
          SQL_LOG(WARN, "failed to add top k frequency item", K(ret));
        } else if (!is_merge && OB_FAIL(extra->flush_batch_rows())) {
          SQL_LOG(WARN, "failed to flush batch rows", K(ret));
  ...
  static constexpr bool is_lob_vec_tc()
  {
    return is_top_fre_hist_lob_vec_tc(in_tc);
  }
  ...
  TO_STRING_KV("aggregate", "top_fre_hist", K_(runtime_in_tc));
private:
