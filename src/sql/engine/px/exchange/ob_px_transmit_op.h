...
int ObPxTransmitOp::send_rows_one_by_one(ObSliceIdxCalc &slice_calc)
{
  ...
  while (OB_SUCC(ret)) {
    ...
    } else if (OB_FAIL((slice_calc.get_slice_indexes<CALC_TYPE>(
  ...
  while (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret) || brs_.size_ <= 0) {
      ...
      if (OB_FAIL((slice_calc.get_slice_idx_batch<CALC_TYPE>(spec_.output_, eval_ctx_,
        ...
        LOG_DEBUG("send rows batch with prefetch", K(CALC_TYPE),
...
inline int ObPxTransmitOp::broadcast_rows(ObSliceIdxCalc &slice_calc)
{
  int ret = OB_SUCCESS;
  UNUSED(slice_calc);
  int64_t row_count = 0;
  ...
  while (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      ObPxNewRow px_row(get_spec().output_, data_msg_type_);
      ObEvalCtx::BatchInfoScopeGuard batch_info_guard(eval_ctx_);
