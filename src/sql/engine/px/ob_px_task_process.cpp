...
ObPxTaskProcess::ObPxTaskProcess(const observer::ObGlobalContext &gctx, ObPxInitTaskArgs &arg)
  : gctx_(gctx), arg_(arg),
...
int ObPxTaskProcess::execute(const ObOpSpec &root_spec)
{
  int ret = OB_SUCCESS;
...
int ObPxTaskProcess::do_process()
{
  ...
  if (OB_INVALID_ID == task_id) {
    ...
    //   - reference sql/executor/ob_worker_session_guard.cpp
    //   - exec, stat etc.
    ...
    if (OB_SUCC(ret)) {
      if (nullptr != arg_.op_spec_root_) {
        ...
        LOG_DEBUG("receive monitoring information",
                  K(arg_.sqc_handler_->get_sqc_init_arg().sqc_.get_monitoring_info()));
  ...
  if (NULL != arg_.sqc_task_ptr_) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCCESS == ERRSIM_INTERRUPT_QC_FAILED) {
        if (OB_SUCCESS != (tmp_ret = ObInterruptUtil::interrupt_qc(arg_.task_, ret))) {
      ...
      for (uint32_t idx = 0; OB_SUCC(ret) && not_null && idx < wb->get_readable_warning_count(); idx++) {
        ...
        if (item != NULL) {
          if (OB_FAIL(rcode.warnings_.push_back(*item))) {
            LOG_WARN("failed to add warning", K(ret));
          }
        } else {
          not_null = false;
        }
...
int ObPxTaskProcess::OpPreparation::reset(const ObOpSpec &op)
{
  int ret = OB_SUCCESS;
...
int ObPxTaskProcess::OpPostparation::apply(ObExecContext &ctx, const ObOpSpec &op)
{
  int ret = OB_SUCCESS;
  ObOperatorKit *kit = ctx.get_operator_kit(op.id_);
  if (OB_ISNULL(kit) || OB_ISNULL(kit->op_)) {
...
int ObPxTaskProcess::OpPostparation::reset(const ObOpSpec &op)
{
  int ret = OB_SUCCESS;
  UNUSED(op);
  return ret;
