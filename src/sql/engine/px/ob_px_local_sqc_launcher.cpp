...
int ObLocalSqcLauncher::decode_arg(const char *buf, const int64_t len, int64_t &pos)
{
  return common::serialization::decode(buf, len, pos, arg_);
}

void ObLocalSqcLauncher::destroy()
{
  /**
...
int ObLocalSqcLauncher::process()
{
  ...
  // The asynchronous launcher returns execution errors through result_.rc_.
  result_.rc_ = ret;
...
int ObLocalSqcLauncher::pre_setup_op_input(ObPxSqcHandler &sqc_handler)
{
  int ret = OB_SUCCESS;
...
int ObLocalSqcLauncher::startup_normal_sqc(ObPxSqcHandler &sqc_handler)
{
  int ret = OB_SUCCESS;
...
int ObLocalSqcLauncher::after_process(int error_code)
{
  int ret = OB_SUCCESS;
  UNUSED(error_code);
   ...
   * the number of workers plus the launcher. release_handler drops the launcher's reference,
   * and the last worker holding a reference
...
void ObLocalSqcFailureReporter::operator()(hash::HashMapPair<ObInterruptibleTaskID,
      ObInterruptCheckerNode *> &entry)
{
  ...
  if (OB_NOT_NULL(sqc_)) {
    if (sqc_->is_ignore_vtable_error() && err_ != OB_SUCCESS
      ...
      // Virtual-table SQC launch failures covered by the whitelist are ignored.
      ...
      sqc_->set_thread_finish(true);
    }
  }
}

int ObLocalSqcFailureReporter::mock_sqc_finish_msg()
{
  int ret = OB_SUCCESS;
  if (OB_NOT_NULL(sqc_)) {
...
int ObLocalFastSqcLauncher::decode_arg(const char *buf, const int64_t len, int64_t &pos)
{
  return common::serialization::decode(buf, len, pos, arg_);
}

void ObLocalFastSqcLauncher::destroy()
{
  /**
   * If the after process flow has been undergone, arg_.sqc_handler_ will be set to null.
   * If arg_.sqc_handler_ is not null here, it means that the after process flow has not been
...
int ObLocalFastSqcLauncher::startup_normal_sqc(ObPxSqcHandler &sqc_handler)
{
  int ret = OB_SUCCESS;
  int64_t dispatched_worker_count = 0;
  ObSQLSessionInfo *session = sqc_handler.get_exec_ctx().get_my_session();
...
// ============================================================================
// In-process SQC launch drivers.
// They run the processor lifecycle on a fresh local worker context:
