...
namespace share
{
// ob_external_table_utils.cpp deleted upstream; re-home the forwarding impl using the
// surviving ObTableSchema::get_external_file_location() (deep-copy into caller allocator).
...
{
  UNUSED(schema_guard);
  return common::ob_write_string(allocator, table_schema.get_external_file_location(), file_location);
}
}
}
