...
#ifndef OB_DTL_MEMORY_MANAGER_H
#define OB_DTL_MEMORY_MANAGER_H
...
class ObDtlMemoryManager
{
public:
  ObDtlMemoryManager();
  virtual ~ObDtlMemoryManager() { destroy(); }
...
OB_INLINE int64_t ObDtlMemoryManager::hash(int64_t chid)
{
  uint64_t val = common::murmurhash(&chid, sizeof(chid), 0);
  return val % hash_cnt_;
}

OB_INLINE int64_t ObDtlMemoryManager::hash(int64_t chid, int64_t ratio)
{
  uint64_t val = common::murmurhash(&chid, sizeof(chid), 0);
