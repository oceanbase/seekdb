...
class ObTaskLocation final
{
  OB_UNIS_VERSION(1);
public:
  explicit ObTaskLocation(const ObTaskID &ob_task_id);
  ObTaskLocation();
  ...
  inline bool is_valid() const {return ob_task_id_.is_valid();}
  inline int64_t hash() const {return ob_task_id_.hash();}
  inline bool equal(const ObTaskLocation &task_loc) const
  {
    return task_loc.ob_task_id_.equal(ob_task_id_);
  }
