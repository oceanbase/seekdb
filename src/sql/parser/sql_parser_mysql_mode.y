...
//-----------------------------non_reserved keyword begin-------------------------------------------
        ...
        JOB JSON JSON_ARRAYAGG JSON_OBJECTAGG JSON_QUERY JSON_VALUE JSON_TABLE AI_SPLIT_DOCUMENT
...
{
  (void)($2);
  // 标记当前表为全文词典表，具体合法性由 DDL resolver 校验。
...
{
  (void)($1);
  // 保留词典表 relation，供 resolver 校验目标并触发指定词典缓存失效。
