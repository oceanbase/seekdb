...
{
  (void)($1);
  ParseNode *db_node = $6->children_[0];
  ParseNode *tb_node = $6->children_[1];
  ParseNode *dict_relation = NULL;
  malloc_non_terminal_node(dict_relation, result->malloc_pool_, T_RELATION_FACTOR, 2, db_node, tb_node);
  malloc_non_terminal_node($$, result->malloc_pool_, T_REFRESH_FULLTEXT_DICT, 1, dict_relation);
}
|
