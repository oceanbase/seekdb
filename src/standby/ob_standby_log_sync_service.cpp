...
{
  return instance_().wait_startup_replay_();
}
...
int ObStandbyLogSyncService::wait_startup_replay_()
{
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(get_local_progress(end_scn, sync_scn))) {
      ...
      break;
    } else if (common::ObTimeUtility::current_time() - start_us >= STARTUP_TIMEOUT_US) {
      ret = OB_TIMEOUT;
      LOG_WARN("wait standby startup replay timed out", KR(ret),
          K(target_scn), K(end_scn), K(sync_scn));
    } else {
    ...
    const share::SCN &source_scn)
{
  ...
  } else if (OB_FAIL(log_handler->append_imported_group(buf, size))) {
    ...
    bool &made_progress)
{
  ...
  ObStandbyGrpcClient client;
  made_progress = false;
  if (fatal_error_ != OB_SUCCESS) {
    ret = fatal_error_;
  } else if (OB_FAIL(get_local_progress(start_scn, replay_scn))) {
    LOG_WARN("failed to get local standby log progress", KR(ret));
    ...
    LOG_WARN("failed to fetch standby logs", KR(ret), K(source_addr), K(start_scn));
  }
  return ret;
}

int ObStandbyLogSyncService::wait_local_replay_()
{
  int ret = OB_SUCCESS;
  const int64_t start_us = common::ObTimeUtility::current_time();
  share::SCN end_scn;
  ...
  while (OB_SUCC(ret)) {
    if (OB_FAIL(get_local_progress(end_scn, sync_scn))) {
      ...
      break;
    } else if (common::ObTimeUtility::current_time() - start_us >= SWITCH_TIMEOUT_US) {
      ret = OB_TIMEOUT;
...
int ObStandbyLogSyncService::prepare_switch_to_primary_(const bool is_failover)
{
  ...
  if (OB_FAIL(guard.get_ret())) {
    ...
    ret = fatal_error_;
  } else if (!is_failover && OB_FAIL(get_source_addr_(source_addr))) {
    LOG_WARN("lossless switchover requires a valid log source", KR(ret));
  } else if (!is_failover && OB_FAIL(query_source_end_scn_(source_addr, target_scn))) {
  ...
  if (OB_SUCC(ret) && OB_FAIL(wait_local_replay_())) {
...
void ObStandbyLogSyncService::runTimerTask()
{
  ...
  if (OB_FAIL(guard.get_ret())) {
    LOG_WARN("failed to lock standby log sync service", KR(ret));
  } else if (!is_inited_ || paused_ || fatal_error_ != OB_SUCCESS) {
  } else if (OB_FAIL(get_source_addr_(source_addr))) {
    ...
    if (REACH_TIME_INTERVAL(10 * 1000 * 1000L)) {
      LOG_WARN("standby log sync round failed", KR(ret), K(source_addr), K_(fatal_error));
    }
  }
}
