...
int ObStandbyRestoreHelper::check_restore_precondition()
{
  ...
  if (!is_valid()) {
    ...
    if (OB_FAIL(grpc_client.init(src_, RPC_TIMEOUT_US, config_->rpc_tls_enabled_))) {
      ...
      const uint64_t local_data_version = DATA_CURRENT_VERSION;
      if (result.data_version_ != local_data_version) {
        ret = OB_NOT_SUPPORTED;
        LOG_ERROR("data version mismatch between primary and standby, cannot restore",
            KR(ret), "primary_version", result.data_version_, "standby_version", local_data_version);
      } else {
...
{
  int ret = OB_SUCCESS;
  standby::ObStandbyLSViewTabletCountResult tablet_count_result;
  standby::ObStandbyGrpcClient grpc_client;
...
int ObStandbyRestoreHelper::fetch_ls_meta(
    ObLSMeta &ls_meta,
...
int ObStandbyRestoreHelper::fetch_next_tablet_info(obcall::ObCopyTabletInfo &tablet_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (!ls_view_ctx->ls_meta_fetched_) {
      ...
      standbyservice::FetchLSViewRes response;
...
int ObStandbyRestoreHelper::fetch_tablet_meta(obcall::ObCopyTabletInfo &tablet_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_ISNULL(tablet_info_ctx->grpc_client_) || !tablet_info_ctx->tablet_info_reader_) {
      ...
      standbyservice::FetchTabletInfoRes response;
    ...
    int64_t &max_snapshot_version)
{
  ...
  {
    for (int64_t i = 0; OB_SUCC(ret) && i < major_sstable_array.count(); ++i) {
      const ObITable *table = major_sstable_array.at(i);
      if (OB_ISNULL(table)) {
        ret = OB_ERR_UNEXPECTED;
    ...
    if (ddl_start_scn > ddl_checkpoint_scn) {
      ...
      if (!ddl_sstable_array.empty()) {
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < ddl_sstable_array.count(); ++i) {
          const ObITable *table = ddl_sstable_array.at(i);
        ...
        if (OB_SUCC(ret)) {
          need_copy_scn_range.end_scn_ = min_start_scn;
        }
      } else {
...
int ObStandbyRestoreHelper::fetch_next_tablet_sstable_header(obcall::ObCopyTabletSSTableHeader &copy_header)
{
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(sstable_info_ctx->grpc_client_) || !sstable_info_ctx->sstable_info_reader_) {
      ...
      standbyservice::FetchTabletSSTableInfoRes response;
      if (!sstable_info_ctx->sstable_info_reader_->Read(&response)) {
        ...
        ret = OB_INVALID_ARGUMENT;
        LOG_ERROR("source returned invalid copy tablet sstable header", K(ret), K(copy_header));
      } else {
...
int ObStandbyRestoreHelper::fetch_next_sstable_macro_range_info(storage::ObCopySSTableMacroRangeInfo &sstable_macro_range_info)
{
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(macro_range_ctx->grpc_client_) || !macro_range_ctx->macro_info_reader_) {
      ...
      standbyservice::FetchSSTableMacroInfoRes response;
    ...
    const int64_t data_version)
{
  ...
  if (!is_valid()) {
    ...
    if (OB_FAIL(arg.copy_macro_range_info_.assign(macro_range_info))) {
      ...
      LOG_INFO("init standby macro block copy stream", K(arg), K_(src));
    }
...
int ObStandbyRestoreHelper::fetch_next_macro_block(storage::ObICopyMacroBlockReader::CopyMacroBlockReadData &read_data)
{
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(macro_block_ctx->grpc_client_) || !macro_block_ctx->macro_block_reader_) {
      ...
      if (OB_ITER_END != ret) {
        LOG_ERROR("failed to fetch macro block header", K(ret), K(macro_block_ctx->copy_table_key_));
      }
    } else if (OB_FAIL(fetch_macro_block_data_(macro_block_ctx, header, data_reader))) {
      LOG_ERROR("failed to fetch macro block data", K(ret), K(header), K(macro_block_ctx->copy_table_key_));
      ...
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("standby baseline requires physical macro block data", K(ret), K(header.data_type_));
    }
  }
  return ret;
}
    ...
    obcall::ObCopyMacroBlockHeader &header)
{
  ...
  if (OB_ISNULL(macro_block_ctx) || !macro_block_ctx->macro_block_reader_) {
    ...
    standbyservice::FetchMacroBlockRes response;
    if (!macro_block_ctx->macro_block_reader_->Read(&response)) {
      if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(
                      macro_block_ctx->macro_block_reader_, macro_block_ctx->grpc_client_))) {
        LOG_ERROR("failed to close macro block reader", K(ret), K(macro_block_ctx->copy_table_key_));
      } else {
        ret = OB_ITER_END;
        LOG_INFO("macro block stream reached end", K(macro_block_ctx->copy_table_key_));
      }
    } else if (OB_FAIL(obgrpc::deserialize_proto_to_ob(response, header))) {
      LOG_ERROR("failed to deserialize ObCopyMacroBlockHeader", K(ret),
          "payload_size", response.size(), "buf_size", response.buf().size());
    } else {
      LOG_INFO("fetched macro block header", K(header));
    }
  }
  ...
  if (OB_ISNULL(macro_block_ctx) || !macro_block_ctx->macro_block_reader_) {
    ...
    if (occupy_size > macro_block_ctx->data_buffer_.capacity()) {
      ...
      if (!macro_block_ctx->macro_block_reader_->Read(&response)) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_ERROR("data size mismatch", K(ret), "expected_size", occupy_size, "actual_size", response.size());
      } else {
