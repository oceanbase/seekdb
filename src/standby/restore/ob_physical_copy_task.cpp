    ...
    const int64_t data_version)
{
  ...
  if (OB_ISNULL(proto_helper) || !table_key.is_valid() || !range_info.is_valid()) {
    ...
    LOG_WARN("task helper should not be null or invalid", K(ret), KP(helper_));
      ...
      table_key, range_info, backfill_tx_scn, data_version))) {
...
int ObPhysicalCopyTask::ObCopyMacroBlockHelperReader::get_next_macro_block(CopyMacroBlockReadData &read_data)
{
  ...
  if (IS_NOT_INIT || OB_ISNULL(helper_)) {
    ...
    LOG_ERROR("failed to fetch next macro block", K(ret), KP_(helper), K(read_data));
  }
  return ret;
}
  ...
  if (!is_inited_) {
    ...
    if (OB_SUCC(ret) && copy_macro_range_info_->macro_block_count_ > 0) {
      if (OB_FAIL(fetch_macro_block_with_retry_(copied_ctx))) {
        LOG_ERROR("failed to fetch major block", K(ret), K(copy_table_key_),
            KPC(copy_macro_range_info_), KPC(copy_ctx_));
  ...
  if (!is_inited_) {
    ...
    while (retry_times < MAX_RETRY_TIMES) {
      ...
      if (OB_FAIL(fetch_macro_block_(retry_times, copied_ctx))) {
        STORAGE_LOG(ERROR, "failed to fetch major block", K(ret), K(retry_times),
            K(copy_table_key_), KPC(copy_macro_range_info_), KPC(copy_ctx_));
      }
...
{
  int ret = OB_SUCCESS;
  ObStandbyRestoreMacroBlockWriter *writer = NULL;
  ...
  if (!is_inited_) {
    ...
    LOG_INFO("init reader", K(copy_table_key_), KPC(copy_macro_range_info_), KPC(copy_ctx_));
    ...
    ObICopyMacroBlockReader *&reader)
{
  ...
  if (!is_inited_) {
    ...
    ObCopyMacroBlockHelperReader *tmp_reader = OB_NEW(ObCopyMacroBlockHelperReader, "HelperMacReader");
    if (OB_ISNULL(tmp_reader)) {
      ...
      const int64_t data_version =
          obcall::ObCopyMacroBlockRangeArg::DISABLE_MACRO_BLOCK_REUSE_DATA_VERSION;
      if (OB_FAIL(tmp_reader->init(copy_ctx_->helper_, copy_table_key_, *copy_macro_range_info_,
                                   backfill_tx_scn, data_version))) {
        LOG_WARN("failed to init helper macro reader", K(ret));
        OB_DELETE(ObCopyMacroBlockHelperReader, "HelperMacReader", tmp_reader);
        tmp_reader = nullptr;
      } else {
        reader = tmp_reader;
        tmp_reader = nullptr;
...
int ObPhysicalCopyTask::get_macro_block_writer_(
    ObICopyMacroBlockReader *reader,
    ObIndexBlockRebuilder *index_block_rebuilder,
    ObStandbyRestoreMacroBlockWriter *&writer)
{
  int ret = OB_SUCCESS;
  ObStandbyRestoreMacroBlockWriter *tmp_writer = nullptr;
  ...
  if (OB_ISNULL(reader) || OB_ISNULL(index_block_rebuilder)) {
    ...
    tmp_writer = OB_NEW(ObStandbyRestoreLocalMacroBlockWriter, "StandbyMacro");
...
void ObPhysicalCopyTask::free_macro_block_reader_(ObICopyMacroBlockReader *&reader)
{
  if (OB_NOT_NULL(reader)) {
    ObCopyMacroBlockHelperReader *helper_reader = static_cast<ObCopyMacroBlockHelperReader *>(reader);
    OB_DELETE(ObCopyMacroBlockHelperReader, "HelperMacReader", helper_reader);
    reader = nullptr;
  }
}

void ObPhysicalCopyTask::free_macro_block_writer_(ObStandbyRestoreMacroBlockWriter *&writer)
{
  if (OB_NOT_NULL(writer)) {
    ObStandbyRestoreLocalMacroBlockWriter *local_writer =
        static_cast<ObStandbyRestoreLocalMacroBlockWriter *>(writer);
    OB_DELETE(ObStandbyRestoreLocalMacroBlockWriter, "StandbyMacro", local_writer);
    writer = nullptr;
  }
}
