...
{
  ObMaxIdCacheItem cache(OB_MAX_USED_OBJECT_ID_TYPE);
  ObMySQLProxy proxy;
  ...
  ASSERT_SUCCESS(cache.fetch_max_id(OB_MAX_USED_OBJECT_ID_TYPE, id, 1, &proxy));
  ASSERT_EQ(id, 2);
  ASSERT_TRUE(hit);
  reset_id_size();
  ...
  ASSERT_SUCCESS(cache.fetch_max_id(
      OB_MAX_USED_OBJECT_ID_TYPE, id, ObMaxIdCacheItem::CACHE_SIZE - 1, &proxy));
  ...
  ASSERT_SUCCESS(cache.fetch_max_id(
      OB_MAX_USED_OBJECT_ID_TYPE, id, ObMaxIdCacheItem::CACHE_SIZE, &proxy));
