...
TEST(TestServerMemoryConfig, vector_limit_is_half_of_effective_memory)
{
  const int64_t physical_memory = 16 * ONE_GIB;
  const int64_t finite_cgroup_limit = 6 * ONE_GIB;
  const int64_t effective_memory_without_cgroup = physical_memory;
  const int64_t effective_memory_with_cgroup =
      MIN(physical_memory, finite_cgroup_limit);

  EXPECT_EQ(8 * ONE_GIB,
            ObServerMemoryConfig::resolve_vector_memory_limit(
                0, effective_memory_without_cgroup));
  EXPECT_EQ(3 * ONE_GIB,
            ObServerMemoryConfig::resolve_vector_memory_limit(
                0, effective_memory_with_cgroup));
  EXPECT_EQ(34567,
            ObServerMemoryConfig::resolve_vector_memory_limit(
                34567, effective_memory_with_cgroup));
}
...
TEST(TestServerMemoryConfig, reload_uses_explicit_memory_budget)
{
  ...
  const int64_t automatic_vector_memory_limit = lib::get_memory_by_percentage(
      get_effective_memory_size(), 50);
  ...
  EXPECT_EQ(automatic_vector_memory_limit, memory_config.get_vector_memory_limit());

  GCONF.memory_budget = 6 * ONE_GIB;
  ASSERT_EQ(OB_SUCCESS, memory_config.reload_config(GCONF));
  EXPECT_EQ(6 * ONE_GIB, memory_config.get_server_memory_budget());
  EXPECT_EQ(lib::get_memory_by_percentage(6 * ONE_GIB, 40),
            memory_config.get_kvcache_memory_limit());
  EXPECT_EQ(lib::get_memory_by_percentage(6 * ONE_GIB, 50),
            memory_config.get_memstore_memory_limit());
  EXPECT_EQ(automatic_vector_memory_limit, memory_config.get_vector_memory_limit());
