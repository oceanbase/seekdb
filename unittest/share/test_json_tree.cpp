...
TEST_F(TestJsonTree, test_add_double_quote)
{
  ...
  str.reset();
  str.assign_ptr("\b\t\n\f\r\"\\", static_cast<int64_t>(strlen("\b\t\n\f\r\"\\")));
  j_buf.reset();
  ASSERT_EQ(OB_SUCCESS, ObJsonBaseUtil::add_double_quote(j_buf, str.ptr(), str.length()));
  cout << "quote: " << j_buf.ptr() << endl;
  str.reset();
  str.assign_ptr("\a", static_cast<int64_t>(strlen("\a")));
  j_buf.reset();
  ASSERT_EQ(OB_SUCCESS, ObJsonBaseUtil::add_double_quote(j_buf, str.ptr(), str.length()));
  str.reset();
  cout << "quote: " << j_buf.ptr() << endl;
  str.assign_ptr("\0", static_cast<int64_t>(strlen("\0")));
  j_buf.reset();
  ASSERT_EQ(OB_SUCCESS, ObJsonBaseUtil::add_double_quote(j_buf, str.ptr(), str.length()));
  str.reset();
  str.assign_ptr("\?", static_cast<int64_t>(strlen("\?")));
  j_buf.reset();
  ASSERT_EQ(OB_SUCCESS, ObJsonBaseUtil::add_double_quote(j_buf, str.ptr(), str.length()));
  cout << "quote: " << j_buf.ptr() << endl;
  str.reset();
  str.assign_ptr("\v", static_cast<int64_t>(strlen("\v")));
  j_buf.reset();
  ASSERT_EQ(OB_SUCCESS, ObJsonBaseUtil::add_double_quote(j_buf, str.ptr(), str.length()));
  cout << "quote: " << j_buf.ptr() << endl;
}
