...
TEST_F(ObExprGeoUtilsTest, parse_axis_order_test)
{
  ...
  ASSERT_EQ(OB_ERR_INVALID_OPTION_KEY_VALUE_PAIR, ObGeoExprUtils::parse_axis_order(ObString("   axis-order: = long-lat ,"), "unit_test", axis_order));
  ASSERT_EQ(ObGeoAxisOrder::INVALID, axis_order);
  ASSERT_EQ(OB_ERR_INVALID_OPTION_VALUE, ObGeoExprUtils::parse_axis_order(ObString("   axis-order= long-lat ,"), "unit_test", axis_order));
  ASSERT_EQ(ObGeoAxisOrder::INVALID, axis_order);
  ASSERT_EQ(OB_ERR_INVALID_OPTION_VALUE, ObGeoExprUtils::parse_axis_order(ObString("   axis-order= srid-defined, axis-order = srid-defined"), "unit_test", axis_order));
  ASSERT_EQ(ObGeoAxisOrder::INVALID, axis_order);
  ASSERT_EQ(OB_ERR_INVALID_OPTION_KEY_VALUE_PAIR, ObGeoExprUtils::parse_axis_order(ObString(" . = ."), "unit_test", axis_order));
  ASSERT_EQ(ObGeoAxisOrder::INVALID, axis_order);
  ASSERT_EQ(OB_SUCCESS, ObGeoExprUtils::parse_axis_order(ObString(""), "unit_test", axis_order));
  ASSERT_EQ(ObGeoAxisOrder::SRID_DEFINED, axis_order);
}
