...
bool __attribute__ ((weak)) _isAvailablePort(unsigned short usPort)
{
  ...
  sprintf(shellCommand,
          "netstat -lntu | awk '{print $4}' | grep ':' | cut -d \":\" -f 2 | sort | uniq | grep %hu",
          usPort);
  sprintf(pcPort, "%hu", usPort);

  std::string output = _executeShellCommand(std::string(shellCommand));
  return output.find(std::string(pcPort)) == std::string::npos;
}
...
int64_t __attribute__ ((weak)) get_rpc_port(int &server_fd_ret)
{
  ...
  for (int port = start_port; port + 2 < end_port; port += 3) {
    if (_isAvailablePort(port)
        && _isAvailablePort(port + 1)
        && _isAvailablePort(port + 2)) {
      const bool can_use = (listen_occupy_port(port, server_fd) == 0);
      const bool can_use1 = can_use && (listen_occupy_port(port + 1, server_fd1) == 0);
      const bool can_use2 = can_use1 && (listen_occupy_port(port + 2, server_fd2) == 0);
      if (can_use && can_use1 && can_use2) {
        ...
        STORAGE_LOG(INFO, "find port fail continue try",
                    K(can_use), K(can_use1), K(can_use2), K(port));
        if (can_use) {
          close(server_fd);
        }
        if (can_use1) {
  ...
  if (find_port == 0) {
    ...
    STORAGE_LOG_RET(WARN, OB_ERR_UNEXPECTED, "net", "ss",
                    _executeShellCommand("ss -antlp").c_str());
    OB_ASSERT(false);
  }
  return find_port;
}
