...
namespace
{
class AllocatorTestEnvironment final : public ::testing::Environment
{
public:
  void SetUp() override
  {
...
::testing::Environment *const ALLOCATOR_TEST_ENVIRONMENT =
    ::testing::AddGlobalTestEnvironment(new AllocatorTestEnvironment());
} // namespace
...
TEST(TestCtxAllocatorSuite, CtxAlloc)
{
...
TEST(TestCtxAllocatorSuite, SysLimit)
{
...
TEST(TestCtxAllocatorSuite, TotalLimit)
{
  TestCtxAllocator test_allocator{};
  ObCtxAllocator &ta = test_allocator.allocator_;
  ObMemAttr attr("CtxAlloc");
...
TEST(TestCtxAllocatorSuite, SetMemoryMgrTwice)
{
  TestCtxAllocator test_allocator{};
  ASSERT_EQ(OB_INIT_TWICE, test_allocator.set_memory_mgr());
}

TEST(TestCtxAllocatorSuite, ctx_limit)
{
...
TEST(TestCtxAllocatorSuite, reserve)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
  const uint64_t ctx_id = 1;
  TestCtxAllocator test_allocator(ctx_id);
...
TEST(TestCtxAllocatorSuite, set_idle)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
...
TEST(TestCtxAllocatorSuite, idle)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
...
TEST(TestCtxAllocatorSuite, chunk_free_list_push_pop_concurrency)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
  const uint64_t ctx_id = 1;
  TestCtxAllocator test_allocator(ctx_id);
