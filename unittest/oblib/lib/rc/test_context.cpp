...
using oceanbase::oblib_test::has_unfree;
...
TEST_F(TestContext, Basic)
{
  ...
  // Other suites in the module test binary may own process-lifetime children
  // of the root context.  The root itself must still have no parent or sibling.
  ASSERT_EQ(nullptr, context->tree_node_.parent_);
  ASSERT_EQ(nullptr, context->tree_node_.next_);
  uint64_t ctx_id = ObCtxIds::WORK_AREA;

  ObPageManager g_pm;
  ObPageManager::set_thread_local_instance(g_pm);
  ASSERT_EQ(OB_SUCCESS, g_pm.set_ctx(ctx_id));
...
bool req_cache_empty(ObCtxAllocator *allocator)
{
  for (int i = 0; i < allocator->req_chunk_mgr_.parallel_; i++) {
    if (allocator->req_chunk_mgr_.chunks_[i]) {
      return false;
    }
  }
  return true;
