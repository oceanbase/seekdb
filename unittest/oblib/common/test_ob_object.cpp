...
int TestObObj::compare_sortkey(const ObCollationType cs_type, const char *lhs, const int lhs_len, const char *rhs, const int rhs_len)
{
  ...
  if (is_valid_collation_free1 && is_valid_collation_free2) {
    ret2 = obj3.check_collation_free_and_compare(obj4);
...
{
  test_sortkey("unittest/oblib/common/CollationTest/CollationTest_NON_IGNORABLE_SHORT.txt");
  test_sortkey("unittest/oblib/common/CollationTest/CollationTest_SHIFTED_SHORT.txt");
}
...
TEST_F(TestObObj, test_varchar)
{
  ...
  // print
  char buffer[128];
  ASSERT_TRUE(0 < obj1.to_string(buffer, sizeof(buffer)));
  const char* expected_str =
      "{\"VARCHAR\":\"abc\", collation:\"invalid_type\", coercibility:\"IMPLICIT\"}";
...
TEST_F(TestObObj, test_to_collation_free_obj)
{
  ...
  ASSERT_TRUE(0 == obj3.check_collation_free_and_compare(obj4));
  ...
  // Fall back to the original collation comparison if this value cannot use
  // the collation-free representation.
  ...
  ASSERT_EQ(OB_SUCCESS, obj2.to_collation_free_obj(obj4, is_valid_collation_free2, allocator));
  if (is_valid_collation_free1 && is_valid_collation_free2) {
    ASSERT_EQ(0, obj3.check_collation_free_and_compare(obj4));
  } else {
    ASSERT_EQ(0, obj1.compare(obj2, CS_TYPE_UTF8MB4_GENERAL_CI));
  }

  // A transformed prefix must keep the ordering of its original value.
  obj1.set_varchar("A");
  obj2.set_varchar("AA");
  obj1.set_collation_type(CS_TYPE_UTF8MB4_GENERAL_CI);
  obj2.set_collation_type(CS_TYPE_UTF8MB4_GENERAL_CI);
  ASSERT_LT(obj1.compare(obj2, CS_TYPE_UTF8MB4_GENERAL_CI), 0);
  ASSERT_EQ(OB_SUCCESS, obj1.to_collation_free_obj(obj3, is_valid_collation_free1, allocator));
  ASSERT_EQ(OB_SUCCESS, obj2.to_collation_free_obj(obj4, is_valid_collation_free2, allocator));
  ASSERT_TRUE(is_valid_collation_free1);
  ASSERT_TRUE(is_valid_collation_free2);
  ASSERT_LT(obj3.check_collation_free_and_compare(obj4), 0);
}

TEST_F(TestObObj, test_check_collation_free_and_compare_mysqlmode)
{
  // mysql mode
  ObObj obj1;
  ObObj obj2;
  ...
  int cmp = obj1.check_collation_free_and_compare(obj2);
  fprintf(stdout, "%d\n", cmp);
  ASSERT_TRUE(cmp < 0);
  ...
  cmp = obj1.check_collation_free_and_compare(obj2);
  ASSERT_TRUE(cmp > 0);
