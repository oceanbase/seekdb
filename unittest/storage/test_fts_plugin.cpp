...
void ObTestFTParseHelper::SetUp()
{
  ...
  ASSERT_EQ(OB_SUCCESS, parse_helper_.init(&allocator_, plugin_name_, plugin_properties_, false));
}
...
TEST_F(ObTestFTParseHelper, test_parse_fulltext)
{
  ...
  ASSERT_EQ(OB_INVALID_ARGUMENT, parse_helper_.init(nullptr, plugin_name_, plugin_properties_, false));
  ASSERT_EQ(OB_INVALID_ARGUMENT, parse_helper_.init(&allocator_, ObString(), plugin_properties_, false));
  ...
  ASSERT_EQ(OB_INIT_TWICE, parse_helper_.init(&allocator_, plugin_name_, plugin_properties_, false));
  ...
  parse_helper_.reset();
  ft_word_map.clear();
  ASSERT_EQ(OB_SUCCESS, parse_helper_.init(&allocator_, "beng.1", plugin_properties_, false));
  ASSERT_EQ(
      OB_SUCCESS,
      parse_helper_.segment(
          meta_,
...
TEST_F(ObTestNgramFTParseHelper, test_parse_fulltext)
{
  ...
  ASSERT_EQ(OB_SUCCESS, parse_helper_.init(&allocator_, common::ObString(STRLEN(plugin_name), plugin_name), plugin_properties_, false));
