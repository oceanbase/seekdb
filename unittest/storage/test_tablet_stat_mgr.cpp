...
class TestTabletStatMgr : public ::testing::Test
{
public:
  TestTabletStatMgr();
  virtual ~TestTabletStatMgr() = default;
  virtual void SetUp() override;
  virtual void TearDown() override;
  static void SetUpTestCase();
  static void TearDownTestCase();
  static void report(ObTabletStatMgr *mgr, const ObTabletStat &stat);
  void batch_report_stat(int64_t report_num);

private:
  ObTabletStatMgr *stat_mgr_;
};

TestTabletStatMgr::TestTabletStatMgr()
  : stat_mgr_(nullptr)
{
}

void TestTabletStatMgr::SetUpTestCase()
{
  ASSERT_EQ(OB_SUCCESS, ObTimerService::get_instance().start());
  EXPECT_EQ(OB_SUCCESS, MockServerRuntimeEnv::get_instance().init());
}

void TestTabletStatMgr::TearDownTestCase()
{
...
void TestTabletStatMgr::SetUp()
{
  ASSERT_TRUE(MockServerRuntimeEnv::get_instance().is_inited());
  int ret = OB_SUCCESS;

  ASSERT_EQ(OB_SUCCESS, ObTimerService::get_instance().start());
  stat_mgr_ = OB_NEW(ObTabletStatMgr, ObModIds::TEST);
  ret = stat_mgr_->init();
  ASSERT_EQ(OB_SUCCESS, ret);
}

void TestTabletStatMgr::TearDown()
{
  stat_mgr_->destroy();
  ObTimerService::get_instance().stop();
  ObTimerService::get_instance().wait();
...
void TestTabletStatMgr::report(ObTabletStatMgr *mgr, const ObTabletStat &stat)
{
...
void TestTabletStatMgr::batch_report_stat(int64_t report_num)
{
...
namespace unittest
{
TEST_F(TestTabletStatMgr, basic_tablet_stat_bucket)
{
...
TEST_F(TestTabletStatMgr, basic_tablet_stream)
{
  ObTabletStat tablet_stat;
...
TEST_F(TestTabletStatMgr, get_all_tablet_stat)
{
  int ret = OB_SUCCESS;
...
TEST_F(TestTabletStatMgr, basic_tablet_stat_mgr)
{
  ...
  ObTabletStatMgr *stat_mgr = stat_mgr_;
...
TEST_F(TestTabletStatMgr, multi_report_tablet_stat)
{
  ObTabletStatMgr *stat_mgr = stat_mgr_;
  ASSERT_TRUE(NULL != stat_mgr);
  ...
  ObTabletStatMgr::TabletStreamMap::iterator iter = stat_mgr_->stream_map_.begin();
...
TEST_F(TestTabletStatMgr, bacth_clear_tablet_stat)
{
  ObTabletStatMgr *stat_mgr = stat_mgr_;
  ASSERT_TRUE(NULL != stat_mgr);
  ASSERT_TRUE(stat_mgr->is_inited_);
...
int main(int argc, char **argv)
{
  ...
  system("rm -f test_tablet_stat_mgr.log*");
  OB_LOGGER.set_file_name("test_tablet_stat_mgr.log", true);
