...
#include "storage/testing/ob_storage_test_schema.h"
...
public:
  ObRowGenerate();
  ~ObRowGenerate();
  int init(const share::schema::ObTableSchema &src_schema, bool is_multi_version_row = false);
  int init(
      const share::schema::ObTableSchema &src_schema,
      common::ObArenaAllocator *allocator,
      bool is_multi_version_row = false);
  int get_next_row(ObDatumRow &row);
  int get_next_row(const int64_t seed, ObDatumRow &row);
  int get_next_row(const int64_t seed,
  ...
  int get_next_row(const int64_t seed, const common::ObArray<uint64_t> &nop_column_idxs, ObDatumRow &row);
  int check_one_row(const ObDatumRow &row, bool &exist);
  // TODO @hanhui to be removed
  int get_next_row(storage::ObStoreRow &row);
  int get_next_row(const int64_t seed, storage::ObStoreRow &row);
  int get_next_row(const int64_t seed,
      const int64_t trans_version,
      const ObDmlFlag dml_flag,
  ...
  int get_next_row(const int64_t seed, const common::ObArray<uint64_t> &nop_column_idxs, storage::ObStoreRow &row);
