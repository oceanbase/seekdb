...
- 不变式 2b：**避免感知 ns_id/ns_name**——显式传递传的是服务实例/上下文（schema service 引用、session、runtime 指针），不是 ns 身份本身。模块拿到 `schema_service&` 就用，不知道也不该知道它是哪个 ns 的；禁止模块收 ns_id 参数后内部分支。ns_id/ns_name 的解释权只归边界层：登录路由（`root@ns` 解析）、NamespaceRegistry、id 编码翻译层。
