...
- 2026-09-22 读路径全面重定向化已落地并四套件 PASS（direct full / direct tls / bootstrap / sql_worker full）。上节"range service 是否走重定向"的待决策项已拍板落地——读不物化：
  ...
  7. 遗留：V24/V23 调试插桩仍在代码中（`PROTOTYPE_V24_LOCK_FAIL`/`_GLOBAL_SPACE`、`PROTOTYPE_V23_SCAN_*`、`SCHEMA_REFRESH_PROF` 等），`resolve_inherited_tablet` 的 4725 打印条件已还原；fork→可用 1s 目标的剩余大头是激活期 schema 全量刷新（55k 串行点查），下一阶段做 schema 懒加载；b1 内 CREATE DATABASE+CREATE TABLE 卡 2m15s 的回归、3000 表规模公平对比测试随懒加载一并验证。
