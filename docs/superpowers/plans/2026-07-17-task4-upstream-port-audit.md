...
| 020 | M | 修改但当前路径不存在 | `src/observer/virtual_table/ob_all_virtual_logservice_cluster_info.cpp` | Task 7 | `src/observer/virtual_table/ob_all_virtual_ddl_dag_monitor.cpp` | 未移植 | HEAD 无同名跟踪路径；待 `test_ddl_dag_monitor`、`observer` 链接、schema 生成检索 |
...
| 072 | M | 修改但当前路径不存在 | `src/share/schema/ob_schema_printer.cpp` | Task 4 | `src/share/schema/ob_schema_printer.cpp` | 未移植 | HEAD 无同名跟踪路径；待 `test_fts_encoding`、`test_fts_property`、`observer` 链接 |
...
| 079 | M | 修改但当前路径不存在 | `src/share/schema/ob_table_dml_param.cpp` | Task 4 | `src/share/schema/ob_table_dml_param.cpp` | 未移植 | HEAD 无同名跟踪路径；待 `test_fts_encoding`、`test_fts_property`、`observer` 链接 |
| 080 | M | 修改但当前路径不存在 | `src/share/schema/ob_table_dml_param.h` | Task 4 | `src/share/schema/ob_table_dml_param.h` | 未移植 | HEAD 无同名跟踪路径；待 `test_fts_encoding`、`test_fts_property`、`observer` 链接 |
| 081 | M | 修改但当前路径不存在 | `src/share/schema/ob_table_param.cpp` | Task 4 | `src/share/schema/ob_table_param.cpp` | 未移植 | HEAD 无同名跟踪路径；待 `test_fts_encoding`、`test_fts_property`、`observer` 链接 |
| 082 | M | 修改但当前路径不存在 | `src/share/schema/ob_table_param.h` | Task 4 | `src/share/schema/ob_table_param.h` | 未移植 | HEAD 无同名跟踪路径；待 `test_fts_encoding`、`test_fts_property`、`observer` 链接 |
...
| 088 | M | 修改但当前路径不存在 | `src/share/vector_index/ob_vector_index_util.cpp` | Task 6 | `src/rootserver/ddl_task/ob_vec_index_build_task.cpp` | 未移植 | HEAD 无同名跟踪路径；待 `test_pipeline_and_op`、`observer` 链接、`ik_custom_dict` |
