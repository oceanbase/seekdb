...
> **2026-07-18 范围调整（当前有效）**：本任务尽量完成本文第 5 节六类优化的单机子集，以 seekdb 单机全文索引构建和未修改的 `tools/benchmark/fts_large_bench.sh` 的同机 Release 实测性能为唯一性能裁判。每个候选项只有在评分提升超过约 2 个评分点波动带、且命中数正确时才保留；无提升或回退必须撤回并记录排除理由。OceanBase 提交 `81c822c...` 仅作为实现参考，不要求机械移植 283 个文件；PX、跨节点路由、跨分区 shuffle、分布式 GI 和分布式 DAG 不在范围内。不得为了追求目录覆盖率牺牲基准性能、稳定性或现有测试。

## 1. 文档范围与基线

本文档定义 Task4 的需求和实现边界。实现基线为 seekdb `vldb_2026` 分支提交 `a0ffb3c3`，OceanBase 上游提交 `81c822ca5cb2d88c3495192d21e6006d6785fbb4`（父提交 `b786266ba3fc07b8437d07c8d1d177580e788cd0`）用于定位可借鉴的优化点。上游 283 文件差异是参考审计资料，不是本次必须逐文件移植的清单。

本任务覆盖分词/解析器、排序、FTS 构建流水线、sort key/位置列表、SQL 计划适配和 DDL DAG 可观测性六类优化；每一类先选择可在单机 seekdb 中落地、能被基准或功能路径验证的子项。分布式专有的 PX/GI/shuffle 机制可以裁剪为单机等价实现，不能直接复用时不强行搬运。
