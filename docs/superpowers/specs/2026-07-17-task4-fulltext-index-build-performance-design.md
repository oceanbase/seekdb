...
> **2026-07-18 范围调整（当前有效）**：本任务以 seekdb 单机全文索引构建的可用功能和 `tools/benchmark/fts_large_bench.sh` 的实测性能提升为首要验收标准。OceanBase 提交 `81c822c...` 仅作为实现参考，不再要求机械完整移植 283 个文件或六大体系中的分布式能力。PX、跨分区 shuffle、租户级 DDL DAG 监控等仅在直接阻塞单机基准瓶颈且能以小范围改动落地时才纳入；否则不属于本次必做功能。
...
本任务优先实现以下六类中能够直接提升单机 `fts_large_bench.sh` 建索引或分词热路径的部分：

1. 分词器和解析器热路径优化（必做）。
2. 单机索引构建中实际被性能分析证明为瓶颈的排序/编码优化（按实测决定）。
3. 辅助表或位置列表功能中 `fts_large_bench.sh` 正确性所需的部分（按实测决定）。

分区本地流水线、多阶段 DAG、PX/GI 适配和 DDL DAG 监控保留为参考方向，不构成本次单机任务的必做范围。
