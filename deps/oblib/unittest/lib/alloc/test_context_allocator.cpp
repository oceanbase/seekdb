...
#include "lib/alloc/ob_context_allocator.h"
#undef private
...
struct TestContextAllocator
{
  TestContextAllocator(int64_t ctx_id = 0)
    : ctx_allocator_(ctx_id, &allocator_),
      allocator_(ctx_allocator_, ctx_id)
  {
    ctx_allocator_.set_server_memory_manager();
    ctx_allocator_.set_limit(INT64_MAX);
  }
  int set_server_memory_manager() { return ctx_allocator_.set_server_memory_manager(); }
...
TEST(TestContextAllocator, ContextAlloc)
{
  CHUNK_MGR.set_max_chunk_cache_size(1<<20);
  TestContextAllocator test_allocator(1);
  ObContextAllocator &ta = test_allocator.allocator_;
  ObMemAttr attr("ContextAlloc", 1);
...
TEST(TestContextAllocator, SystemLimit)
{
  TestContextAllocator test_allocator{};
  ObContextAllocator &ta = test_allocator.allocator_;
  ObMemAttr attr("ContextAlloc");
  // Baseline on the server sum_hold_ (the counter the limit checks), not the
  // global CHUNK_MGR hold. A fresh alloc maps two chunks (metadata + data).
  int64_t hold = test_allocator.ctx_allocator_.get_memory_hold();
  ...
  hold = test_allocator.ctx_allocator_.get_memory_hold();
...
TEST(TestContextAllocator, MemoryLimit)
{
  TestContextAllocator test_allocator{};
  ObContextAllocator &ta = test_allocator.allocator_;
  ObMemAttr attr("ContextAlloc");
  const int64_t hold = test_allocator.ctx_allocator_.get_memory_hold();
  ...
  // Keep CHUNK_MGR open so only the server memory limit is under test.
...
TEST(TestContextAllocator, SetMemoryManagerTwice)
{
  TestContextAllocator test_allocator{};
  ASSERT_EQ(OB_INIT_TWICE, test_allocator.set_server_memory_manager());
}

TEST(TestContextAllocator, context_limit)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
  const uint64_t ctx_id = 1;
  TestContextAllocator test_allocator(ctx_id);
  ObContextAllocator &ctx_allocator = test_allocator.allocator_;
  ...
  int64_t alloced = 0;
  ctx_allocator.set_limit(limit);
  ctx_allocator.set_hard_limit(limit);
  while (true) {
    if (NULL == ctx_allocator.alloc(size, attr)) {
      break;
    } else {
      alloced += size;
    }
  ...
  cout << "hold: " << ctx_allocator.get_hold() << endl;
  ASSERT_TRUE(alloced > limit / 2 && alloced <= limit);
  ASSERT_TRUE(ctx_allocator.get_hold() > limit / 2 && ctx_allocator.get_hold() <= limit);
}

TEST(TestContextAllocator, reserve)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
  const uint64_t ctx_id = 1;
  TestContextAllocator test_allocator(ctx_id);
  ...
  ObMemAttr attr("ContextAlloc", ctx_id);
...
TEST(TestContextAllocator, set_idle)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
...
TEST(TestContextAllocator, idle)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
...
TEST(TestContextAllocator, chunk_free_list_push_pop_concurrency)
{
  CHUNK_MGR.set_limit(1L * 1024L * 1024L * 1024L);
  const uint64_t ctx_id = 1;
  TestContextAllocator test_allocator(ctx_id);
