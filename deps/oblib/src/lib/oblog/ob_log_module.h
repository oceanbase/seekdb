...
#define MACRO_ARGS_DIAG_WARN , FOR_DIAG_WARN
...
#ifndef OB_LOG_COMPILE_HIDE_DIAG_WARN
#define OB_LOG_COMPILE_HIDE_DIAG_WARN 0
#endif

#ifdef ENABLE_DEBUG_LOG
#if OB_LOG_COMPILE_LEVEL >= OB_LOG_LEVEL_DEBUG
...
#define IS_LOG_ENABLED_FOR_DIAG_WARN false
#elif OB_LOG_COMPILE_LEVEL >= OB_LOG_LEVEL_DIAG_WARN
#define IS_LOG_ENABLED_FOR_DIAG_WARN true
#else
#define IS_LOG_ENABLED_FOR_DIAG_WARN false
#endif
...
#define GET_LOG_FUNC_ATTR_FOR_DIAG_WARN noinline,cold
...
#define LOG_ERRCODE_FOR_DIAG_WARN ret
...
#define _OB_NUM_LEVEL_LOG(level, errcode, _fmt_, args...)                                                 \
  (::oceanbase::common::need_to_print(level) ?                                                            \
...
#define OB_MOD_LOG(parMod, level, info_string, args...)                                          \
    do { if (IS_LOG_ENABLED(level)) {                                                            \
    if (OB_UNLIKELY(::oceanbase::common::need_to_print(::oceanbase::common::OB_LOG_ROOT::M_##parMod, OB_LOG_LEVEL_##level))) \
    { ::oceanbase::common::OB_PRINT("["#parMod"] ", OB_LOG_LEVEL(level), info_string, LOG_KVS(args)); } \
    } } while (false)

#define _OB_MOD_LOG(parMod, level, _fmt_, args...)                                               \
    do { if (IS_LOG_ENABLED(level)) {                                                            \
    ((::oceanbase::common::need_to_print(oceanbase::common::OB_LOG_ROOT::M_##parMod, OB_LOG_LEVEL_##level)) \
    ? _OB_PRINT("["#parMod"] ", level, _fmt_, ##args) : (void) 0);                               \
    } } while (false)

#define OB_SUB_MOD_LOG(parMod, subMod, level, info_string, args...)                              \
    do { if (IS_LOG_ENABLED(level)) {                                                            \
    if (OB_UNLIKELY(::oceanbase::common::need_to_print(::oceanbase::common::OB_LOG_ROOT::M_##parMod, \
...
#define _OB_SUB_MOD_LOG(parMod, subMod, level, _fmt_, args...)                                   \
    do { if (IS_LOG_ENABLED(level)) {                                                            \
    ((::oceanbase::common::need_to_print(oceanbase::common::OB_LOG_ROOT::M_##parMod,             \
                              oceanbase::common::OB_LOG_##parMod::M_##subMod,                    \
                              OB_LOG_LEVEL_##level)) ?                                           \
...
#define OB_MOD_LOG_RET(mod_ptr, level, errcode, info_string, args...)                              \
    { int __ret__ = errcode; int ret = __ret__;                                                    \
      do { if (IS_LOG_ENABLED(level)) {                                                            \
      if (OB_UNLIKELY(::oceanbase::common::need_to_print(OB_LOG_LEVEL_##level)))                   \
...
#define OB_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OB_LOG(level, ##args); }

//
// The following LOG MACROS are used for logging with specified error code, which are generated with the shell script:
//     sed -n '/BEGIN[^[]MODULE LOG MACRO DEFINE/,/END[^[]MODULE LOG MACRO DEFINE/p' deps/oblib/src/lib/oblog/ob_log_module.h | grep 'define[[:space:]]*[[:alnum:]_]*_LOG(level' -o | sed -e 's/^define//g' -e 's/(level//g' | while read l ; do echo "#define ${l}_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; $l(level, ##args); }"; done

// BEGIN XXX_LOG_RET MACRO DEFINE
#define BLSST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; BLSST_LOG(level, ##args); }
#define _BLSST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _BLSST_LOG(level, ##args); }
#define CLIENT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; CLIENT_LOG(level, ##args); }
...
#define _IMPS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _IMPS_LOG(level, ##args); }
#define WRS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; WRS_LOG(level, ##args); }
...
#define _MEMT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _MEMT_LOG(level, ##args); }
#define MRSST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; MRSST_LOG(level, ##args); }
#define _MRSST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _MRSST_LOG(level, ##args); }
#define MYSQL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; MYSQL_LOG(level, ##args); }
#define _MYSQL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _MYSQL_LOG(level, ##args); }
#define OFS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_LOG(level, ##args); }
#define _OFS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_LOG(level, ##args); }
#define PS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; PS_LOG(level, ##args); }
#define _PS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _PS_LOG(level, ##args); }
#define RPC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; RPC_LOG(level, ##args); }
#define _RPC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _RPC_LOG(level, ##args); }
#define RS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; RS_LOG(level, ##args); }
#define _RS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _RS_LOG(level, ##args); }
#define BOOTSTRAP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; BOOTSTRAP_LOG(level, ##args); }
#define _BOOTSTRAP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _BOOTSTRAP_LOG(level, ##args); }
#define SERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SERVER_LOG(level, ##args); }
#define _SERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SERVER_LOG(level, ##args); }
#define SHARE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_LOG(level, ##args); }
#define _SHARE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_LOG(level, ##args); }
#define SQL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_LOG(level, ##args); }
#define _SQL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_LOG(level, ##args); }
#define PL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; PL_LOG(level, ##args); }
#define _PL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _PL_LOG(level, ##args); }
#define JIT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; JIT_LOG(level, ##args); }
#define _JIT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _JIT_LOG(level, ##args); }
#define STORAGE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STORAGE_LOG(level, ##args); }
#define _STORAGE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STORAGE_LOG(level, ##args); }
...
#define _RU_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _RU_LOG(level, ##args); }
#define REPLAY_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; REPLAY_LOG(level, ##args); }
...
#define _DETECT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _DETECT_LOG(level, ##args); }
#define PALF_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; PALF_LOG(level, ##args); }
#define _PALF_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _PALF_LOG(level, ##args); }
#define STANDBY_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STANDBY_LOG(level, ##args); }
#define _STANDBY_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STANDBY_LOG(level, ##args); }
#define COORDINATOR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COORDINATOR_LOG(level, ##args); }
#define _COORDINATOR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COORDINATOR_LOG(level, ##args); }
#define FLT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; FLT_LOG(level, ##args); }
#define _FLT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _FLT_LOG(level, ##args); }
#define MVCC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; MVCC_LOG(level, ##args); }
#define _MVCC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _MVCC_LOG(level, ##args); }
#define WRS_CLUSTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; WRS_CLUSTER_LOG(level, ##args); }
#define _WRS_CLUSTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _WRS_CLUSTER_LOG(level, ##args); }
#define WRS_SERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; WRS_SERVER_LOG(level, ##args); }
#define _WRS_SERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _WRS_SERVER_LOG(level, ##args); }
#define LIB_ALLOC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_ALLOC_LOG(level, ##args); }
#define _LIB_ALLOC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_ALLOC_LOG(level, ##args); }
#define LIB_CHARSET_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_CHARSET_LOG(level, ##args); }
#define _LIB_CHARSET_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_CHARSET_LOG(level, ##args); }
#define LIB_CONTAIN_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_CONTAIN_LOG(level, ##args); }
#define _LIB_CONTAIN_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_CONTAIN_LOG(level, ##args); }
#define LIB_CPU_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_CPU_LOG(level, ##args); }
#define _LIB_CPU_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_CPU_LOG(level, ##args); }
#define LIB_ENCRYPT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_ENCRYPT_LOG(level, ##args); }
#define _LIB_ENCRYPT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_ENCRYPT_LOG(level, ##args); }
#define LIB_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_FILE_LOG(level, ##args); }
#define _LIB_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_FILE_LOG(level, ##args); }
#define LIB_HASH_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_HASH_LOG(level, ##args); }
#define _LIB_HASH_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_HASH_LOG(level, ##args); }
#define LIB_JASON_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_JASON_LOG(level, ##args); }
#define _LIB_JASON_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_JASON_LOG(level, ##args); }
#define LIB_LIST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_LIST_LOG(level, ##args); }
#define _LIB_LIST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_LIST_LOG(level, ##args); }
#define LIB_LOCK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_LOCK_LOG(level, ##args); }
#define _LIB_LOCK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_LOCK_LOG(level, ##args); }
#define LIB_MYSQLC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_MYSQLC_LOG(level, ##args); }
#define _LIB_MYSQLC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_MYSQLC_LOG(level, ##args); }
#define LIB_NUM_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_NUM_LOG(level, ##args); }
#define _LIB_NUM_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_NUM_LOG(level, ##args); }
#define LIB_OBJP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_OBJP_LOG(level, ##args); }
#define _LIB_OBJP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_OBJP_LOG(level, ##args); }
#define LIB_PROB_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_PROB_LOG(level, ##args); }
#define _LIB_PROB_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_PROB_LOG(level, ##args); }
#define LIB_PROFILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_PROFILE_LOG(level, ##args); }
#define _LIB_PROFILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_PROFILE_LOG(level, ##args); }
#define LIB_QUEUE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_QUEUE_LOG(level, ##args); }
#define _LIB_QUEUE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_QUEUE_LOG(level, ##args); }
#define LIB_REGEX_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_REGEX_LOG(level, ##args); }
#define _LIB_REGEX_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_REGEX_LOG(level, ##args); }
#define LIB_STRING_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_STRING_LOG(level, ##args); }
#define _LIB_STRING_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_STRING_LOG(level, ##args); }
#define LIB_TASK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_TASK_LOG(level, ##args); }
#define _LIB_TASK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_TASK_LOG(level, ##args); }
#define LIB_TRHEADL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_TRHEADL_LOG(level, ##args); }
#define _LIB_TRHEADL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_TRHEADL_LOG(level, ##args); }
#define LIB_TIME_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LIB_TIME_LOG(level, ##args); }
...
#define _LIB_OCI_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _LIB_OCI_LOG(level, ##args); }
#define OFS_BLOCK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_BLOCK_LOG(level, ##args); }
#define _OFS_BLOCK_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_BLOCK_LOG(level, ##args); }
#define OFS_BLOCKSERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_BLOCKSERVER_LOG(level, ##args); }
#define _OFS_BLOCKSERVER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_BLOCKSERVER_LOG(level, ##args); }
#define OFS_CLIENT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_CLIENT_LOG(level, ##args); }
#define _OFS_CLIENT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_CLIENT_LOG(level, ##args); }
#define OFS_CLUSTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_CLUSTER_LOG(level, ##args); }
#define _OFS_CLUSTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_CLUSTER_LOG(level, ##args); }
#define OFS_COMMON_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_COMMON_LOG(level, ##args); }
#define _OFS_COMMON_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_COMMON_LOG(level, ##args); }
#define OFS_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_FILE_LOG(level, ##args); }
#define _OFS_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_FILE_LOG(level, ##args); }
#define OFS_FS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_FS_LOG(level, ##args); }
#define _OFS_FS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_FS_LOG(level, ##args); }
#define OFS_MASTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_MASTER_LOG(level, ##args); }
#define _OFS_MASTER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_MASTER_LOG(level, ##args); }
#define OFS_REGRESSION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_REGRESSION_LOG(level, ##args); }
#define _OFS_REGRESSION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_REGRESSION_LOG(level, ##args); }
#define OFS_RPC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_RPC_LOG(level, ##args); }
#define _OFS_RPC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_RPC_LOG(level, ##args); }
#define OFS_STREAM_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_STREAM_LOG(level, ##args); }
#define _OFS_STREAM_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_STREAM_LOG(level, ##args); }
#define OFS_UTIL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_UTIL_LOG(level, ##args); }
#define _OFS_UTIL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _OFS_UTIL_LOG(level, ##args); }
#define OFS_LIBS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; OFS_LIBS_LOG(level, ##args); }
...
#define _RPC_TEST_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _RPC_TEST_LOG(level, ##args); }
#define COMMON_CACHE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_CACHE_LOG(level, ##args); }
#define _COMMON_CACHE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_CACHE_LOG(level, ##args); }
#define COMMON_EXPR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_EXPR_LOG(level, ##args); }
#define _COMMON_EXPR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_EXPR_LOG(level, ##args); }
#define COMMON_LEASE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_LEASE_LOG(level, ##args); }
#define _COMMON_LEASE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_LEASE_LOG(level, ##args); }
#define COMMON_MYSQLP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_MYSQLP_LOG(level, ##args); }
#define _COMMON_MYSQLP_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_MYSQLP_LOG(level, ##args); }
#define COMMON_PRI_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_PRI_LOG(level, ##args); }
#define _COMMON_PRI_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_PRI_LOG(level, ##args); }
#define COMMON_STAT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_STAT_LOG(level, ##args); }
#define _COMMON_STAT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_STAT_LOG(level, ##args); }
#define COMMON_UPSR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; COMMON_UPSR_LOG(level, ##args); }
#define _COMMON_UPSR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _COMMON_UPSR_LOG(level, ##args); }
#define SHARE_CONFIG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_CONFIG_LOG(level, ##args); }
#define _SHARE_CONFIG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_CONFIG_LOG(level, ##args); }
#define SHARE_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_FILE_LOG(level, ##args); }
#define _SHARE_FILE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_FILE_LOG(level, ##args); }
#define SHARE_INNERT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_INNERT_LOG(level, ##args); }
#define _SHARE_INNERT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_INNERT_LOG(level, ##args); }
#define SHARE_INTERFACE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_INTERFACE_LOG(level, ##args); }
#define _SHARE_INTERFACE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_INTERFACE_LOG(level, ##args); }
#define SHARE_LOG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_LOG_LOG(level, ##args); }
#define _SHARE_LOG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_LOG_LOG(level, ##args); }
#define SHARE_PT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_PT_LOG(level, ##args); }
#define _SHARE_PT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_PT_LOG(level, ##args); }
#define SHARE_SCHEMA_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_SCHEMA_LOG(level, ##args); }
#define _SHARE_SCHEMA_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_SCHEMA_LOG(level, ##args); }
#define SHARE_TRIGGER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_TRIGGER_LOG(level, ##args); }
#define _SHARE_TRIGGER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_TRIGGER_LOG(level, ##args); }
#define SHARE_LOCATION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SHARE_LOCATION_LOG(level, ##args); }
#define _SHARE_LOCATION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SHARE_LOCATION_LOG(level, ##args); }
#define STORAGE_REDO_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STORAGE_REDO_LOG(level, ##args); }
#define _STORAGE_REDO_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STORAGE_REDO_LOG(level, ##args); }
#define STORAGE_COMPACTION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STORAGE_COMPACTION_LOG(level, ##args); }
#define _STORAGE_COMPACTION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STORAGE_COMPACTION_LOG(level, ##args); }
#define RS_COMPACTION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; RS_COMPACTION_LOG(level, ##args); }
#define _RS_COMPACTION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _RS_COMPACTION_LOG(level, ##args); }
#define STORAGE_BLKMGR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STORAGE_BLKMGR_LOG(level, ##args); }
#define _STORAGE_BLKMGR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STORAGE_BLKMGR_LOG(level, ##args); }
#define STORAGE_FTS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; STORAGE_BLKMGR_LOG(level, ##args); }
#define _STORAGE_FTS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _STORAGE_BLKMGR_LOG(level, ##args); }
#define SQL_ENG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_ENG_LOG(level, ##args); }
#define _SQL_ENG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_ENG_LOG(level, ##args); }
#define SQL_EXE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_EXE_LOG(level, ##args); }
#define _SQL_EXE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_EXE_LOG(level, ##args); }
#define SQL_OPT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_OPT_LOG(level, ##args); }
#define _SQL_OPT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_OPT_LOG(level, ##args); }
#define SQL_JO_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_JO_LOG(level, ##args); }
#define _SQL_JO_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_JO_LOG(level, ##args); }
#define SQL_PARSER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_PARSER_LOG(level, ##args); }
#define _SQL_PARSER_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_PARSER_LOG(level, ##args); }
#define SQL_PC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_PC_LOG(level, ##args); }
#define _SQL_PC_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_PC_LOG(level, ##args); }
#define _SQL_PLANCACHE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_PLANCACHE_LOG(level, ##args); }
#define SQL_RESV_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_RESV_LOG(level, ##args); }
#define _SQL_RESV_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_RESV_LOG(level, ##args); }
#define SQL_REWRITE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_REWRITE_LOG(level, ##args); }
#define _SQL_REWRITE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_REWRITE_LOG(level, ##args); }
#define SQL_SESSION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_SESSION_LOG(level, ##args); }
#define _SQL_SESSION_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_SESSION_LOG(level, ##args); }
#define SQL_CG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_CG_LOG(level, ##args); }
#define _SQL_CG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_CG_LOG(level, ##args); }
#define SQL_MONITOR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_MONITOR_LOG(level, ##args); }
#define _SQL_MONITOR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_MONITOR_LOG(level, ##args); }
#define SQL_DTL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_DTL_LOG(level, ##args); }
#define _SQL_DTL_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_DTL_LOG(level, ##args); }
#define SQL_DAS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_DAS_LOG(level, ##args); }
#define _SQL_DAS_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_DAS_LOG(level, ##args); }
#define SQL_QRR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SQL_QRR_LOG(level, ##args); }
#define _SQL_QRR_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SQL_QRR_LOG(level, ##args); }
#define DETECT_LOG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; DETECT_LOG_LOG(level, ##args); }
#define _DETECT_LOG_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _DETECT_LOG_LOG(level, ##args); }
#define SERVER_OMT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; SERVER_OMT_LOG(level, ##args); }
#define _SERVER_OMT_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; _SERVER_OMT_LOG(level, ##args); }
#define RS_LB_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; RS_LB_LOG(level, ##args); }
#define RS_RESTORE_LOG_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; RS_RESTORE_LOG(level, ##args); }
...
#define FORWARD_USER_NOTE(errcode, msg)                                         \
  if (OB_NOT_NULL(msg)) { \
    ...
    if (::oceanbase::common::need_to_print(OB_LOG_LEVEL_WARN)) { \
...
#define LOG_WARN(args...) LOG_MACRO_JOIN(USING_LOG_PREFIX, _LOG) (DIAG_WARN, ##args)
#define LOG_WARN_RET(args...) LOG_MACRO_JOIN(USING_LOG_PREFIX, _LOG_RET) (DIAG_WARN, ##args)
#define _LOG_WARN(args...) _LOG_MACRO_JOIN(USING_LOG_PREFIX, _LOG) (DIAG_WARN, ##args)
#define _LOG_WARN_RET(args...) _LOG_MACRO_JOIN(USING_LOG_PREFIX, _LOG_RET) (DIAG_WARN, ##args)
...
#define LOG_PRINT_EXPR_RET(level, errcode, args...) { int __ret__ = errcode; int ret = __ret__; LOG_PRINT_EXPR(level, ##args); }
