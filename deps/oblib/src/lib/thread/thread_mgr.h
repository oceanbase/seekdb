...
public:
  ...
  virtual int set_handler(TGLinkTaskHandler &handler)
  {
    UNUSED(handler);
    return common::OB_NOT_SUPPORTED;
  }
  ...
  virtual int push_task(common::LinkTask *task)
  {
    UNUSED(task);
    ob_abort();
    return common::OB_NOT_SUPPORTED;
...
class MySimpleThreadPool : public common::ObSimpleThreadPoolBase<T>
{
public:
  using TaskType = typename QueueTypeMap<T>::TaskType;
  void run1() override
  {
    handler_->set_thread_cnt(this->get_thread_count());
    handler_->set_thread_idx(this->get_thread_idx());
    common::ObSimpleThreadPoolBase<T>::run1();
  }
  void handle(TaskType *task) override
  {
    handler_->handle(task);
  }
  void handle_drop(TaskType *task) override
  {
    handler_->handle_drop(task);
  }
  TGTaskHandlerTemplate<TaskType> *handler_ = nullptr;
};

template<typename ThreadPoolType = MySimpleThreadPool<common::ObLightyQueue>>
class TG_QUEUE_THREAD_IMPL : public ITG
{
public:
  TG_QUEUE_THREAD_IMPL(ThreadCountPair pair, const int64_t task_num_limit)
    : min_thread_num_(pair.get_thread_cnt()),
      max_thread_num_(pair.get_thread_cnt()),
      task_num_limit_(task_num_limit)
  {}
  TG_QUEUE_THREAD_IMPL(int64_t thread_num, const int64_t task_num_limit)
    : min_thread_num_(thread_num),
      max_thread_num_(thread_num),
      task_num_limit_(task_num_limit)
  {}
  ~TG_QUEUE_THREAD_IMPL() { destroy(); }
  ...
  int push_task(typename ThreadPoolType::TaskType *task) override
  {
    int ret = common::OB_SUCCESS;
    if (OB_ISNULL(qth_)) {
      ret = common::OB_ERR_UNEXPECTED;
...
using TG_QUEUE_THREAD = TG_QUEUE_THREAD_IMPL<MySimpleThreadPool<common::ObLightyQueue>>;
using TG_LINK_QUEUE_THREAD = TG_QUEUE_THREAD_IMPL<MySimpleThreadPool<common::ObTLinkQueue16>>;
...
private:
  static constexpr int MAX_ID = 1228;
