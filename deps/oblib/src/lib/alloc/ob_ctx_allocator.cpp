...
ObCtxAllocatorState::ObCtxAllocatorState(uint64_t ctx_id,
    ObCtxAllocator *allocator)
    : resource_handle_(), ctx_id_(ctx_id), allocator_(allocator),
      wash_related_chunks_(0), washed_blocks_(0), washed_size_(0)
{}

ObCtxAllocatorState::~ObCtxAllocatorState()
{
  allocator_ = NULL;
}

void ObCtxAllocatorState::set_req_chunkmgr_parallel(int32_t parallel)
{
...
void *ObCtxAllocator::alloc(const int64_t size, const ObMemAttr &attr)
{
  return common_realloc(NULL, size, attr, *this, obj_mgr_);
}

int64_t ObCtxAllocator::get_obj_hold(void *ptr)
{
...
void* ObCtxAllocator::realloc(const void *ptr, const int64_t size, const ObMemAttr &attr)
{
  return common_realloc(ptr, size, attr, *this, obj_mgr_);
}

void ObCtxAllocator::free(void *ptr)
{
  common_free(ptr);
}
int ObCtxAllocatorState::iter_label(VisitFunc func) const
{
  int ret = OB_SUCCESS;
  struct ItemWrapper
  {
  ...
  if (OB_UNLIKELY(!mem_dump.is_inited())) {
    ...
    SMART_VAR(ItemWrapper[item_cap], items) {
      ...
      if (ObCtxIds::KVSTORE_CACHE_ID == ctx_id_) {
        ...
        IGNORE_RETURN with_resource_handle_invoke([&](const ObMemoryMgr *mgr) {
...
void ObCtxAllocatorState::print_usage() const
{
  int ret = OB_SUCCESS;
...
AChunk *ObCtxAllocator::pop_chunk()
{
...
void ObCtxAllocator::push_chunk(AChunk *chunk)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  lib::ObMutexGuard guard(chunk_freelist_mutex_);
...
AChunk *ObCtxAllocator::alloc_chunk(const int64_t size, const ObMemAttr &attr)
{
  int ret = OB_SUCCESS;
...
void ObCtxAllocator::free_chunk(AChunk *chunk, const ObMemAttr &attr)
{
  if (chunk != nullptr) {
    ObDisableDiagnoseGuard disable_diagnose_guard;
...
void ObCtxAllocator::dec_hold(const int64_t size)
{
  ctx_allocator_.dec_hold(size);
}

void ObCtxAllocatorState::dec_hold(const int64_t size)
{
...
int ObCtxAllocator::set_idle(const int64_t set_size, const bool reserve/*=false*/)
{
  int ret = OB_SUCCESS;
...
void ObCtxAllocatorState::get_chunks(AChunk **chunks, int cap, int &cnt)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  allocator_->get_chunks_(chunks, cap, cnt);
}

int64_t ObCtxAllocatorState::get_used() const
{
  int64_t used = 0;
...
ObLabelItem ObCtxAllocatorState::get_label_usage(ObLabel &label) const
{
  ObLabelItem item;
  item.reset();
  IGNORE_RETURN iter_label([&](ObLabel &label_, LabelItem *l_item)
...
void ObCtxAllocatorState::update_wash_stat(int64_t related_chunks, int64_t blocks, int64_t size)
{
...
void ObCtxAllocator::on_alloc(AObject& obj, const ObMemAttr& attr, const bool light_backtrace_allowed)
{
...
void ObCtxAllocator::on_free(AObject& obj, ABlock& block)
{
...
void ObCtxAllocator::common_free(void *ptr)
{
  ObDisableDiagnoseGuard disable_diagnose_guard;
  if (NULL != ptr) {
    AObject *obj = reinterpret_cast<AObject*>((char*)ptr - AOBJECT_HEADER_SIZE);
