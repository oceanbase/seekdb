...
extern ObQSync& get_global_dchash_qsync();
...
public:
  ...
  public:
    Handle(DCHash& host, int& err, int node_change_count):
        host_(host), qsync_(get_global_dchash_qsync()), ref_(-1), array_(NULL),
...
public:
  ...
  Node* next(Node* node) {
    get_global_dchash_qsync().acquire_ref();
    while(NULL != (node = next_node(node))
          && node->is_dummy_node()) {
      // add usleep for dummy node test
      // usleep(1 * 1000 * 1000);
    }
    get_global_dchash_qsync().release_ref();
    return node;
  }
private:
