...
extern ObQSync& get_global_link_hashmap_qsync();
...
public:
  ...
  virtual void free_node(RefNode* ref_node) = 0;
  virtual ~ObILinkHashMap() {}
};
...
class ObLinkHashMap : public ObILinkHashMap
{
protected:
...
public:
  explicit ObLinkHashMap(int64_t min_size = DEFAULT_MIN_SIZE, int64_t max_size = INT64_MAX)
    : ref_handle_(get_retire_station()),
      ...
      magic_code_(0)
  {}
  ObLinkHashMap(AllocHandle alloc_handle, int64_t min_size = DEFAULT_MIN_SIZE, int64_t max_size = INT64_MAX)
  ...
  int insert_and_get(const Key &key, Value* value)
  {
    ...
    if (OB_ISNULL(node = alloc_node(value))) {
      ...
      if (OB_LIKELY(0 == hash_ret)) {
        ...
        tg.click();
        static_cast<ObILinkHashMap*>(node->host_)->free_node(node);
        tg.click();
      }
    }
...
private:
  ...
  virtual void free_node(RefNode* node) override
  {
    alloc_handle_.free_node(static_cast<HashNode*>(node));
  }
