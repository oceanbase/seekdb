...
public:
  typedef ObLinkHashMap<SessionID, ObDiagnosticInfo,
      DiagnosticInfoValueAlloc<ObDiagnosticInfo, SessionID>, oceanbase::common::RefHandle, 2>
      ObDiInfos;
...
public:
  ...
  int for_each_and_delay_release_ref(std::function<bool(const SessionID &, ObDiagnosticInfo *)> &fn);
