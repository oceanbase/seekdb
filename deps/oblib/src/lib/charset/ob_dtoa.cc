...
void ob_gcvt_help2(int *width, int *len, char **dend, char **src,
                   char **end, char **dst, int sign, int decpt, char *buf, size_t sizeofbuf,
                   int *exp_len, char **res, bool *error, double x)
{
  const char *dend_ptr = *dend;
  char *dst_ptr = *dst;
  char *src_ptr = *src;
...
{
  return ob_gcvt_strict(x, type, width, to, error, TRUE, FALSE);
}

size_t ob_gcvt_opt(double x, ob_gcvt_arg_type type, int width, char *to,
                      bool *error, bool is_binary_double)
{
  return ob_gcvt_strict(x, type, width, to, error,
                        is_binary_double, false);
}

size_t ob_gcvt_strict(double x, ob_gcvt_arg_type type, int width, char *to,
    bool *error, bool is_binary_double, bool use_force_e_format)
{
