...
void ob_fd_disable_ssl(ssl_state_st &ssl_st)
{
  if (OB_LIKELY(NULL == ssl_st.ssl)) {
    ...
    SSL_set_shutdown(ssl_st.ssl, mode);
    ERR_clear_error();
    SSL_shutdown(ssl_st.ssl);
    SSL_free(ssl_st.ssl);
    ssl_st.ssl = NULL;
    ssl_st.hand_shake_done = 0;
  }
}
...
ssize_t ob_read_regard_ssl(int fd, void *buf, size_t nbytes, ssl_state_st &ssl_st)
{
  ...
  if (OB_LIKELY(NULL == ssl_st.ssl)) {
    ...
    if (OB_UNLIKELY(0 == ssl_st.hand_shake_done)) {
      ...
      ERR_clear_error();
      rbytes = SSL_read(ssl_st.ssl, buf, nbytes);
      if (rbytes <= 0) {
        int ssl_error = 0;
        ssl_error = SSL_get_error(ssl_st.ssl, rbytes);
  ...
  if (OB_LIKELY(NULL == ssl_st.ssl)) {
    wbytes = write(fd, buf, nbytes);
     ...
     if (OB_UNLIKELY(0 == ssl_st.hand_shake_done)) {
      ...
      if (0 < (wbytes = SSL_write(ssl_st.ssl, buf, nbytes))) {
      } else {
        int ssl_error = 0;
        ssl_error = SSL_get_error(ssl_st.ssl, wbytes);
