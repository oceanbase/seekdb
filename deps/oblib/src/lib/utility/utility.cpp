...
bool read_cgroup_memory_limit(const char *path, int64_t &limit)
{
  ...
  if (nullptr != file) {
    ...
    if (nullptr != fgets(value_buf, sizeof(value_buf), file)) {
      char *value_start = value_buf;
      while (isspace(static_cast<unsigned char>(*value_start))) {
        ++value_start;
      }
      if (0 == strncmp(value_start, "max", 3) &&
          ('\0' == value_start[3] || isspace(static_cast<unsigned char>(value_start[3])))) {
        char *end = value_start + 3;
        ...
        char *end = nullptr;
        errno = 0;
        const unsigned long long value = strtoull(value_start, &end, 10);
        while (nullptr != end && isspace(static_cast<unsigned char>(*end))) {
          ++end;
        }
        if (0 == errno && end != value_start && '\0' == *end && value > 0 &&
...
bool unescape_mountinfo_path(const char *input, char *output, const int64_t output_size)
{
  ...
  while (succeeded && '\0' != input[input_pos]) {
    ...
    int64_t consumed = 1;
    if ('\\' == value && input[input_pos + 1] >= '0' && input[input_pos + 1] <= '7' &&
                             ...
                             const int64_t mounted_group_path_size)
{
  ...
  if (succeeded) {
    const int copied = snprintf(
        mounted_group_path, mounted_group_path_size, "%s", relative_path);
    succeeded = copied > 0 && copied < mounted_group_path_size;
  }
  if (!succeeded && nullptr != mounted_group_path && mounted_group_path_size > 0) {
    mounted_group_path[0] = '\0';
  }
  return succeeded;
}

bool update_cgroup_hierarchy_limit(const CgroupMountInfo &mount_info,
                                   const char *group_path,
  ...
  if (!get_mounted_cgroup_path(
    ...
    while (!finished) {
      ...
      const int path_length = 0 == strcmp(current_path, "/")
          ...
          : snprintf(limit_path, sizeof(limit_path), "%s%s/%s", mount_info.mount_point_,
                     current_path, limit_file);
...
void set_default_cgroup_mount(const char *mount_point, CgroupMountInfo &mount_info)
{
  const int root_length = snprintf(
      mount_info.root_, sizeof(mount_info.root_), "%s", "/");
  const int mount_point_length = snprintf(
      mount_info.mount_point_, sizeof(mount_info.mount_point_), "%s", mount_point);
  mount_info.valid_ = root_length > 0 &&
                      root_length < static_cast<int>(sizeof(mount_info.root_)) &&
                      mount_point_length > 0 &&
                      mount_point_length < static_cast<int>(sizeof(mount_info.mount_point_));
}
                       ...
                       CgroupMountInfo &mount_info)
{
  mount_info.valid_ =
      unescape_mountinfo_path(
          escaped_root, mount_info.root_, sizeof(mount_info.root_)) &&
      unescape_mountinfo_path(
          escaped_mount_point, mount_info.mount_point_, sizeof(mount_info.mount_point_)) &&
      '/' == mount_info.root_[0] && '/' == mount_info.mount_point_[0];
  return mount_info.valid_;
}

void update_cgroup_limits_from_mountinfo(const char *cgroup_v1_path,
                                         const char *cgroup_v2_path,
                                         const bool scan_matching_roots,
                                         bool &parsed_v1_limit,
                                         bool &parsed_v2_limit,
                                         int64_t &minimum_limit)
{
  // Prefix-matching mounts are authoritative. A second pass over nonmatching
  // roots supports cgroup namespaces that expose group paths relative to the
  // namespace root. The second pass is used only when no matching mount
  // yielded a readable limit file.
  const bool scan_v1 = '\0' != cgroup_v1_path[0] && !parsed_v1_limit;
  const bool scan_v2 = '\0' != cgroup_v2_path[0] && !parsed_v2_limit;
  FILE *file = scan_v1 || scan_v2 ? fopen("/proc/self/mountinfo", "r") : nullptr;
  if (nullptr != file) {
    bool found_v1_limit = false;
    bool found_v2_limit = false;
    ...
    while (nullptr != fgets(line, sizeof(line), file)) {
      ...
      if (nullptr != separator) {
        *separator = '\0';
        ...
        IGNORE_RETURN strtok_r(nullptr, " \t\r\n", &right_save_ptr); // mount source
        char *super_options = strtok_r(nullptr, " \t\r\n", &right_save_ptr);
        const bool is_cgroup_v2 = scan_v2 && nullptr != file_system_type &&
                                  0 == strcmp(file_system_type, "cgroup2");
        const bool is_cgroup_v1 = scan_v1 && nullptr != file_system_type &&
                                  nullptr != super_options &&
                                  0 == strcmp(file_system_type, "cgroup") &&
                                  controller_list_contains(super_options, "memory");
        if (nullptr != root && nullptr != mount_point &&
            (is_cgroup_v1 || is_cgroup_v2)) {
          CgroupMountInfo mount_info;
          if (init_cgroup_mount(root, mount_point, mount_info)) {
            const char *group_path = is_cgroup_v2 ? cgroup_v2_path : cgroup_v1_path;
            const bool root_matches =
                is_cgroup_path_prefix(mount_info.root_, group_path);
            if (root_matches == scan_matching_roots) {
              const bool parsed_limit = update_cgroup_hierarchy_limit(
                  mount_info, group_path,
                  is_cgroup_v2 ? "memory.max" : "memory.limit_in_bytes",
                  minimum_limit);
              if (is_cgroup_v2) {
                found_v2_limit = found_v2_limit || parsed_limit;
              } else {
                found_v1_limit = found_v1_limit || parsed_limit;
              }
            }
          }
        }
    ...
    fclose(file);
    parsed_v1_limit = parsed_v1_limit || found_v1_limit;
    parsed_v2_limit = parsed_v2_limit || found_v2_limit;
  }
}
...
int64_t get_cgroup_memory_limit()
{
    ...
    while (nullptr != fgets(line, sizeof(line), file)) {
      ...
      if (nullptr != first_colon && nullptr != second_colon) {
        ...
        *second_colon = '\0';
        ...
        char *controllers = first_colon + 1;
        char *group_path = second_colon + 1;
        group_path[strcspn(group_path, "\r\n")] = '\0';
        if (0 == strcmp(hierarchy_id, "0") && '\0' == controllers[0]) {
          const int copied = snprintf(
              cgroup_v2_path, sizeof(cgroup_v2_path), "%s", group_path);
          if (copied <= 0 || copied >= static_cast<int>(sizeof(cgroup_v2_path))) {
            cgroup_v2_path[0] = '\0';
          }
        } else {
          char *save_ptr = nullptr;
          for (char *controller = strtok_r(controllers, ",", &save_ptr);
               nullptr != controller;
               controller = strtok_r(nullptr, ",", &save_ptr)) {
            if (0 == strcmp(controller, "memory")) {
              const int copied = snprintf(
                  cgroup_v1_path, sizeof(cgroup_v1_path), "%s", group_path);
              if (copied <= 0 || copied >= static_cast<int>(sizeof(cgroup_v1_path))) {
                cgroup_v1_path[0] = '\0';
              }
              break;
            }
  ...
  int64_t minimum_limit = INT64_MAX;
  bool parsed_v1_limit = false;
  bool parsed_v2_limit = false;
  update_cgroup_limits_from_mountinfo(
      cgroup_v1_path, cgroup_v2_path, true,
      parsed_v1_limit, parsed_v2_limit, minimum_limit);
  update_cgroup_limits_from_mountinfo(
      cgroup_v1_path, cgroup_v2_path, false,
      parsed_v1_limit, parsed_v2_limit, minimum_limit);
  if ('\0' != cgroup_v2_path[0] && !parsed_v2_limit) {
    CgroupMountInfo cgroup_v2_mount;
    set_default_cgroup_mount("/sys/fs/cgroup", cgroup_v2_mount);
    IGNORE_RETURN update_cgroup_hierarchy_limit(
        cgroup_v2_mount, cgroup_v2_path, "memory.max", minimum_limit);
  }
  if ('\0' != cgroup_v1_path[0] && !parsed_v1_limit) {
  ...
  return INT64_MAX == minimum_limit ? 0 : minimum_limit;
}
} // namespace
#endif
