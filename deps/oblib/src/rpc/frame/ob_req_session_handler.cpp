...
ObReqSessionHandler::ObReqSessionHandler()
{
  sessid_ = ObTimeUtility::current_time();
  ObMemAttr attr(ObModIds::OB_HASH_NODE_NEXT_WAIT_MAP);
  SET_USE_500(attr);
...
int64_t ObReqSessionHandler::generate_session_id()
{
  return ATOMIC_AAF(&sessid_, 1);
}

bool ObReqSessionHandler::wakeup_next_thread(ObRequest &req)
{
  bool bret = false;
  ...
  if (OB_SUCCESS != (get_session_id(req, sessid))) {
    ...
    if (OB_SUCCESS != hash_ret) {
      ...
      if (OB_SUCCESS != hash_ret) {
        ...
        // obcall RPC stream transport removed: dead path (no OB_RPC stream req).
        LOG_WARN_RET(OB_ERR_UNEXPECTED, "previous stream request hasn't been processed yet, "
                     ...
                     "previous request", *wait_object.req_);
        bret = false;
      } else {
...
int ObReqSessionHandler::prepare_for_next_request(int64_t sessid)
{
  int ret = OB_SUCCESS;
...
int ObReqSessionHandler::get_session_id(const ObRequest &req, int64_t &session_id) const
{
  int ret = OB_SUCCESS;

  // obcall RPC stream transport removed: no OB_RPC request carries a stream
  // session id any more. This is a dead path.
  UNUSED(req);
...
int ObReqSessionHandler::destroy_session(int64_t sessid)
{
  int ret = OB_SUCCESS;
  WaitObject wait_object;
  int hash_ret = next_wait_map_.get_refactored(sessid, wait_object);
  if (OB_SUCCESS != hash_ret) {
    ...
    if (wait_object.req_) {
      RPC_REQ_OP.response_result(wait_object.req_);
...
int ObReqSessionHandler::wait_for_next_request(int64_t sessid,
                                               ObRequest *&req,
