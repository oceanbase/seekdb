...
{
  if (nullptr != argv) {
    if (0 == strcasecmp(argv, facility_str[MY])) {
      facility = MY;
    }
  }
}
...
bool parse_param(int args, char* argv[])
{
  ...
  if (1 < args && 3 >= args) {
    ...
    int c = getopt_long(args, argv, ":hV", long_options, &option_index);
    if (-1 != c) {
      switch (c) {
        case 'h': {
          print_help();
...
static bool ob_init_error_to_oberror(int mysql_err[][OB_MAX_SAME_ERROR_COUNT], int os_err[][OS_MAX_SAME_ERROR_COUNT])
{
  bool bret = true;
  int error_code = -1;
  int k = 0;
...
bool init_global_info()
{
  ...
  return ob_init_error_to_oberror(g_mysql_error, g_os_error);
}
