...
function _obd_sh_complete_func
{
  ...
  all_cmds["$*"]="prepare deploy redeploy reinstall start stop restart destroy upgrade mysqltest pid ssh less gdb sql mysql edit list sys display create_tenant drop_tenant sysbench tpch tpcc tpcds graph display-trace set-config"
  case $prev in
  list)
    return 0
    ;;
