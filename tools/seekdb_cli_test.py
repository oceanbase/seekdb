...
class FakeSeekdbServer:
    ...
    def _send_resultset(
        self, conn, start_seq, columns, rows, default_in_columns=True, more_after=False
    ):
        seq = start_seq
        ...
        for name in columns:
            ...
            seq += 1
        status = 2 | (seekdb_cli.SERVER_MORE_RESULTS_EXISTS if more_after else 0)
        send_packet_raw(conn, seq, b"\xfe" + struct.pack("<HH", 2, status))
        seq += 1
        for row in rows:
            send_packet_raw(conn, seq, row_packet(row))
            seq += 1
        status = 2 | (seekdb_cli.SERVER_MORE_RESULTS_EXISTS if more_after else 0)
        send_packet_raw(conn, seq, b"\xfe" + struct.pack("<HH", 0, status))
        return seq + 1
    ...
    def _handle_command(self, conn, payload):
        ...
        if upper.startswith("SELECT 1 AS ID"):
        ...
        elif upper.startswith("SELECT '' AS X"):
            self._send_resultset(conn, 1, ["x"], [("",)])
            ...
            next_seq = self._send_resultset(
                conn, 1, ["a"], [(1,)], more_after=True
            )
            self._send_resultset(conn, next_seq, ["b"], [(2,), (3,)])
...
class PacketStatusTests(unittest.TestCase):
    def test_eof_status_flags_after_warnings(self):
        # Classic EOF packet: 0xfe, warning_count (2 bytes), then status.
        status = 2 | seekdb_cli.SERVER_MORE_RESULTS_EXISTS
        payload = b"\xfe" + struct.pack("<HH", 2, status)
        self.assertEqual(seekdb_cli.packet_status(payload), status)

    def test_ok_status_flags_before_warnings(self):
        # OK packet: 0x00, affected rows, last insert id, then status.
        status = 2 | seekdb_cli.SERVER_MORE_RESULTS_EXISTS
        payload = b"\x00\x00\x00" + struct.pack("<HH", status, 0)
        self.assertEqual(seekdb_cli.packet_status(payload), status)

    def test_unknown_payload_returns_zero(self):
        self.assertEqual(seekdb_cli.packet_status(b"\x23\x01\x02"), 0)
...
class SendPacketTests(unittest.TestCase):
    ...
    def test_exact_max_packet_gets_terminator(self):
        ...
        try:
            ...
            self.assertFalse(readable, "no extra packet may follow the terminator")
...
class RenderTests(unittest.TestCase):
    ...
    def test_clip_to_width_truncates_with_ellipsis(self):
        self.assertEqual(seekdb_cli.clip_to_width("abcdefgh", 6), "abc...")

    def test_clip_to_width_keeps_short_text(self):
        self.assertEqual(seekdb_cli.clip_to_width("abc", 10), "abc")

    def test_render_batch_uses_tabs_and_null(self):
        self.assertEqual(
            seekdb_cli.render_batch(["a", "b"], [(1, None)]),
            "a\tb\n1\tNULL",
        )

    def test_render_vertical_labels_rows(self):
        out = seekdb_cli.render_vertical(["a", "b"], [(1, "x")])
...
class NamedPipeTests(unittest.TestCase):
    def test_discover_socket_uses_pipe_file_on_windows(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            (Path(tmp) / "run").mkdir()
            (Path(tmp) / "run" / "sql.pipe").write_text("12345-6789")
            args = seekdb_cli.build_parser().parse_args(["--data-dir", tmp])
            expected = str(Path(tmp) / "run" / "sql.pipe")
            with patch("seekdb_cli.os.name", "nt"):
                self.assertEqual(seekdb_cli.discover_socket(args), expected)

    def test_discover_socket_ignores_pipe_file_on_posix(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            (Path(tmp) / "run").mkdir()
            (Path(tmp) / "run" / "sql.pipe").write_text("12345-6789")
            args = seekdb_cli.build_parser().parse_args(["--data-dir", tmp])
            self.assertIsNone(seekdb_cli.discover_socket(args))

    def test_named_pipe_resolve_reads_discovery_file(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            discovery = Path(tmp) / "sql.pipe"
            discovery.write_text("12345-6789")
            self.assertEqual(
                seekdb_cli.named_pipe_resolve(str(discovery)),
                "\\\\.\\pipe\\12345-6789",
            )

    def test_named_pipe_resolve_accepts_explicit_pipe_path(self):
        self.assertEqual(
            seekdb_cli.named_pipe_resolve("\\\\.\\pipe\\12345-6789"),
            "\\\\.\\pipe\\12345-6789",
        )

    def test_named_pipe_resolve_rejects_empty_discovery_file(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            discovery = Path(tmp) / "sql.pipe"
    ...
    def test_named_pipe_resolve_rejects_missing_file(self):
        with self.assertRaises(seekdb_cli.ProtocolError):
            seekdb_cli.named_pipe_resolve("/nonexistent/run/sql.pipe")
...
class HandshakeTests(unittest.TestCase):
    ...
    def test_parse_ok_reads_affected_and_warnings(self):
        payload = (
            b"\x00"
            + b"\x05"  # affected rows
            + b"\x00"  # last insert id
            + struct.pack("<HH", 2, 3)  # status, warnings
            + b"done"
        )
        ok = seekdb_cli.parse_ok(payload)
        self.assertEqual(ok["affected_rows"], 5)
        self.assertEqual(ok["status"], 2)
        self.assertEqual(ok["warnings"], 3)
        self.assertEqual(ok["info"], "done")
...
class EndToEndTests(unittest.TestCase):
    ...
    def test_execute_vertical(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            server = FakeSeekdbServer(tmp)
            try:
                result = self._run_cli(
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertIn("1. row", result.stdout)
                self.assertIn("id: 1", result.stdout)
                self.assertIn("name: hello", result.stdout)
            finally:
                server.close()
    ...
    def test_missing_socket(self):
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            ...
            self.assertEqual(result.returncode, 1)
            self.assertIn("no seekdb SQL socket/named pipe found", result.stderr)

    def test_stale_socket_gives_clear_error(self):
        """A leftover socket with no listener must hint that the app stopped."""
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            run_dir = Path(tmp) / "run"
            run_dir.mkdir()
            (run_dir / "sql.sock").touch()
            result = self._run_cli(
                tmp,
                "--data-dir",
                str(tmp),
            ...
            self.assertIn("SQL socket exists", result.stderr)
    ...
    def test_execute_call_multiple_resultsets(self):
        """CALL-style statements with several result sets must drain all of them."""
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            server = FakeSeekdbServer(tmp)
            try:
                result = self._run_cli(
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertIn("1 row(s) in set", result.stdout)
                self.assertIn("2 row(s) in set", result.stdout)
                self.assertIn("a", result.stdout)
                self.assertIn("b", result.stdout)
            finally:
                server.close()

    def test_execute_empty_string_row_does_not_desync(self):
        """Rows whose first cell is an empty string must not be read as OK."""
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            server = FakeSeekdbServer(tmp)
            try:
                result = self._run_cli(
    ...
    def test_missing_socket_file_gives_clear_error(self):
        """A --socket path that does not exist must fail with a clear message."""
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            result = self._run_cli(
                tmp,
                "--socket",
                str(Path(tmp) / "run" / "sql.sock"),
                "-e",
                "SELECT 1;",
            )
            self.assertEqual(result.returncode, 1)
            self.assertIn("cannot connect to SQL socket", result.stderr)
    ...
    def test_interactive_exits_when_connection_lost(self):
        """A dropped connection must end the session instead of looping forever."""
        try:
            import pty
        except ImportError:
            self.skipTest("pty is not available on this platform")
        with tempfile.TemporaryDirectory(prefix="seekdb-cli-e2e-") as tmp:
            ...
            try:
                ...
                text = output.decode("utf-8", "replace")
                self.assertEqual(rc, 0, text)
                self.assertIn("connection to the database was lost", text)
            finally:
                server.close()
