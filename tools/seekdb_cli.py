...
the database is running; Windows embedded mode publishes a named pipe at
``<data-dir>/run/sql.pipe``.  This CLI speaks the MySQL wire protocol directly
over that socket/pipe (or over TCP for server mode), so it has no dependency
on pymysql or any other client library.
...
# Windows embedded seekdb exposes its SQL service on a named pipe whose name
# is published in a discovery file at ``<data-dir>/run/sql.pipe``.
...
def packet_status(payload):
    """Return the server status flags carried by an EOF/OK terminator.

    The classic five-byte EOF packet is 0xfe, warning_count (2 bytes) and
    then status_flags (2 bytes); the OK packet carries status_flags before
    warnings.
    """
    if payload and payload[0] == EOF_PACKET and len(payload) >= 5:
        return struct.unpack_from("<H", payload, 3)[0]
    if payload and payload[0] == OK_PACKET and len(payload) >= 7:
        return parse_ok(payload)["status"]
    return 0


def parse_results(sock):
    """Read one resultset (or OK/error); return (columns, rows, ok, more).

    ``more`` is true when the server still has another resultset queued for
    the same statement (for example a stored procedure call).
    """
    ...
    if first == OK_PACKET:
        ok = parse_ok(payload)
        return None, None, ok, bool(ok["status"] & SERVER_MORE_RESULTS_EXISTS)
    if first == ERR_PACKET:
    ...
    # EOF after the column definitions (classic) or OK when the result set
    # is empty (DEPRECATE_EOF, which we do not negotiate).
    eof, _ = read_packet(sock)
    more = False
    if eof and eof[0] == OK_PACKET:
        ok = parse_ok(eof)
        return columns, [], ok, bool(ok["status"] & SERVER_MORE_RESULTS_EXISTS)
    if eof and eof[0] == EOF_PACKET and len(eof) == 5:
        more = bool(packet_status(eof) & SERVER_MORE_RESULTS_EXISTS)
    ...
    while True:
        ...
        # In the classic protocol the terminator is a five-byte EOF packet
        # (0xfe, server status, warnings).  Rows may legitimately start with
        # any byte -- including 0x00 when the first cell is an empty string --
        # so only an exact-length EOF ends the result set here.
        if row_payload[0] == EOF_PACKET and len(row_payload) == 5:
            more = more or bool(
                packet_status(row_payload) & SERVER_MORE_RESULTS_EXISTS
            )
            return columns, rows, None, more
...
def execute_statement(sock, statement):
    """Execute one statement and return all of its result sets.

    Each entry is a ``(columns, rows, ok)`` tuple as produced by
    ``parse_results``; statements that yield more than one result set (for
    example stored procedure calls) are drained until the server is done.
    """
    send_packet(sock, 0, bytes((COM_QUERY,)) + statement.encode("utf-8"))
    results = []
    while True:
        columns, rows, ok, more = parse_results(sock)
        results.append((columns, rows, ok))
        if not more:
            break
    return results
...
def discover_socket(args):
    ...
    endpoint_file = "sql.pipe" if os.name == "nt" else "sql.sock"
    for directory in candidates:
        sock_path = os.path.join(directory, "run", endpoint_file)
        if os.path.exists(sock_path):
            return sock_path
    return None


def named_pipe_resolve(hint):
    """Return the Windows named-pipe path for an embedded endpoint.

    ``hint`` is either an explicit ``\\\\.\\pipe\\<name>`` path or the path to
    the discovery file (``run/sql.pipe``) whose content names the live pipe.
    """
    if hint.startswith(WINDOWS_PIPE_PREFIX):
        return hint
    try:
        with open(hint, "r", encoding="ascii") as handle:
            bare_name = handle.read().strip()
    except OSError as exc:
        raise ProtocolError(
            "cannot read named-pipe discovery file {!r}: {}".format(hint, exc)
        ) from None
    if not bare_name:
        raise ProtocolError("empty named-pipe discovery file {!r}".format(hint))
    return WINDOWS_PIPE_PREFIX + bare_name
...
def connect(args):
    ...
    if args.host:
        ...
        if socket_path is None:
            raise ProtocolError(
                "no seekdb SQL socket/named pipe found; is the embedded database running? "
                "Pass --socket PATH, or --host/--port for server mode."
            )
        if os.name == "nt":
            pipe_path = named_pipe_resolve(socket_path)
            if not hasattr(socket, "AF_UNIX"):
                raise ProtocolError(
                    "connecting to a Windows named pipe requires Python 3.9 or newer; "
                    "use --host/--port to connect over TCP instead"
                )
            ...
            try:
                ...
                raise ProtocolError(
                    "cannot connect to named pipe {!r}; is the embedded database "
                    "running? (was the application stopped?)".format(pipe_path)
                ) from None
        else:
            sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            try:
                ...
                raise ProtocolError(
                    "connection refused on {!r}; the SQL socket exists but no embedded "
                    "database is accepting connections right now (was the application "
                    "stopped?)".format(socket_path)
                ) from None
            except OSError as exc:
                raise ProtocolError(
                    "cannot connect to SQL socket {!r}: {}".format(socket_path, exc)
                ) from None
    ...
    if sock is None:
        try:
            ...
            raise ProtocolError(
                "connection refused on {}:{}; is a seekdb server running there?".format(
                    endpoint[0], endpoint[1]
                )
            ) from None
        except OSError as exc:
            raise ProtocolError(
                "cannot connect to {}:{}: {}".format(endpoint[0], endpoint[1], exc)
            ) from None
    else:
...
def run_statements(args, statements):
    ...
    try:
        ...
        for statement in statements:
            try:
                for columns, rows, ok in execute_statement(sock, statement):
...
def build_parser():
    ...
    parser.add_argument(
        ...
        "-S",
        default=None,
        help="path to the local SQL socket (or named-pipe path on Windows)",
    )
    parser.add_argument(
    ...
    try:
        ...
        while True:
            if not connection_alive:
                ...
                print(
                    "seekdb-cli: the connection to the database was lost; exiting",
                    file=sys.stderr,
                )
                break
            try:
            ...
            if text.strip():
                ...
                if executed:
                    for stmt in executed:
                        try:
                            for columns, rows, ok in execute_statement(sock, stmt):
                                print_result(columns, rows, ok, args)
                        except MysqlError as exc:
                            print(str(exc), file=sys.stderr)
                        except Exception as exc:
