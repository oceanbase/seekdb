...
def render_table(columns, rows, max_width):
    ...
    lines = []
    lines.append("+".join("-" * (min(w, max_width) + 2) for w in widths))
    lines.append(
        "| "
        + " | ".join(clip(h, min(w, max_width)).ljust(min(w, max_width)) for h, w in zip(headers, widths))
        + " |"
    )
    lines.append("+".join("-" * (min(w, max_width) + 2) for w in widths))
    for row in cells:
        lines.append(
            "| "
            + " | ".join(clip(v, min(w, max_width)).ljust(min(w, max_width)) for v, w in zip(row, widths))
            + " |"
        )
    lines.append("+".join("-" * (min(w, max_width) + 2) for w in widths))
    return "\n".join(lines)
    ...
    try:
        ...
        while True:
            try:
                ...
                break
            trimmed = line.strip()
            if not buffer_lines and trimmed.lower() in ("exit", "quit", "\\q"):
                break
